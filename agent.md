...
### 2026-04-06 주문 확정 여부 제거 기준 (최신)

- 주문 도메인에서 `확정 여부(계획/확정)` 개념은 더 이상 사용하지 않는다.
- 주문 목록/상세 화면에서 `확정 여부` 필터/컬럼/입력 UI는 사용하지 않는다.
- 주문 수정 잠금 판단 기준은 아래 2가지만 사용한다.
  - 수동 잠금(`modificationLockedAt`)
  - 배정 계약 데이터 존재 여부
- `WorkOrder.confirmationStatus` 컬럼은 DB 호환을 위해 남아 있을 수 있으나, 서버/화면 로직에서는 레거시로 취급한다.
- 문서 하단의 `확정 여부` 관련 과거 메모는 현재 기준으로 적용하지 않는다.

### 2026-04-06 배정 업무 상세 ST/CT 편집 및 스타일 ST 동기화

- 작업 배정 > 업무 상세의 공정 표에서 `ST`, `CT`는 모두 직접 입력 가능한 값으로 사용한다.
- 저장 시 assignment `ctSnapshot`에는 공정별 `stSeconds`, `ctSeconds`를 각각 저장한다.
- 저장된 `stSeconds`는 스타일 공정 표준(`StyleProcessStandard`)의 해당 수량 버킷 ST 값으로 동기화한다.
  - 매칭 기준은 `styleId + processCode` 우선, 필요 시 공정명 fallback을 사용한다.
  - 버킷은 주문 수량을 `ST_STANDARD_BUCKETS` 기준으로 변환한 구간으로 저장한다.
- 스타일 상세 `수량별 시간`에서 수동 ST 강조색은 파란 계열을 사용한다.

### 2026-04-10 작업 기록(기록 목록/기록 상세) 기준

- 기록 목록(`WorkList`)은 `1인 평균 CT`를 기본 표시한다.
  - 계산식: `총 CT / 작업자 수(workerCount)`
  - 작업자 수가 `0`이면 평균 CT는 `-`로 표시한다.
- 기록 상세(`WorkDetail`)에서 다른 라인에 배정된 예외 스타일을 선택한 경우,
  - CT는 **현재 선택한 배정카드(해당 라인)의 공정 CT**를 기준으로 매칭한다.
  - 공정 매칭 우선순위는 `processKey -> processId -> processCode -> 공정명(다국어)` 순서로 적용한다.
- 작업 기록은 같은 작업일(`workDate`) 기준으로 `작업자 + 스타일 + 공정` 조합 중복 입력을 허용하지 않는다.
  - 프론트 입력 단계에서 공정 선택 옵션을 필터링한다.
  - 서버 저장 단계에서도 동일 조건을 재검증해 우회 저장을 막는다.
- 공정명 다국어 표시 안정성을 위해 작업/배정 스냅샷에는 가능한 한 `processId`, `nameKo`, `nameEn`, `nameVi`를 함께 보존한다.
...
- 스타일 상세에 `수량별 시간` 탭을 추가해 ST 버킷별(`1,3,5,10,30,50,100,300,500,1000,3000,5000,10000`) PT/ST/AT를 한 화면에서 확인한다.
  - 표는 `공정`을 3행(PT/ST/AT)으로 나눈 행 구조를 사용한다.
  - ST 셀은 탭 안에서 직접 입력/수정할 수 있고, 입력 시 해당 버킷 `stValues`를 `setBy=MANUAL`로 저장한다.
  - 시간 단위는 모두 초를 기준으로 하며 `초` 텍스트 표시는 생략한다.
  - ST 셀은 해당 버킷에 수동 입력된 값(`setBy=MANUAL|LEGACY`)일 때 파란 강조색만 준다. (`수동` 텍스트 배지는 표시하지 않음)
...
- ST 버킷은 `1~3~5~10~30~50~100~300~500~1000~3000~5000~10000` 구간으로 관리한다.
...
- 공장 선택 → 라인 선택 → 작업자 선택 → 스타일 선택 → 공정 선택 → 수량 입력
- 기록 상세에서 배정 카드 직접 선택 단계는 사용하지 않는다.
- 스타일/공정 옵션은 `진행` 상태이면서 `ctSnapshot`이 있는 배정 기준으로만 제공한다.
  - 내 라인 진행 배정 스타일/공정은 일반 항목으로 표시한다.
  - 다른 라인 진행 배정 스타일/공정은 선택 가능하되 `예외` 라벨로 표시한다.
- 미배정 스타일(배정 없음)은 작업 기록 스타일 옵션에서 제외한다.
- 배정의 `ctSnapshot.processes`가 비어 있는 데이터는 공정 선택 옵션을 만들지 않는다. (`전체 공정` 같은 synthetic 옵션 사용 금지)
- WorkRecord에 `assignmentPlanId` 자동 연결 → WorkRecord ↔ AssignmentPlan 연결 확보
- 현재는 배정 기반 작업 기록만 지원한다. (배정 없는 작업 기록은 별도 정책 확정 전까지 제외)
...
- 배정 카드 우클릭 메뉴에 `진행/완료` 토글을 둔다.
- **완료 카드(`isCompleted=true`)는 고정 카드로 취급**한다.
  - 드래그/드롭 이동 비활성화
  - 수량 분할 비활성화
  - 자동 리플로우/밀어내기 대상에서 제외 (다른 카드 재배치 시 완료 카드를 넘겨 밀 수 없음)
- 파일: `frontend/src/pages/App/assign/AssignBoard.jsx`
