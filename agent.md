...
- 테스트용 샘플 주문 1건:
  - `orderNumber = TSBR-AM01160-20260315-A`
  - `style = AM01160`
  - `color = NAVY`
  - `gender = M`
  - `quantity = 7,000`
- 현재 `Sample Line 1`, `Sample Line 2`가 각각 20명일 때, 위 주문은 두 라인 합산 기준 약 `34.07일` 분량이다.
- 카드 수를 늘리지 않기 위해 `1스타일 + 1색상 + 1성별` 단일 카드로 넣는다.
...
- `ST(q)`: 수량 q 주문 기준의 개당 표준시간, 시작점은 `PT(1000)`
- `CT(q)`: 배정 건별 수량 q 기준의 개당 저장 CT, 기본값은 `ST(q)`이며 건별 조정 가능
...
- `진행 단계`:
  - 값: `---`, `접수`, `제작`, `출고`, `정산`
  - 의미: **확정된 주문**이 현재 어디까지 진행됐는지 보여주는 단계
- `계획` 주문은 진행 단계가 없다. 화면에서는 `---`로 표시한다.
- `확정` 주문은 진행 단계가 반드시 의미를 가진다. 목록 필터에서는 `확정 여부=확정`일 때 `---`를 선택할 수 없다.
...
- `확정 여부`가 `계획 -> 확정`으로 바뀌면 진행 단계는 자동으로 `접수`가 된다.
- 확정된 주문에 연결된 카드로 작업 기록(`WorkRecord`)이 생성되면 진행 단계는 자동으로 `제작`으로 바뀐다.
...
- 현재 화면 라벨 기준:
  ...
  - `SHIPPED` = `출고`
  - `SETTLED` = `정산`
- `PRODUCTION_DONE`는 과거 상태값 호환용 레거시로 남아 있으며, 신규 정책의 주 사용값으로 보지 않는다.

### 아직 안 만든 것 / 이후 개발 메모

- `출고` 자동 전환:
  - 나중에 출고 기능이 생기면, 그 이벤트 시점에 확정 주문의 진행 단계를 `출고`로 자동 변경한다.
- `정산` 자동 전환:
  - 나중에 정산 기능이 생기면, 그 이벤트 시점에 확정 주문의 진행 단계를 `정산`으로 자동 변경한다.
- `확정 주문 수정 워크플로우`:
  - 아직 구현하지 않았다.
  - 정책 확정 후 개발한다.
  - 검토 후보:
    ...
    - 수정 요청 워크플로우
    - 변경 주문(delta) 분리
- 출고/정산 기능을 붙일 때도 `확정 여부`와 `진행 단계`를 하나의 상태로 합치지 않는다.

### 로딩 / 재요청 원칙

- 탭 전환만으로는 데이터 재요청이 발생하면 안 된다.
- 목록 재로딩은 전역 카운터가 아니라 명시적 새로고침 신호로만 처리한다.
- 숨겨진 keep-alive 탭은 새로고침 신호가 와도 즉시 재요청하지 않고, 다시 활성화될 때만 처리한다.
- 현재 적용 대상:
  - 스타일 저장 후 `스타일` 목록 갱신
  - 스타일 변경 후 `주문` 화면의 스타일 선택 목록 갱신
  - 주문 저장 후 `주문` 목록 갱신
