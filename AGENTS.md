...
- 공장 설정의 `wagePerSecond`는 전체 급여의 초당 환산값이 아니라 공장 공통 생산수당 초당 단가다. 신규 저장은 이 단가를 직접 입력하며 `targetMonthlyWage`에서 역산하지 않는다. 레거시 물리 컬럼명은 향후 안전한 마이그레이션 전까지 호환용으로만 보존한다.
