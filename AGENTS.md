...
- 생산수당 화면은 공장 선택 없이 선택 월의 모든 공장 작업기록을 함께 계산한다. 월 필터는 관리 시작 월부터 현재 월까지 허용한다. 종료된 월은 계산 후 월 단위 스냅샷으로 확정하지만, 현재 월은 이후 작업기록이 계속 추가되므로 조회 시점까지의 예상 생산수당만 계산하고 스냅샷으로 확정하지 않는다.
