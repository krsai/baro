...
- 메인 목록은 월별 계산본을 `월 1행`으로 표시하고 월별로 생성·잠금/해제·삭제한다. 잠금과 삭제 UI는 주문 목록의 공통 스위치·휴지통 패턴을 사용하며 잠긴 월은 해제 후에만 삭제한다. 월 행을 선택하면 해당 월 전체의 공장·라인·직원·공정별 상세를 연다. 공장·라인별 작업기록·출퇴근·계산근거 준비 상태는 선택한 계산 월의 사전 점검 정보이지 별도 계산본 행이 아니다.
