...
- 스타일 상세의 공정 추가·수정 화면에는 필수 선택 필드 `작업 종류`를 둔다. 사용자 표시값은 봉제·다림질·검품·포장이고 서버/DB 소스오브트루스는 `ProductionStage`의 `SEWING`, `IRONING`, `INSPECTION`, `PACKING`이다. 공정명 문자열로 작업 종류를 추론하지 않는다.
...
- 스타일 공정 목록은 작업 종류를 표시하고 종류별로 구분해 볼 수 있어야 한다. 기존 스타일을 열면 백필된 기존 공정이 모두 봉제로 표시되어야 하며 기존 PT·ST·CT·AT와 배정 snapshot 값은 작업 종류 백필만으로 변경하지 않는다.
