...
- 영업 관리의 청구서를 하위 그룹으로 바꾸고 청구서 목록(`/invoices/list`)과 청구서 작성/수정(`/invoices/new`)을 분리했다. `/invoices`는 목록으로 리다이렉트한다. 목록의 작성 버튼과 작성 화면의 목록 버튼은 공통 `navigateToPath`를 사용해 탭·미저장 변경 보호 흐름을 따른다.
- 기존 고객 선택·다중 주문 초안은 작성 경로에서 유지한다. 실제 발행 목록·저장된 청구서 수정은 서버 원장 구현 전까지 미지원이다. 메뉴 명칭만으로 저장/수정 구현 완료를 뜻하지 않는다.
- 청구서 회귀 검사, 작업공간 탐색 검사, 번역 키 검사 및 양쪽 빌드로 검증한다. 실제 브라우저 조작 검증은 미실행이다. 상세 진행은 `INVOICE_LOCK_ROADMAP.md`를 따른다.
