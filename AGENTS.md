...
- 관측 범위 밖이지만 최소 관측수량의 1/2 이상, 최대 관측수량의 2배 이하인 수량은 회귀선을 연장하지 않고 가장 가까운 실제 관측점의 장당 AT를 범위 밖 참고값으로 표시한다. 허용 배수 밖은 계산하지 않으며 모든 표시값은 ST 최소 기준과 양수 검사를 계속 통과해야 한다.
