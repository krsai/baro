...
- 과소·초과 청구가 가능하므로 `누적 청구량 >= 주문량`만으로 완료 처리하지 않는다. 분할 청구 중 잔량과 최종 과소/초과 마감을 구별한다. 정상 수량의 자동/명시 마감 여부와 차이 마감의 사유·승인·허용 기준은 미확정이며 임의 오차율을 만들지 않는다. 청구 완료는 입금 완료와 별도다.
...
아래는 기존 코드 현황이다. 2026-09-14 요구에 따라 별도 메뉴·분할 청구로 전환할 예정이며 현재 주문 상세 초안 진입점과 전체 생산 완료 조건을 새 설계의 확정 계약으로 해석하지 않는다.
