...
## 2026-09-25 외주 업체 FK 표시·작업 종류 분류

- 외주 기록은 이미 `outsourcingPartnerId -> Organization.id` FK로 연결돼 있었으나 목록 DTO가 업체의 현재 이름 대신 저장 당시 `outsourceVendorName` 스냅샷을 표시했다. 현재 이름은 관계에서 읽고 스냅샷은 감사 필드로 별도 보존하도록 수정했다. 운영 DB 13건은 고아·타 조직·잘못된 유형 0건이며, 업체명 변경으로 인한 스냅샷 차이 1건을 확인했다.
- 외주 업체 한 곳이 여러 작업을 제공할 수 있도록 조직별 `OutsourcingServiceType`과 다대다 연결을 추가했다. 단춧구멍·재단·자수·나염/인쇄·워싱·주름·봉제·마감·기타 기본 분류를 제공하며 업체 등록/수정과 목록 검색·표시에 연결했다. 파트너와 분류가 같은 소유 조직이어야 하는 복합 FK를 DB에서 강제한다.
- 운영 DB에 반복 실행 마이그레이션을 적용했다. `Buttonhole(MS Nguyệt)`는 이름으로 확정 가능한 단춧구멍 분류만 초기 연결했고 `Nga`는 추측하지 않았다. `OrganizationHoliday.orgId`의 누락 FK도 고아 0건 확인 후 복구했다. FK 없는 다른 청구서 item/style/price/bucket ID는 원본 삭제 후에도 발행본을 보존하는 불변 스냅샷 계약이므로 FK를 추가하지 않았다.
- Prisma 생성, 백엔드/프론트엔드 빌드, 외주 기록 회귀 검사를 통과했다. 실제 배포 브라우저 조작 검증은 미실행이다.

## 2026-09-25 주문 상세 성별 열 접기

- 주문 상세 가로 보기의 남성·여성·공용 헤더에서 각 성별을 독립적으로 접고 펼칠 수 있다. 접힌 그룹은 사이즈 입력 열 대신 해당 성별 합계 한 열을 남긴다.
- 접기는 표시 상태만 바꾸며 입력 수량과 저장 payload를 변경하지 않는다. 표시되는 열 수에 맞춰 테이블 최소 너비도 줄어든다.
- 열 전환은 지원 브라우저에서 View Transition으로 280ms 동안 폭과 내용을 함께 전환하고 화살표도 회전한다. 운영체제의 동작 줄이기 설정을 존중하며 미지원 브라우저는 즉시 전환으로 안전하게 대체한다.
- 프론트엔드 빌드와 주문 목록/상세 구조 검사 5건, 번역·인코딩 검사를 통과했다. 실제 브라우저 조작은 미검증이다.

## 2026-09-25 주문·청구 생산 진행률 일치

- 주문 목록에 생산 진행률과 `생산 수량 / 주문 수량`을 표시한다. `/orders`가 화면의 주문을 한 번에 배정 진행 집계하며 주문마다 별도 API를 호출하지 않는다.
- 완료 승인된 배정은 공정 FK가 없는 과거 CT 스냅샷 때문에 기록 기반 생산량이 0으로 재구성되더라도 `completionTargetQuantity`를 고객용 생산 수량으로 사용한다. 원기록 수량은 `recordedQuantity`로 분리해 진단 근거를 잃지 않는다.
- 완료 배정은 진행률 100%로 계산하고, 미배정 수량은 주문 전체 진행률의 분모에 계속 포함한다. 주문 목록, 청구 주문 선택표, 배정 상세와 청구서 작성 원본이 같은 공통 계산을 사용한다.
- 청구 관련 32건, 주문 목록 4건과 양쪽 빌드·번역/인코딩 검사를 통과했다. 실제 운영 데이터와 브라우저 표시는 미검증이다.

## 2026-09-25 명시적 최종 정산 잠금·해제

- 최종 잠금은 최초 발행이나 수량 임계치가 아니라 발행 목록의 `최종 마감` 승인 때만 설정한다. 발행본의 모든 주문을 한 번에 잠그며 주문별 최종 인정 수량·사유, 담당자, 시각과 근거 발행본을 보존한다.
- 잠금 승인과 해제는 별도 ADMIN 제한 없이 판매자 조직의 기존 INVOICE 접근 권한으로 통일했다. 해제도 사유·담당자·시각을 이벤트로 남기고 재시도 키로 중복을 막는다.
- 최종 잠긴 주문은 주문/항목/카드/배정/직원·외주 작업기록 변경을 서버 재검사와 PostgreSQL 트리거로 막는다. 해제는 청구 잠금만 제거하고 생산 완료·급여 보호를 해제하거나 연결 데이터를 삭제하지 않는다.
- 잠긴 발행본의 개정은 잠금 근거를 새 발행본으로 옮기고 REBASE 이력을 남긴다. 잠금 근거 발행본은 먼저 명시적으로 해제하지 않으면 취소할 수 없다.
- 최종 잠금 3건, 발행 원장 8건, 초안 저장 7건과 양쪽 빌드를 통과했다. 실제 운영 PostgreSQL 적용과 브라우저 조작 검증은 미실행이다.

## 2026-09-23 청구 입금 배분·개정 원장

- 여러 주문을 포함한 발행본의 실입금은 주문별 금액을 명시적으로 입력한다. 배분 합계는 입금액을 넘을 수 없고 다른 문서의 주문을 연결할 수 없다. 배분 변경은 기존 행을 삭제하지 않고 무효 담당자·시각·사유를 남긴 뒤 새 배분 묶음을 추가한다. 결제·주문·문서 복합 FK도 교차 문서 연결을 거부한다.
- 발행 후 정정은 원본을 덮어쓰지 않는다. 원본 스냅샷에서 개정 초안을 만든 뒤 서버가 현재 주문·단가·정산 원장으로 다시 검증해 새 발행본을 만들고 원본·루트·개정 차수·사유를 연결한다. 직전 유효본은 `SUPERSEDED`로 바뀌어 채권을 중복 집계하지 않으며 원본 입금·배분 이력은 보존한다.
- 발행 목록과 상세는 개정 연결, 문서 입금, 주문별 유효 배분 및 무효 이력을 조회한다. 작성 주문 목록은 원장 기준 발행 이력 없음/있음 필터와 통화별 누적 신규채권을 표시하되 비율 청구를 수량 잔여로 추측 환산하지 않는다.
- 스키마 반복 적용, 교차 문서 FK 거부, 배분 재시도·변경 사유·초과 차단, 개정 초안 복원 및 청구 관련 회귀를 검증했다. 실제 PostgreSQL 운영 DB와 브라우저 조작 검증은 미실행이며 최종 정산 잠금 시점은 여전히 정책 확정이 필요하다.

## 2026-09-23 감사 후속 — 권한·공장 FK·표시 정합성

- 조직 수정에서 구독 필드는 시스템 관리자만 바꿀 수 있다. 조직 ADMIN의 구독 payload는 무시하지 않고 403으로 거부한다. 관리 역할은 ADMIN/OPERATOR/ACCOUNTANT이며 WORKER는 급여·수량정산·주문·배정 보드·생산완료·작업기록 mutation을 실행할 수 없다. 급여 조회도 같은 역할 경계를 적용한다.
- 조직 접근 결과의 프로세스 전역 60초 캐시를 제거하고 요청 단위로만 재사용한다. 직원 상태·역할·구독 변경은 다음 요청부터 즉시 적용된다. 시스템 관리자의 기본 조직은 실제 테넌트인 MANUFACTURER/BRAND만 선택한다.
- Employee·AttendanceEntry·WorkLog·AtTrainingBucket의 공장 참조를 `(factoryId, orgId)` 복합 FK로 전환했다. 기존 교차 조직 데이터가 있으면 migration_fix가 행 수를 밝히고 중단하며 임의 교정하지 않는다. 이력 보존을 위해 해당 공장 삭제는 Restrict다.
- 공용 통화 포맷터는 VND/KRW를 정수, USD를 소수 둘째 자리로 표시하며 언어별 locale을 사용한다. 배정·급여·작업 목록/상세의 중복 VND 포맷터를 통합했고 공용 미저장 경고의 베트남어 톤부호를 교정했다.
- QC 검토와 공정 버전 관리의 주요 사용자 문구를 한/영/베트남어로 분리했다. 직원 목록의 검색·필터와 작업 목록 헤더는 공용 sticky `PageToolbar`를 사용하며, 공장 월간 배정의 상태 색은 공용 `LABEL_PALETTE`를 사용한다.
- 직원 생성은 활성 기본 직급을 애플리케이션에서 명시적으로 조회해 `gradeId`를 저장한다. 기본 직급이 없으면 409로 중단하며 DB 트리거는 방어선으로만 남긴다.
- AT 자체 관측은 백엔드와 동일하게 양수 assignmentPlanId가 있는 행만 인정한다. 소량 경계는 서버 값을 우선하며 100 미만 경계는 실제 자체 소량 관측이 있을 때만 인정한다.
- `TENURE_YEARS`는 입사일 기준 완료된 만 근속연수의 정수값이다. 1년 미만은 0이며 기존 확정 급여 스냅샷은 재계산하지 않는다. 전역 컬러 마스터 저장은 모든 MANUFACTURER/BRAND의 배정 카드를 재구성하고 미잠금 배정 스냅샷을 갱신하는 의도된 전역 동기화 경로다.

## 2026-09-22 회차별 정산·신규 채권 중복 방지

- 주문·통화별 이전 신규채권과 단일 주문 문서의 유효 실입금을 조회해 이전 청구액·실입금액·기본 차감액을 구분한다. 초안에서 차감액을 바꾸면 사유가 필수이며 발행 시 서버가 현재 원장으로 다시 계산해 불변 스냅샷에 저장한다.
- 안내금액과 장부에 새로 추가할 채권을 분리했다. 이전 청구 5,000·입금 3,000, 새 총액 10,000에서 3,000을 차감하면 안내금액은 7,000이지만 기존 미수 2,000을 제외한 신규 채권은 5,000이다. 목록·초안/발행 PDF도 안내금액/기존 미수/신규 채권을 나눠 표시한다.
- 통화별 원장을 섞지 않는다. 여러 주문이 들어간 발행본의 문서 단위 입금은 주문별로 추측 배분하지 않고 검토 표시한다. 다음은 명시적 입금 배분 원장과 발행 후 정정/개정 이력이며 실제 PostgreSQL·브라우저 검증은 미실행이다.

## 2026-09-22 C 발행 원장 서버 기반

- `Invoice`·`InvoiceOrder`·`InvoiceLine`으로 정식 발행 스냅샷 기반을 추가했다. 한 문서의 여러 주문과 주문의 여러 문서를 허용하며 원주문/항목 FK가 삭제돼도 발행 당시 공개 ID, 표시값, 수량, 단가 구간, 단가와 금액을 보존한다. 반복 실행 가능한 생성 SQL과 시작 필수 스키마 검사도 연결했다.
- 발행 서비스는 초안 revision·판매자/구매자 범위·주문 updatedAt·전체 행 키를 Serializable Tx에서 재검사한다. 현재 관계 단가표로 주문별 스타일 수량 구간, 행별 통화 반올림, 주문별 청구 비율을 서버에서 다시 계산하며 브라우저 합계를 신뢰하지 않는다. 다른 주문의 같은 스타일 수량은 합산하지 않는다.
- 조직별 정식 번호·순번·발행 재시도 키를 고유하게 보장하고 동일 요청 재시도는 기존 발행본을 반환한다. 판매자 범위 발행 목록 API도 추가했다.
- 저장된 초안을 재검토하고 미저장 변경이 없는 경우에만 화면에서 정식 발행할 수 있다. 발행 목록은 불변 스냅샷을 다시 열어 PDF 인쇄할 수 있다. 취소는 삭제 대신 상태·사유·담당자·시각을 남기며 같은 주문의 후속 발행이 있으면 역순 취소만 허용한다.
- 주문별 명시적 발행 회차를 원장에 추가했다. 한 문서에 여러 주문이 있어도 각 주문의 기존 발행 이력에 따라 회차를 독립적으로 증가시키고 목록에 표시한다.
- `InvoicePayment`는 실제 입금을 청구액과 분리해 기록한다. 발행 통화, 수령 시점, 참고번호, 담당자를 보존하고 재시도 중복을 막는다. 잘못된 입금은 삭제 대신 무효 이력을 남기며 취소 문서에는 새 입금을 받지 않는다. 목록은 청구액·실입금·미수/초과를 따로 표시한다.
- 문서가 여러 주문을 포함할 때 실입금을 주문별로 자동 배분하지 않는다. 이전 청구액·입금액·기본 차감액·수동 차감액과 사유 이력, 개정본 연결 및 최종 잠금은 다음 단계다. 실제 운영 DB/브라우저 검증도 남아 있다.
- 발행 계산/누락 단가/스키마·라우트/반복 SQL·원본 삭제 보존 4건과 기존 초안 저장 7건, Prisma 검증·생성 및 백엔드 빌드 통과. 실제 PostgreSQL과 브라우저 검증은 미실행이다.

## 2026-09-22 주문 매출 금액 계산 상세 확인

- 주문 목록의 수주 금액과 단가 누락 표시를 클릭하면 계산 상세 모달을 연다. 서버가 목록 합계와 함께 같은 계산에서 나온 스타일별 합산 수량, 적용 수량 구간, 단가, 행 금액을 반환하므로 화면에서 합계 근거를 직접 확인할 수 있다.
- 단가는 고객 관계의 현재 판매 단가표를 사용한다. 스타일별 주문 항목 수량을 합산하고, 합산 수량 이하의 가장 큰 구간을 선택하며 첫 구간보다 작으면 첫 구간을 쓴다. 스타일별 구간 재정의가 있으면 그 버전을 쓴다. 완성된 범위가 여러 개면 CMT를 우선하고 같은 기준에서는 USD를 우선한다.
- 한 스타일이라도 단가가 없으면 부분 합계를 수주 금액으로 표시하지 않고 누락 행을 모달에 표시한다. 통화 기호를 USD로 고정하던 목록 표시도 실제 선택 통화로 고쳤다. 현재 단가표 기준 조회이며 발행 청구서 스냅샷은 아니다.
- 단가 화면이 저장 후 `SALES_PRICES` 작업공간 이벤트를 보내면 열린 주문 목록은 캐시를 우회해 서버에서 주문 금액을 다시 조회한다. 모달은 주문 객체 복사본 대신 주문 ID로 현재 목록을 참조하므로 열려 있는 동안에도 새 단가와 합계로 바뀐다. 주문 탭이 뒤에 있으면 변경 표시 후 탭을 다시 열 때 갱신한다.
- 계산 상세의 스타일 표기는 `스타일명 (스타일 코드)` 순서다. 이름이나 코드 한쪽만 있으면 존재하는 값만 표시하고 두 값이 같으면 중복 표기하지 않는다.
- 백엔드 빌드, 계산 회귀 3건, 프론트엔드 빌드, 번역 키·인코딩 검사를 통과했다. 실제 운영 DB와 브라우저 클릭 검증은 미실행이다.

## 2026-09-22 개발 환경 통합 및 청구서 B2 초안 저장

- 원격 fa6153ca까지 출퇴근/공장 도메인 관련 3개 커밋을 반영하고 이 PC의 미완성 B2 초안 저장을 마무리했다. 기존 수정은 로컬 전용 backup/pc-invoice-b2-20260922 / cc677003에 보존했다. 이전 주문 동기화 백업도 유지하며 임시 빌드 로그는 main에 포함하지 않는다.
- 청구서 목록에서 저장된 초안을 열고 수정/삭제할 수 있다. 입력 수량·비율·문서 정보만 저장하며 현재 생산/단가로 재계산하고 출력 검토를 다시 요구한다. 정식 발행·원장·선금 정산·최종 잠금은 미구현이다.
- 판매자 INVOICE 권한과 고객/주문 범위, 원주문 updatedAt/항목 FK를 같은 Serializable Tx에서 확인한다. revision으로 수정/삭제 경쟁을 막고 clientKey로 같은 생성 재시도를 보호한다. 원주문 삭제 시 입력은 보존하고 FK만 SetNull로 바뀐다. 주문이 바뀌면 저장 입력 검토와 새 초안 작성을 안내한다.
- 생성 SQL을 migration_fix.sql에도 연결하고 필수 컬럼/FK 및 Prisma client 검사를 추가했다. 저장 중 입력 보호·캐시·다른 목록 갱신·한/영/베트남어 안내와 test:invoice-draft-store를 전체 회귀에 연결했다. 검증 범위와 후속 작업은 INVOICE_LOCK_ROADMAP.md의 2026-09-22 기록을 따른다. 다음 개발은 C 발행 원장·회차별 정산이다.
- 검증: 전체 스위트 실행과 청구 API 테스트 도구 보완 후 재검사로 로컬 33/34개 스위트·362건 통과. 관계별 버킷 PostgreSQL 통합은 전용 URL 미설정이며 GitHub CI에서 확인한다. 초안 저장 7건과 양쪽 빌드·번역/인코딩은 통과했다. 실제 Prisma 초안 API 경쟁·운영 DB 반영/브라우저 사용 검증은 미실행이다.

## 2026-09-22 감사 후속 — 레거시 Line 제거 및 회귀 게이트 복구

- 권한 정책의 LINE 키·라인장 유틸·죽은 캐시 경로, 공장 UI의 라인 잔여 문구/삭제 응답 참조, 배정 DnD의 line-row/line-slot, 작업기록 오류 코드와 번역을 공장 기준으로 정리했다. 재고의 모의 불출 코드는 ISSUE_TO_PRODUCTION으로 정리했다. 일반 문자열 행/주문 행 등 Line 도메인과 무관한 용어와 과거 제거 마이그레이션/감사 도구는 유지한다. 운영 DB 변경은 하지 않았다.
- 관계별 버킷 PostgreSQL 통합 픽스처도 제거된 db.line 생성과 lineId 대신 factoryId를 사용한다. AST 기반 제거 검사는 주석/설명문을 무시하고 실제 식별자·인용 속성·Prisma 모델 접근을 검사하며 backend/src, frontend/src와 해당 통합 픽스처를 보호한다.
- 보드 공정 재검증 필드의 assignmentQuantity를 payload의 quantity로 수정했다. 수량만 바뀌고 CT/ST 스냅샷이 같은 경우도 검사한다. DB AssignmentPlan.assignmentQuantity 자체는 정상 필드이므로 유지한다.
- AT 기존 7개 계산 실패는 8/5의 확정 계약과 낡은 픽스처/기대값의 불일치였다. ST seed 누락은 가짜 관측을 생성하지 않으며, 1회 ST 비례 배분·2배 상한·미설명 시간 보존이 기준이다. 반복 재배분/seed 초기화 기대를 제거하고 수식으로 배분·시간 보존을 검증한다. eventCount는 적합 독립변수가 아니다. 실제 저곡선 거부 사례도 추가했다. atTraining.ts 계산 로직은 변경하지 않았고 21건 통과했다.
- test:regression은 test:*를 자동 수집하고 모든 스위트 실행 후 실패 목록과 비정상 종료를 반환한다. 자기 자신과 factory-domain에 이미 포함된 line-removal-transition만 제외한다. 누락된 6개 스위트를 포함해 33개를 실행한다. 낡은 소스 검사는 현재 확정 계약에 맞추고 급여 저장 경계·단가·근로기간·진행률 일부는 실제 함수 호출로 보강했다.
- GitHub Actions에 push(main)/PR 회귀·빌드·번역/인코딩 검사와 일회용 PostgreSQL 16을 추가했다. 로컬 전체 실행은 32/33 스위트 통과, assertion 354건 통과·0건 실패였다. 나머지 PostgreSQL 통합은 전용 URL 미설정으로 실행 불가이며 CI 성공을 아직 확인하지 않았다. 이후 통합 픽스처 방지 검사 1건을 추가했다. 양쪽 빌드·번역·인코딩·AT 안정 배분 별도 검사는 통과했다. 운영 DB/브라우저 검증과 보안 후순위 항목은 미완료다.
- APP_AUDIT_2026-09-08.md는 해결 항목을 제거하고 남은 검증 제한·보안·DB·UI 항목만 계속 관리한다.

## 2026-09-21 출퇴근·급여 관리 대상 통합 및 과거 기록 제외

- 출퇴근 목록의 `관리 대상 설정`과 급여 계산의 설정은 같은 직원별 설정을 사용한다. `alwaysFullAttendance`는 출퇴근 관리 제외·급여 만근 처리, `payrollExcluded`는 출퇴근과 급여 모두 제외다. 두 그룹 모두 출퇴근 목록 인원·상세 입력·파일 등록·급여의 출퇴근 누락 검사에서 제외한다. 사무/퇴사 여부만으로 제외하지 않는 기존 근로기간 규칙은 일반 직원에게 계속 적용한다.
- 공통 설정은 퇴사자를 기본 숨기고 `퇴사 직원까지 보기` 토글로 표시한다. 숨긴 직원의 기존 설정도 저장 payload에 보존한다. Trần Thanh Ly처럼 퇴사한 예외 직원의 설정을 숨겼다는 이유로 해제하지 않는다. 과거 재직일의 일반 직원 출퇴근은 퇴사 후에도 조회·등록 가능하다.
- `AttendanceEntry.managementExcluded`에 과거 기록의 관리 상태를 저장한다. 직원의 두 예외 플래그 변경 시 DB 트리거가 동일 트랜잭션에서 과거 행을 동기화하고, 새 기록/수정 시에도 같은 기준을 적용한다. 시각·메모 등 원본은 삭제하거나 만근 시각으로 바꾸지 않는다. 일반 직원으로 복귀하면 과거 행도 다시 관리 대상으로 전환한다. 관리 제외 기록은 출퇴근 API·급여 출퇴근 입력·AT 학습에서 제외하고 제외 직원을 AT의 8시간 대체 근거로 쓰지 않는다. 기존 확정 급여 스냅샷을 자동 재작성하지 않는다.
- 설정 저장은 EMPLOYEES/SALARY_SYSTEM_SETTINGS 이벤트로 열린 화면에 반영한다. 출퇴근 상세에 미저장 입력이 있으면 자동 재조회를 보류한다. 파일 등록의 `일치/미매칭`은 `등록 가능/제외`로 바꾸고 제외가 없으면 `전체 등록` 버튼을 표시한다.
- 운영 Railway DB 적용 완료: BRVN0001 정동원·BRVN0020 Trần Thanh Ly·BRVN0024 Trần Thị Uyên은 출퇴근 제외, BRVN0017 Trương Yến Ly는 출퇴근·급여 제외로 이미 설정돼 있었다. 네 직원의 저장된 출퇴근 행은 0건이며 화면의 공란은 입력 대상 행이었다. 관리 상태 컬럼/트리거를 적용하고 기존 1,306건의 시각·메모·원본 값 보존을 트랜잭션 내 검증했다. 2026-08-15 DB 기준 대상은 20명 중 제외 4명 → 관리 16명, 저장된 관리 대상 기록 15건이다.
- 검증: 출퇴근 22건(PGlite 트리거 동기화·원본 보존·롤백 및 실제 라우트/급여 준비 검사 더블 포함), 급여/근무시간 10건, 양쪽 빌드·번역·인코딩 통과. 운영 DB 적용은 완료했고 브라우저에서 배포 화면 조작 검증은 미실행이다. 적용 도구는 `backend/scripts/apply-attendance-management-exclusion.js`(dry-run 기본, `--apply` 적용, 세션 URL만 사용)다.

## 2026-09-21 출퇴근 파일 등록 — 사무·퇴사 직원 및 사번 인식

- 출퇴근 조회·입력·파일 등록은 조직/공장 범위 안의 사무 직원도 포함한다. 현재 퇴사 상태만으로 제외하지 않고 기록일이 입사일~퇴사일(양 끝 포함)에 해당하면 허용한다. 입사 전·퇴사 후 기록은 가져오기 검토 모달에서 제외 사유를 표시하며, 저장 API도 근로기간 밖 기록을 성공으로 조용히 제외하지 않고 거부한다. 퇴사일이 없는 퇴사 직원은 기간을 추측하지 않는다.
- 이벤트형(출근/퇴근 각각 한 행)과 일별 요약형(한 행에 출근+퇴근) 모두 행 전체에서 `회사코드 4자리+숫자 4자리` 사번을 먼저 찾는다. 사번의 열 제목·위치는 무관하며 `Tên`에 사번이 있으면 기기 ID보다 우선하고 이름 검증값으로 사용하지 않는다. 전체 사번은 회사코드까지 정확히 비교한다. 한 행에 서로 다른 전체 사번이 있으면 중단한다. 전체 사번이 없는 기존 파일의 사번 열 숫자 매칭과 실제 이름 대조는 유지한다.
- 일별 요약의 날짜+출근/퇴근 열을 단일 시각보다 우선하여 `Thời gian biểu`를 펀치 시각으로 쓰지 않는다. 파일 등록의 확인·미매칭·파싱/사번 충돌/저장 오류는 모달로 표시한다. 아래 9/14~9/15의 사번 열 우선·숫자만 비교·미매칭 없는 경우 window.confirm 정책을 대체한다.
- 엑셀 버퍼 기반 두 양식·퇴사 사무 직원·근로기간 경계 및 실제 저장 라우트의 테스트 더블을 포함해 출퇴근 등록 17건 검증. 운영 원본 파일 업로드·실제 브라우저/운영 DB 검증은 별도이며 운영 출퇴근 데이터는 이번 작업에서 입력하지 않았다.

## 2026-09-17 주문 저장 기반 배정 전환

- B1.5b의 수동 잠금 의존을 카드 재구성/응답·드래그·보드 저장·진행 상태·주문 잠금 UI에서 제거했다. `isOrderReadyForAssignment`로 저장된 관계형 항목의 스타일 FK와 수량을 검사하며 보드 저장은 같은 Serializable Tx에서 접근 범위와 준비 여부를 재검사한다. PT/ST 및 기존 배정 보호는 유지한다.
- 과거 `modificationLockedAt/By`는 이력으로 남기고 수정 잠금이나 최종 정산 잠금으로 사용하지 않는다. 구 잠금 API는 인증 후 410을 반환하며 기존 배정 수량/ST 동기화를 호출하지 않는다. 수동 잠금 버튼은 제거했다. 청구 원장·최종 정산 잠금은 아직 미구현이다.
- 주문 삭제는 같은 Serializable Tx에서 수정 경쟁과 모든 연결 배정을 검사한다. 배정이 있으면 409 검토를 요구하며 배정 없는 주문의 삭제와 양쪽 카드 정리는 함께 커밋/롤백한다. 열린 배정 탭은 주문 저장 이벤트를 반영하고 미저장 내용이 있으면 갱신을 보류한다.
- 관련 94건·양쪽 빌드·번역/인코딩 검사 통과. 관계 무결성/매출 단가 확대 검사 3건은 변경 전 HEAD에서도 실패함을 확인했다. 실제 Prisma/PostgreSQL 및 브라우저 검증은 미실행. 다음 개발은 `INVOICE_LOCK_ROADMAP.md`의 B2 서버 초안이며 운영 검증 제한은 별도 추적한다.

## 2026-09-17 카드 재구성의 배정 수량·분할 보존

- `reconcileAssignmentCards`는 재구성 대상 주문 전체 수량에서 실제 배정량을 빼 미배정 잔량만 갱신한다. 배정된 카드 ID/수량/내용을 유지하고 기존 미배정 분할 합계가 맞으면 분할을 유지한다. 합계가 달라지면 미배정 잔량 하나로 정리한다. 기존 배정·CT/ST·실적은 이 경로에서 변경하지 않는다.
- FK가 깨졌거나 같은 카드를 여러 배정이 참조하거나 카드/배정 수량이 다르면 409로 검토를 요구한다. 과거/0수량의 참조된 카드와 재구성 원본 범위 밖 카드도 보존한다. 계획 수량 자동 재분배/기존 손상 자동 수선은 하지 않는다.
- 재구성 함수의 같은 Tx에서 배정을 조회한다. 백엔드 빌드와 관련 55건 검사 통과. 운영 DB/브라우저 검증 미실행. **기존 잠금 필터/드래그/보드 저장/상태/UI는 아직 전환 전**이며 다음 단계는 로드맵의 일괄 전환이다. 이 변경은 그 전제인 카드 보존 수정이다.

## 2026-09-17 개발 환경 중복 작업 정리

- 이 PC의 미완성 B1.5b 작업은 로컬 전용 `backup/pc-order-sync-20260917` / `bfe422cd`에 보존했고 main에는 반영하지 않았다. 원격 `3a3dd049`까지의 16개 커밋과 주문/카드 저장 부분이 겹치며 `git merge-tree`에서 index.ts 충돌을 확인했다. main은 원격 최신으로 fast-forward했다.
- 백업의 임시 편집 스크립트/전체 변경을 최신 코드에 실행하거나 일괄 병합하지 않는다. 원격에서 완료한 저장 통합·배정 보호 및 아래 최신 선금 정책을 유지하고, 로드맵의 미완료 잠금 의존 제거부터 이어간다. 백업은 원격에 없으며 코드 완성/검증본이 아니다.
- 최신 코드의 백엔드 빌드와 관련 30건 검사 통과. 이번 작업은 환경 정리·인계 기록만이며 기능 추가나 운영 DB 변경은 없다. 상세는 `INVOICE_LOCK_ROADMAP.md`의 환경 동기화 항목을 참조한다.

## 최신 확정 — 선금 이후 주문 변경·회차별 정산 (구현 전)

- 발행본은 보존하며 후속 회차가 있으면 이전 회차 삭제 불가. 3차→2차→1차 역순만 허용한다. 원본/취소 이력 보존과 동시 발행·삭제 검증은 후속 구현한다.
- 선금 발행 후 주문 수량 변경·스타일 삭제를 허용하는 방향으로 기존 발행 즉시 주문 잠금 제안을 대체한다. 삭제된 스타일과 무관하게 이전 선금은 해당 주문의 새 총금액에서 차감한다. 생산/배정 CT/ST·실적 보호는 별개이며 현재 스타일 삭제 가드를 단순 제거하지 않는다. 최종 정산 잠금 시점은 미확정이다.
- 최종 수량과 새 단가로 재산정하고 이전 발행본은 그대로 보존한다. 이전 확정 정산액을 기본 차감값으로 불러오되 이전 청구액·실입금액·이번 차감액을 구분해 비교하고 수동 변경을 허용한다. 변경 사유/담당자/시점/기본값과 변경값을 남기며 과거 청구/입금 기록을 덮어쓰지 않는다.
- 1차 $5,000 청구·$3,000 입금에서 $3,000만 차감하는 경우, 기존 미수 $2,000을 다음 안내에 포함하더라도 신규 청구액과 중복 채권으로 계산하지 않는다. 초과 선금은 감액/환급/차기 차감 검토 대상으로 처리한다. 저장·출력 설계 및 검증 항목은 `INVOICE_LOCK_ROADMAP.md` 최신 확정 절을 따른다. 이번 변경은 정책 기록이며 기능은 미구현이다.

## 2026-09-16 주문 수정·배정 영향 판정·카드 저장 통합

- `PUT /orders/:orderId`는 기존 Serializable 잠금/수정 경쟁 재검사 뒤 `guardOrderSaveAssignments`로 조직별 배정 영향을 검사하고 주문 항목과 이전/현재 양쪽 조직 카드를 같은 Tx에서 저장한다. 실패하면 주문까지 롤백한다. 기존 CT/ST·배정 수량·실적/완료는 변경하지 않는다.
- 단일 성별은 성별 유지 및 조직별 배정 합계 이상 범위의 수량 증감을 허용한다. 배정량 침범·연결 스타일 삭제·성별 구성 변경·배정 있는 주문의 구매자/판매자 변경은 검토를 요구한다. 혼합 성별 수량 변경은 배정별 성별 근거가 없어 보수적으로 차단한다. FK/수량 누락도 추측하지 않는다. 미배정 스타일의 추가/삭제는 허용한다.
- 주문 저장의 카드 정리 시 `preserveAssignedCards`로 배정이 참조하는 카드를 삭제하지 않는다. 카드 재구성 자체의 미잠금 제외 필터는 아직 남아 있다. 다음은 `INVOICE_LOCK_ROADMAP.md`의 조회/드래그/보드 저장/상태/잠금 UI 일괄 전환이다.
- 급여 주석 조회에 Tx를 전달한다. 현재 최종 급여 잠금은 미구현으로 기존 잠금 로더는 빈 집합이며, 이번 가드는 실적/완료/급여 구분과 무관하게 모든 기존 배정을 보호한다. 화면에는 배정 검토 오류를 한/영/베트남어로 안내한다.
- 관련 81건 및 양쪽 빌드·번역 검사 통과. 저장 경로는 실제 라우트/서비스와 트랜잭션 더블로 검증하고 항목 FK는 PGlite 검사도 유지한다. 실제 Prisma/PostgreSQL 통합·운영 브라우저 검증은 미실행이다.

## 2026-09-16 청구 수량 기반·주문별 비율 정산 및 진행률

- 최신 사용자 결정은 세 가지 청구 방식 선택을 대체한다. 항상 수량×단가로 작성하고 팝업 정산 영역의 주문별 비율을 적용한다. 빈칸 100%, 0 초과 100 이하·소수 둘째 자리까지이며 전체 비율 적용도 제공한다. 수량은 비율로 나누지 않는다. 주문별 정수 소수 반올림 후 합산하고 출력본에 비율·이번 금액을 표시한다.
- 선택 표는 고객 중복 열을 제거하고 납기·생산수량/주문수량 진행률을 보여준다. 배정 버튼으로 스타일·공장·배정/생산 수량·배정 진행률을 펼친다. 판매자/고객의 현재 페이지 주문만 배정 실적을 일괄 조회하며 불명 실적은 0으로 추측하지 않는다.
- 부분 출하·선청구 검토를 위해 새 팝업에서는 전체 생산 완료 조건을 경고로 바꿨다. 수량·단가·비율 오류 및 사용자 검토 조건은 유지한다. 아직 정식 발행/저장은 없으며 표시된 차액은 이번 기준액의 참고값이다. 실제 잔여는 원장 연결 후 계산한다. 기존 별도 금액 계산 유틸은 이력용 검증만 남고 현재 화면에서 사용하지 않는다.
- B1.5b 후속으로 주문 수정 저장을 Serializable로 바꾸고 같은 트랜잭션에서 접근 범위·updatedAt·잠금을 재검사한다. 요청 처리 중 새 잠금/다른 수정이 끼어들면 409로 거부한다. 카드 갱신/영향 판정의 전체 통합은 아직 미완료다. 상세 체크와 후속 원장 요구는 `INVOICE_LOCK_ROADMAP.md`를 따른다.
- 검증: 관련 40건(청구서 29·주문 항목/잠금 경쟁 8·탭 탐색 3), 양쪽 빌드·번역 검사 통과. 실제 브라우저/운영 DB 검증은 미실행이다.

## 2026-09-16 청구서 목록·작성 메뉴 계층

- 사용자 정정: 사이드바에는 청구서 메뉴 하나만 둔다. 클릭하면 목록(`/invoices`)이 열리고 목록의 청구서 작성 버튼에서 작성 페이지(`/invoices/new`)를 새 작업 탭으로 연다. 스타일 목록→스타일 추가와 같은 구조이며 사이드바에 목록/작성 메뉴를 나누지 않는다. 기존 `/invoices/list`는 `/invoices`로 이동한다. 공통 `navigateToPath`로 탭·미저장 변경 보호 흐름을 따른다.
- 기존 고객 선택·다중 주문 초안은 작성 경로에서 유지한다. 실제 발행 목록·저장된 청구서 수정은 서버 원장 구현 전까지 미지원이다. 메뉴 명칭만으로 저장/수정 구현 완료를 뜻하지 않는다.
- 이번 메뉴 정정은 청구서 화면·작업공간 탐색 검사 7건, 번역 키 검사와 프론트 빌드 통과. 실제 브라우저 조작 검증은 미실행이다. 상세 진행은 `INVOICE_LOCK_ROADMAP.md`를 따른다.

## 2026-09-16 청구서 고객 선택 및 발행·정정 잠금 정책

- 같은 고객의 여러 주문을 체크박스로 선택해 하나의 검토 초안을 작성한다. 페이지 이동에도 선택을 유지하고 고객 변경 시 비운다. 원본은 고객/판매자 ID를 검사하고 주문별 itemId/styleId를 보존한다. 같은 스타일이 여러 주문에 있어도 초안 단가는 각 주문 내 스타일별 청구 수량으로 선택한다.
- 작성 표는 주문/스타일별 주문·생산·청구 수량·단가·합계를 보여주고 세부 행에서 청구 수량/Remark를 입력한다. 생산을 색상/사이즈별로 추정 배분하지 않는다. 출력에는 주문번호·청구 수량·단가·금액·Remark를 넣으며 주문/생산 참고 수량과 내부 수량 변경 사유는 제외한다. 문서 전체 Remarks도 유지한다. 실제 원장 다대다 연결·정식 발행은 아직 미구현이다.

- 메뉴 기본 화면은 기존 발행 청구서 목록이며 `청구서 작성` 버튼으로 작성 화면에 들어간다. 작성은 고객 → 미청구/잔여 청구 주문 → 청구 내역 순서다. 현재 목록/작성 화면 분리는 완료했으나 실제 발행 목록과 청구 잔여 필터는 원장 미구현으로 미완료이며 조회 불가/이력 확인 필요를 명시한다. 주문 목록을 발행 목록으로 대신 보여주거나 모든 주문을 미청구로 추정하지 않는다.

- `/invoices`의 작성 화면은 고객 선택 → 해당 고객 주문 선택 → 주문·생산 정보 기반 청구 내역 작성 순서다. `/invoices/customers`는 현재 판매자 공장 주문의 고객만 반환하고 `/invoices/orders`는 `buyerOrgId`를 필수로 검사한다. INVOICE 권한을 유지하며 주문 원본 API에도 선택 고객 범위를 전달한다. 아직 검토용 초안이고 정식 발행/잠금은 미구현이다.
- 사용자의 최신 결정: 해제는 판매자 제조사(공장) 조직 ADMIN 권한으로 제한하고 고객사는 참고용으로 사용한다. 금액·주소 등 청구서 정정 저장은 주문/배정 잠금을 해제하지 않는다. 잘못 만든 청구서 삭제 시 해제 방향이다. 이는 아래 9/14의 별도 청구 완료 시 잠금·정정 전 해제 설계를 대체한다.
- 여러 분할 발행본 중 일부 삭제 시 해제 조건 및 첫 부분 발행 적용은 확인 중이다. 과소/초과 마감 조건은 미정으로 유지한다. 삭제는 원본 이력 보존을 위한 취소 처리 방식을 권장하며 아직 구현하지 않았다. 상세 확정/미확정과 진행은 `INVOICE_LOCK_ROADMAP.md`를 따른다.

## 2026-09-15 출퇴근 엑셀 "일별 요약(출근/퇴근 분리 열)" 형태 지원 추가

- 출퇴근 기기가 실제로 두 가지 보고서 형태를 내보낸다: ① 기존 지원 형태("event", 사번 열 + 단일 시각 열, 펀치 한 번당 한 행), ② 신규 지원 형태("day-summary", 날짜 열 + 출근/퇴근 열이 분리, 하루당 한 행이며 출근만/퇴근만/둘 다 있을 수 있음). 두 형태 모두 `attendanceFileImport.js`가 헤더 열 구성만으로 자동 판별한다(보고서 제목 문자열에 의존하지 않음).
- 이 기기는 별도 사번 필드를 지원하지 않아 사용자가 "Tên"(이름) 열에 직접 사번(`BRVN0003` 등)을 입력한다. `day-summary` 형태에서는 사번 전용 열이 없으면 이 이름 열 값을 그대로 사번으로 취급하고, 이 경우 실제 이름 데이터가 없으므로 이름 대조 검증(오타/사번 오입력 감지)은 생략한다. 사번 전용 열과 이름 열이 둘 다 있는 기존 형태는 그대로 이름 대조 검증을 유지한다.
- 헤더 인식에서 `clockIn`/`clockOut` 키워드(`Thời Gian Vào`/`Thời Gian Ra`류)를 `timestamp`보다 먼저 확인한다. "Thời gian biểu"(근무 시간대 고정값, 실제 펀치 시각 아님) 같은 열이 먼저 매칭돼 진짜 출근/퇴근 열보다 앞서 오인되는 걸 막기 위함이다. `day-summary` 형태로 판정되면 일반 `timestamp` 열은 아예 쓰지 않으므로 이 열이 잘못 잡혀도 결과에 영향 없다.
- `day-summary` 행은 출근/퇴근 중 하나만 있어도 되고 값이 "-"/빈칸이면 그 항목은 없는 것으로 본다(추측하지 않음). 이미 원본에 출근/퇴근이 명시적으로 구분돼 있으므로, 기존 "이벤트 여러 개를 모아 최소/최대 시각으로 출근·퇴근을 추론"하는 방식 대신 원본이 말한 값을 그대로 쓴다(더 정확함). 한쪽만 있으면 기존과 동일하게 다른 한쪽을 08:00/17:00으로 자동 보완하고 메모를 남긴다. 둘 다 없는 행(그 날 근무 없음)은 건너뛴다.
- `npm run test:attendance-import`에 `scripts/attendance-day-summary-import-regression.test.mjs`(실제 xlsx 버퍼로 헤더 인식·파싱·플랜 생성까지 전 구간 검증)를 추가했다. 사용자가 보여준 실제 화면 데이터로 재현해 08:00/17:00 자동 보완, 둘 다 없는 행 스킵, "Thời gian biểu" 오인식 없음을 확인했다.
...
## 2026-09-14 조직 코드 기반 사번 채번 (BARO GARMENT -> BRVN 적용)

- `Organization.code`(영문 대문자 4자리, 조직 간 중복 불가, 시스템 관리자만 변경 가능 — 기존 `organization.routes.ts`에 이미 구현돼 있었음)가 설정된 조직은 사번을 `{코드}{4자리 순번}`(예: `BRVN0025`)로 자동 채번한다. `employeeNumber.ts`의 채번 로직은 사번 끝의 숫자만 보고 최댓값을 찾으므로, 접두어 도입 이전(`0024`)과 이후(`BRVN0025`) 사번이 섞여 있어도 정확히 이어서 채번한다. 조직 코드가 없는 조직은 기존처럼 접두어 없는 숫자만 채번한다.
- "사업체" 화면(`OrganizationDetail.jsx`)에 조직 코드를 읽기 전용으로 표시한다(사번 접두어 용도, 시스템 관리자만 변경 가능하다는 안내 포함). 코드 자체의 검증·중복 방지·수정 권한 제한은 이번에 새로 만든 게 아니라 기존 구현을 화면에 노출만 한 것이다.
- `backend/scripts/assign-organization-code.js`(dry-run 기본, `--confirm`으로 적용)로 조직에 코드를 부여하고 기존 직원 전체 사번에 같은 접두어를 일괄 반영한다. `employeeNo`는 다른 테이블의 FK가 아니라 `Employee` 표시/매칭용 문자열이라 값만 바꿔도 작업기록·출퇴근·배정·급여 이력에 영향이 없다. 운영 DB에 실제 적용 완료: BARO GARMENT(orgId=1) 코드 `BRVN`, 기존 직원 26명 `0001~0026` -> `BRVN0001~BRVN0026`.
- 출퇴근 엑셀 매칭(`attendanceFileImport.js`)도 접두어 형태와 무관하게 사번 끝의 숫자만 추출해 비교하도록 함께 고쳐서, 접두어가 붙은 사번도 출퇴근 기기의 순수 숫자 ID와 정상 매칭된다(아래 "출퇴근 Excel 매칭을 사번 단일 방식으로 전환" 항목에서 이 매칭 로직을 이름-매칭 제거까지 더 확장했다).

## 2026-09-14 출퇴근 Excel 매칭을 사번 단일 방식으로 전환 (정책 변경, §"2026-09-09 출퇴근 Excel 사번 매칭" 대체)

- 출퇴근 파일 업로드의 직원 매칭은 이제 엑셀에서 추출한 사번(`Employee.employeeNo`)만을 유일한 근거로 쓴다. 사번 열이 비었거나 사번으로 끝나는 숫자가 없는 행은 이름이 함께 적혀 있어도 더 이상 이름으로 대신 매칭하지 않는다(`missing_employee_code`로 미매칭 처리). 기존에 있던 "사번 없는 행만 전체 이름/성+이름 축약 매칭"이라는 §2026-09-09 정책은 이 항목으로 대체됐다.
- 사번이 있고 그 사번의 직원을 찾은 경우에만 이름을 참고용으로 검증한다(오타·다른 직원 사번 오입력을 잡기 위함, 성조 제거·Đ 변환·중간 이름 축약 호환 유지) — 이름 불일치·사번 중복은 여전히 `ATTENDANCE_EMPLOYEE_CONFLICT`로 전체 파일 처리를 중단한다.
- 2026-09-21부터 헤더는 시각/날짜·출근·퇴근의 배치를 구분하고, 회사코드+4자리 사번은 열 제목과 무관하게 행 전체에서 찾는다. 실제 이름만으로는 매칭하지 않는다.
- `buildAttendanceImportPlan`이 `unmatchedDetails`(사번·이름·시각·사유)를 반환하고 저장 전 확인 모달을 표시한다. 미매칭이 있으면 상세 표에서 확인 후 "일치한 항목만 가져오기"를 눌러야 저장한다. 2026-09-21부터 미매칭이 없어도 같은 확인 모달을 사용하며 오류도 모달로 표시한다.
- 회사코드+4자리 전체 사번은 2026-09-21부터 접두어까지 정확히 비교한다. 전체 사번이 없는 기존 숫자 사번 파일에만 숫자 정규화 매칭을 유지한다.
- `npm run test:attendance-import`에 코드-only 매칭(이름만으로는 매칭되지 않음, 사번 있으면 이름 변형은 호환 검증에만 사용)과 `unmatchedDetails` 구조를 검증하는 테스트를 추가/교체했다. 실제 브라우저로 다이얼로그 동작을 확인하지는 않았다.

## 2026-09-14 청구서 B1.5a — 주문 항목 ID 보존

- `PUT /orders/:orderId`는 모든 WorkOrderItem을 삭제/재생성하지 않는다. `planOrderItemWrites`로 현재 주문의 공개 항목 ID를 비교해 기존 PK를 유지하고 추가/삭제된 행만 처리한다. 레거시 빈 itemId는 응답과 같은 PK 문자열을 사용한다. 중복 식별자는 409로 거부하며 스타일/색상 이름으로 연결을 추측하지 않는다.
- 주문과 항목 갱신은 동일 트랜잭션이다. `test:order-item-identity`는 수정/추가/삭제 계획, 레거시/다른 주문/중복 식별자 및 별도 PGlite 최소 스키마의 FK 보존·실패 롤백을 검증한다. 실제 Prisma API와 운영 DB 검증은 미실행이다.
- 백엔드 빌드와 관련 24건 검사 통과. 이번 단계는 기존 잠금과 카드 갱신 트리거를 변경하지 않았다. 다음은 `INVOICE_LOCK_ROADMAP.md`의 B1.5b이며 B2 서버 초안보다 먼저 진행한다. 청구 원장 연결 후 행 삭제/속성 변경 보호는 별도 구현해야 한다. 수량·금액 일치로 자동 청구 완료하지 않는다.

## 2026-09-14 주문 저장 기반 배정 카드 갱신 — 전환 방향 확정, 코드 미변경

- 배정 카드 갱신은 앞으로 유효한 주문 생성/수정 저장 성공에 연결한다. 주문 잠금/재잠금을 카드 갱신의 필수 트리거로 사용하지 않으며 청구 완료 잠금과 분리한다. 이번에는 코드 조사와 문서만 수행했으므로 현재 앱은 여전히 기존 잠금 기반이다.
- 현 구현에서 수정 저장은 카드/배정을 갱신하지 않는다. 신규 저장은 재구성을 호출하지만 조회의 `modificationLockedAt not null` 필터로 미잠금 주문이 제외된다. AssignBoard 드래그와 서버 보드 저장도 미잠금 주문을 차단하고, 주문 진행 상태도 잠금 상태에 의존한다. 함수 호출 한 곳만 옮기지 말고 이 경로들을 함께 전환한다.
- `syncAssignmentPlansForOrderLock`는 카드 표시 갱신 이상의 작업으로 단일 배정 수량/ST를 재계산한다. 다중 배정 합계 불일치, 완료/급여 잠금 수량 변경은 거부하며 동일 총수량에서는 조기 반환하므로 성별 구성 변경도 별도 검증해야 한다. 저장 트리거 이전 시 기존 배정을 임의 재배분하거나 확정 CT/ST·실적을 덮어쓰지 않는다.
- 수정 저장의 WorkOrderItem 전체 삭제/재생성은 항목 PK를 바꾼다. 청구 행의 원주문 항목 FK를 도입하기 전에 안정적인 항목 ID 기반 저장으로 전환해야 한다. 자세한 조사와 검증 기준은 INVOICE_LOCK_ROADMAP.md의 B1.5에 기록하며 B2 서버 청구서 저장보다 먼저 진행한다.

## 2026-09-14 청구서 B1 — 별도 메뉴 및 3종 청구 초안

- 영업 관리의 `/invoices`에서 청구 대상 주문을 검색해 초안을 작성한다. 주문 상세의 초안 버튼은 제거했다. 기존 주문 잠금은 배정 준비 분리 전이므로 그대로이며, 청구 메뉴는 주문/배정/급여를 변경하지 않는다.
- INVOICE는 ORDER와 독립인 메뉴/API 권한이다. `ROLE_ACCESS_POLICY` 버전 11에서 제조사 ADMIN/ACCOUNTANT를 기본 허용하며 과거 버전도 같은 신규 기본값으로 전환한다. 버전 11 정책에 저장한 해제는 유지한다. 판매자 조직 주문만 조회하며 구 invoice-source API도 INVOICE 검사를 따른다.
- 사용자 확정: 청구 방식은 수량, 전체 금액의 비율, 합의 금액 세 가지다. 비율·합의 금액은 수량을 소비하지 않으며 생산 완료나 단가표가 없어도 합의된 금액으로 검토 초안을 만들 수 있다. 금액 방식의 양식은 PAYMENT REQUEST이고 수량 표를 만들지 않는다.
- 비율 초안은 합의된 전체 금액을 입력하며 현재 단가표의 전체 주문 참고값은 명시적으로 선택할 때만 사용한다. 비율 0 초과 100 이하(소수 2자리), 양수 금액 및 통화 자릿수를 검사하고 정수 소수 연산으로 반올림한다. 수량 방식의 기존 생산 완료 검사는 이번 단계에서 변경하지 않았다.
- 수량/금액 일치만으로 자동 청구 완료하지 않는다. 잠금 트리거는 상단 2026-09-16의 최신 발행·정정 정책을 따른다. 비율/합의 금액과 수량 청구를 혼용할 때 선청구 금액의 정산 배분을 별도로 구현해야 한다. 청구와 실제 입금은 별개다.
- 현재는 검토용 초안이며 서버 저장·회차별 누적 발행·선금 차감·정식 발행·완전 잠금은 미구현이다. 창 닫기/이동 시 입력 소실을 안내한다. 개발 상태와 후속 작업은 INVOICE_LOCK_ROADMAP.md에서 계속 갱신한다.
- `test:invoice-draft`에 금액 3회 분할, 100% 자동 완료 없음, 정수 소수/통화 검증, PDF 수량 미소비, 독립 권한/판매자 범위/API 페이지 조회 검사를 포함했다. B1 관련 35건·양쪽 빌드 통과, 운영 화면·배포 확인은 미실행이다.

## 2026-09-14 별도 청구서 메뉴·분할 청구 및 청구 완료 잠금

아래는 당시 결정 이력이다. 잠금 트리거·정정·해제 조직 범위는 상단 2026-09-16 정책으로 대체됐으며 현재 구현으로 해석하지 않는다.

- 청구서는 별도 메뉴에서 작성·관리하며 한 주문의 부분/분할 청구는 필수다. 주문 화면에서 직접 완전 잠금하지 않는다. 청구서 한 장 발행만으로 잠그지 않고, 누적 청구 수량을 기본 조건으로 해당 주문을 **청구 완료 처리할 때** 주문·관련 배정을 완전 잠금한다. 이 요구가 9월 13일의 수동 완전 잠금/발행 즉시 잠금 제안을 대체한다. 현재 코드는 아직 변경되지 않았다.
- 과소·초과 청구가 가능하므로 `누적 청구량 >= 주문량`만으로 완료 처리하지 않는다. 정확히 맞아도 담당자의 명시적인 청구 완료 처리가 필요하다. 분할 청구 중 잔량과 최종 과소/초과 마감을 구별하며 차이 마감의 사유·승인·허용 기준은 미확정이다. 임의 오차율을 만들지 않는다. 청구 완료는 입금 완료와 별도다.
- 잠금 해제 자격은 메뉴 접근 정책과 분리해 조직 관리자(`Employee.orgRole = ADMIN`)로 제한하는 방향을 유지한다. 현장 직무/표시명이나 메뉴 권한만으로 해제를 허용하지 않는다. 공유 주문의 해제 가능 조직 범위는 아직 미확정이다.
- 현 잠금은 주문 수정·삭제 차단과 배정 카드/수량 동기화를 겸한다. 배정 준비는 청구 완료 잠금과 독립적으로 전환하며 구 잠금 데이터를 일괄 새 완전 잠금으로 해석하지 않는다. 발행본은 부분 청구 중에도 보존하며 주문 수정으로 덮어쓰지 않는다.
- 사용자가 여러 개발 환경에서 이어서 작업할 수 있도록 별도 로드맵을 명시적으로 요청했다. 이 기능의 단계·검증·인계 기록은 루트 `INVOICE_LOCK_ROADMAP.md`에서 관리하며 `todo.md`에는 진입 링크만 둔다. 이 문서에서 미확정 제안과 완료 코드/운영 검증을 구별한다.

## 2026-09-13 청구서 1단계 — 주문별 검토용 PDF 초안

아래는 A단계 당시 이력이다. 2026-09-14 B1에서 별도 메뉴·세 가지 작성 방식으로 전환했다. 당시 주문 상세 진입점은 제거됐으며 전체 생산 완료 조건은 수량 초안에만 남아 있다.

- 판매자 조직의 주문 상세 `청구서 초안`에서 저장된 주문·스타일별 생산 진행·현재 활성 고객 단가표를 읽는다. `GET /orders/:orderId/invoice-source`는 판매자 조직과 ORDER 권한을 확인한다. 조회는 저장·잠금 변경을 하지 않는다.
- 주문 색상·성별·사이즈 수량은 초안 시작값이며 스타일별 생산 수량을 세부 행에 임의 배분하지 않는다. 수량 수정은 초안에만 적용하고 주문과 달라진 행에는 사유가 필요하다. 완료/생산 근거가 확인되지 않거나 단가가 없으면 PDF 초안 출력을 막는다.
- 초안 단가는 스타일별 청구 수량 합계의 활성 수량 버킷으로 선택한다. CMT/FP·통화를 구분하고 스타일별 버킷 예외를 우선한다. 빈 가격을 인접 버킷/과거 버전/0원으로 채우지 않는다. 통화별 금액은 정수 소수 연산 및 행별 반올림 후 합산한다.
- CMT는 Manufacturing Service Invoice, FP는 Commercial Invoice 영문 양식이며 브라우저 인쇄의 PDF 저장을 사용한다. 모든 출력은 `DRAFT / NOT ISSUED`이고 고객 발송용 정식 발행본이 아니다. 초안은 창을 닫으면 폐기되며 서버 보관 기능은 아직 없다.
- 당시 정식 발행·스냅샷·최종 잠금은 미구현이었다. 현재는 2026-09-25 정책에 따라 명시적 최종 마감과 INVOICE 접근 사용자의 해제로 구현됐으며 이 과거 ADMIN 전용 제안은 폐기됐다.
- 양식 참고: https://www.trade.gov/commercial-invoice 및 https://www.trade.gov/pro-forma-invoice . 회귀 검증은 `test:invoice-draft`로 실행하며 운영 주문/API/UI의 실제 사용 검증은 별도다.

## 2026-09-11 반복 생산 검증 기반 AT 근거 점수

- 독립 배정 3건 이상부터 배정 하나의 전체 관측을 제외하고 나머지로 제약 회귀를 다시 적합해 제외 배정의 장당 시간을 예측한다. 저장 a/b·공통 사전 예측은 검증 학습에 사용하지 않는다.
- 배정별 상대 오차(최대 1)를 동일 가중 평균한다. 기존 축적 점수 25%와 오차 기반 품질 점수 75%를 합치고, 3/4/5건의 상한을 각각 70/80/90점으로 둔다(이후 최대95). 출퇴근 대체 감점과 개별 수량 외삽 감점은 유지하며 스타일 전체에는 외삽 감점이 없다.
- 검증은 배분 노동시간 관측에 대한 자체 곡선 일관성이다. 독립 실측 시간이나 공통 혼합 모델 전체의 운영 정확률이 아니다. 가중치·오차 환산은 초기 정책이며 실제 정확도로 보정되지 않았다. 관측 배열별 WeakMap 캐시를 사용하며 관측 갱신은 새 배열로 수행한다.
- 네 배정 일관성/큰 편차, 중복 배정으로 검증 건수 부풀리기 금지, 저장 예측값 유출 방지, 수량과 무관한 전체 점수를 회귀 검증한다. 운영 AM01160의 실제 점수와 배포 확인은 미완료다.

## 2026-09-11 스타일 전체 AT 신뢰도 분리

- 스타일 목록·공정 화면 점수는 특정 q 또는 timeRefQuantity와 무관한 전체 실적 근거 점수다. 공정 PT 양수값(없으면 동일 가중 1)으로 집계하며 예측 AT로 가중하지 않는다. 미관측 공정은 0점으로 포함한다.
- 독립 배정 축적 최대 60점(건당 5), 기본 10점, 수량 다양성 최대 20점, 관측 범위 최대 5점에서 반복 편차·출퇴근 대체 감점을 적용한다. 동일 수량 반복도 성장하지만 수량 다양성 점수를 만들지는 않는다. 잠정 표시나 공통모델 혼합 여부가 자체 근거 점수를 25점으로 제한하지 않는다.
- 개별 수량 점수만 관측 범위 밖 거리의 log2에 비례한 감점(두 배 거리당 15점, 최대 60점)을 적용한다. 스타일 전체 점수에는 외삽 감점을 적용하지 않는다. 공통 사전만 있고 자체 관측이 없으면 자체 실적 근거는 0점이다.
- 이 수치는 검증된 예측 정확률이 아닌 규칙 기반 근거 점수다. UI 설명에 이를 명시한다. 가중치·거리 감점의 현장 오차 교정은 미완료이며 공통 학습 데이터를 자체 관측 개수로 더하지 않는다. AT 값·CT/ST·급여는 변경하지 않는다.

## 2026-09-11 AT 극소량 안전장치 전 경로 적용

- 자체 실적만 사용하는 하향 외삽도 최소 관측점의 접선으로 완화한다. 기존 응답에 공통 모델 경계 필드가 없어도 프론트에서 안전장치를 적용한다.
- 100장 미만 생산이 드문 운영 기준을 반영해 공통 모델의 참고 경계는 최소 100장이다. 자체 실적에 더 작은 수량이 있으면 해당 수량까지 경계를 낮춰 관측 범위의 값을 보존한다. 공통 학습 타 스타일의 소량 하나가 대상 스타일의 안전장치를 해제하지 않는다.
- 100장 미만 자체 실적이 없는 경우 1~99장 예측은 100장 장당 AT의 두 배 미만으로 완만하게 제한된다(양의 a, 비음수 b 기준). 이는 초기 보수적 예측 정책으로 실측 데드타임이나 검증된 견적 상한이 아니다. 신규·기존 응답·자체 실적 경로와 실제 소량 관측 보존을 회귀 검증한다.

## 2026-09-11 극소량 AT 외삽 완화

- 공통 준비시간 적합에는 수량비 2 이상인 관측 쌍만 사용한다. 가까운 대량 두 점에서 불안정한 절편을 추출하지 않는다. 이 기준은 초기 보수적 정책이며 운영 정확도로 보정된 값은 아니다.
- 경계 m은 참고 스타일별 최소 관측수량의 중앙값을 스타일 동일 가중으로 집계하고, 자체 최소 관측이 더 작으면 그 수량으로 낮춘다. q>=m은 a+b/q, q<m은 경계의 접선 a+(b/m)*(2-q/m)으로 연장한다. 실적 범위의 곡선을 보존하면서 극소량에서 발산하지 않고 장당 시간 비증가·총시간 증가·경계 연속성을 유지한다.
- 완화 구간은 항상 잠정 표시한다. 이는 실측 고정 준비시간의 물리 법칙이 아닌 자료 부족 구간의 보수적 예측 정책이다. 소량 실적이 생기면 경계가 내려가며 실적 지점을 완화하지 않는다. 관측·확정 CT/ST·급여를 수정하지 않는다.
- 회귀 검사에 q=1/10, 경계, 총시간, 3300장 앵커 및 가까운 대량 관측 제외를 포함한다. 실제 청구 정확도·소량 현장 실측 검증은 아직 필요하다.

## 2026-09-11 공통·카테고리 AT 사전 추정

- `atSharedPrior.ts`는 같은 조직·생산 단계·성별 적용 범위의 타 스타일 실적을 PT로 정규화해 공통 준비시간 곡선을 구성한다. 카테고리는 `Style.collection`의 저장값으로 비교한다. 공장별 모델이 아닌 조직·생산 단계 공통 모델이며 조직을 넘는 공유는 하지 않는다.
- 학습 공정은 서로 다른 배정과 수량이 각각 2개 이상이어야 한다. 중앙값 기울기/절편, 적합 오차와 PT 대비 속도 검사를 통과한 공정만 참고한다. 스타일별 중앙값을 다시 집계해 공정 수 많은 스타일의 과대 가중을 방지하며 대상 스타일 전체는 공통 학습에서 제외한다.
- 카테고리 가중은 스타일 수/(스타일 수+3), 자체 곡선 가중은 배정 수/(배정 수+5)인 초기 축소 추정이다. 이는 통계적으로 보정된 신뢰확률이 아니다. 유효한 공통 근거가 없고 자체 곡선이 있으면 자체 곡선을 그대로 사용한다. 단일 수량은 공통 절편을 참고하되 자체 총 실적에 맞게 기울기를 보정하고, 양의 기울기가 성립하지 않으면 준비시간 0의 잠정 평균으로 돌아간다.
- 기존 무실적 빈칸 계약을 대체한다. PT 우선, 없으면 활성 ST 기준의 사전 추정도 표시한다. 공통 근거가 없으면 PT/ST 평평한 잠정값이며 근거 없이 준비시간을 생성하지 않는다. 공통 모델의 학습 seed는 PT만 사용해 AT로 수정된 ST의 재학습을 피한다. 시간 형태는 a+b/q, a>0,b>=0이다.
- `atSharedPrediction`은 조회 시 계산하는 읽기 전용 응답이다. 저장 정규화에서 제거하고 작업 관측·확정 CT/ST·급여 JSON을 변경하지 않는다. 스타일 변경 감지에서도 제외한다. 화면 AT 툴팁에 카테고리/조직 공통/자체 실적 보정/PT·ST 잠정 출처를 표시한다.
- `test:at-shared-prior`는 조직·단계 격리, 카테고리·공통 fallback, 3,300장 앵커, 자체 가중 및 합성 스타일 홀드아웃을 검증한다. `audit-at-shared-prior.mjs`는 JSON 관측을 읽어 스타일 전체를 제외한 예측 오차와 PT baseline을 비교한다. 운영 데이터 정확도 및 대용량 조회 성능 검증은 아직 미완료다. 절편은 실제 측정 데드타임이 아닌 추정 준비시간이다.

## 2026-09-11 AT 소량 외삽 제한 제거

- 단일 수량의 동일 ST 버킷 제한과 최소 관측수량의 1/2 제한을 제거했다. 단일 수량은 동일 장당 잠정값, 복수 수량의 소량 외삽은 기존 제약 적합 관측점의 a+b/q 곡선을 연장한다. 범위 밖 값은 외삽/잠정 표시를 유지한다.
- 대량 회귀는 최대 관측점 장당 시간보다 높아지지 않게 제한한다. 회귀가 기존 크기 검사를 실패하면 관측점 값으로 되돌아가 역전시키지 않고 빈칸으로 둔다. 무관측·비정상 계산은 계속 제외하며 CT/ST 및 관측은 수정하지 않는다.


## 2026-09-11 단일 수량 실적의 대량 AT 잠정 추정

- 유효 관측 수량이 한 종류뿐이어도 관측보다 큰 수량의 AT를 해당 수량의 누적 배분시간/누적 생산량으로 계산한다. q=1,000 및 q=3,000에서 ST 버킷이 없어도 표시한다. 준비시간이나 대량 생산 할인율은 임의로 만들지 않고 `provisional-extrapolated` / `PROVISIONAL_CONSTANT`로 구분한다.
- 복수 수량의 대량 회귀는 a>0, b>=0 및 기존 크기 검사를 통과할 때 사용한다. 실패하면 빈칸 대신 최대 관측점의 기존 제약 적합 장당 시간을 잠정값으로 사용한다. 하한에 맞춰 시간을 강제로 끌어올리지 않는다. 무관측 공정은 여전히 추정하지 않으며 소량/보간 경로는 유지한다. 확정 CT/ST/급여 JSON 및 관측 데이터는 수정하지 않는다.
- 반복 실적은 장당 잠정값 갱신에 기여하나 수량 다양성을 만들어내지 않는다. 기존 신뢰도 기준을 유지하며 새로 표시되는 외삽값 때문에 스타일 신뢰 등급이 더 보수적으로 나올 수 있다.
- `node scripts/audit-at-extrapolation.mjs observations.json`은 `{processes:[{id,atV2Observations,stBuckets}]}`를 읽어 최대 수량 관측 전체를 제외하고 작은 수량으로 재적합한 예측과 단순 평균을 비교한다. 저장된 a/b는 재사용하지 않는다. 배분 노동시간을 검증 대상으로 하므로 독립 실측 정확도 검증과는 다르다. 운영 데이터 평가는 아직 미실행이며 공정 간 부분 풀링은 미도입이다.

## 2026-09-10 AT 대량 방향 추정치를 관측 최대치의 4배 상한 없이 회귀식으로 계산 (정책 변경)

- **문제 제기**: 사용자가 8월 작업기록·출퇴근을 입력하고 AT 갱신(초기화 후 재실행 포함)까지 했는데도 AT 신뢰도가 거의 안 올라간다고 지적했다. 또한 "TKD WINDBREAKER JACKET"처럼 매일 입력해도, 고객이 실제 발주하는 수량(예: 3,000장)을 미리 알 수 없어 배정 자체가 항상 작게 나가는 스타일은 q=1,000 참고값이 아예 안 뜨는 문제도 지적했다. "데이터가 쌓이면 안 만들어본 수량에 대해서도 추정 신뢰도가 계속 개선됐으면 좋겠다"는 것이 핵심 요구였다.
- **원인 조사 결과 (코드로 확인)**:
  1. **신뢰도 등급은 "일 수"가 아니라 "서로 다른 배정(주문) 개수"로 결정된다.** 관측 1건의 단위는 `공정ID:배정ID`이며(`backend/src/index.ts`), 같은 배정에 작업기록을 아무리 더 넣어도 기존 관측 1건이 갱신될 뿐 개수가 늘지 않는다. USABLE(75%+)은 8개, TRUSTED(85%+)는 16개, VERIFIED(95%+)는 24개 이상을 요구한다(`frontend/src/utils/processTime.js`). 같은 주문을 계속 입력한 경우 신뢰도가 거의 안 오르는 것은 설계대로 동작한 것이었다 — 오르려면 서로 다른 신규 배정(주문)이 쌓여야 한다.
  2. **대량 방향 참고값이 "학습된 회귀식"이 아니라 "가장 큰 관측치의 장당 시간을 그대로 복사"하는 방식이었다.** `resolveProcessAtV2PerPieceSeconds`가 관측 최대 수량을 벗어나면 `atParams.a/b`(전체 관측을 강건회귀로 학습한 계수)를 전혀 쓰지 않고, 가장 큰 관측점의 장당 시간을 그대로 굳혀서 보여줬다. 그리고 그 최대 관측치의 **4배**를 넘으면 무조건 `null`이었다. 그래서 실제 배정 최대 수량이 250장 미만인 스타일은 q=1,000이 영원히 안 뜨고, 4배 이내라도 데이터를 더 넣어봤자 "복사값" 자체가 안 바뀌어 개선되는 느낌이 없었다.
- **확정된 수정 (사용자 확인 후 진행)**: 대량 방향(관측 최대치보다 큰 수량)만 다음과 같이 바꿨다.
  - `atParams.a × q + atParams.b`(공정 전체 관측을 반영한 실제 학습 회귀식)로 직접 계산한다. 관측이 더 쌓여 `a/b`가 갱신되면, 한 번도 만들어본 적 없는 q=1,000 같은 값도 그 갱신을 그대로 반영해 같이 바뀐다.
  - **4배 상한을 없앴다.** 관측 최대치를 아무리 벗어나도 더 이상 `null`을 반환하지 않고 항상 값을 계산해서 보여준다.
  - 신뢰도 구분은 기존 톤 체계(`extrapolated`/`provisional-extrapolated`)를 그대로 재사용한다. 별도의 신규 UI 표시는 만들지 않았다 — 화면에는 값과 함께 기존의 낮은 신뢰도 스타일(회색/이탤릭 등)이 그대로 적용된다.
- **의도적으로 그대로 둔 것**: 소량 방향(관측 최소치보다 작은 수량)은 전혀 손대지 않았다. 셋업시간 왜곡 위험 때문에 여전히 "가장 작은 관측점의 장당 시간을 그대로 복사"하고, 최소 관측치의 1/2 미만이면 여전히 `null`이다. 관측 범위 **안**(두 관측점 사이)의 보간 방식도 안 바꿨다 — 그건 국소 두 점 사이의 선형 보간이며 이번 변경과 무관한 별도 로직이다.
- **신뢰도 등급(관측 개수 요구치) 자체는 이번에 바꾸지 않았다.** "같은 배정 반복 입력으로는 신뢰도가 잘 안 오른다"는 지적은 설계 의도와 일치하는 것으로 확인했고, 별도 요청이 없어 이 문서의 §"AT 모델"/"신뢰도 상태" 기준을 그대로 유지한다.
- `scripts/at-reliability-regression.test.mjs`의 기존 "4배 넘으면 null" 가정 테스트 3개를 새 동작에 맞게 다시 썼고, 관측이 갱신되면 q=1,000처럼 한 번도 만든 적 없는 수량의 추정치도 실제로 바뀐다는 걸 확인하는 전용 테스트를 추가했다.
...
- **원인**: 앞서 제거한 "저장 전 스타일 전체 배정 검사"와 별개로, 배정 하나하나를 재계산하는 루프 안에도 `if (!hasValidAssignmentProcessRefs(plan)) throw ...`라는 두 번째 사전 검사가 남아 있었다. 이 검사는 "지금 저장된 버전과 맞는지"가 아니라 "그 배정이 지금 들고 있는 CT/ST 스냅샷 자체가 내부적으로 앞뒤가 맞는지"(공정 ID가 CT/ST 양쪽에 똑같이 있는지, 버전 ID가 서로 일치하는지)를 확인하는데, 정확히 `공정 연결 확인` 경고가 뜨는 배정이 이 조건에도 걸려 재계산을 시작하기도 전에 막혔다.
- **분석**: 실제 재계산 함수(`buildEditableAssignmentCtSnapshotFromLiveStyle`)는 이 배정의 기존 스냅샷을 통째로 신뢰해서 베이스로 삼지 않는다. 확정 버전의 살아있는 공정 목록을 기준으로 순회하면서, 기존 스냅샷에 같은 공정 ID가 "우연히" 있으면 그 수기 조정값만 참고하고, 없으면 그냥 새로 계산한다. 즉 기존 스냅샷이 내부적으로 깨져 있어도 결과가 오염되지 않도록 이미 안전하게 설계되어 있어서, 이 사전 검사는 실제로 막아야 할 위험이 없는데 정확히 고쳐야 할 대상만 막고 있었다.
- **수정**: 이 두 번째 사전 검사도 제거했다. 재계산 결과 자체의 검증(`refreshed.readiness.ready` 확인, ST 누락 검사 등)은 그대로 유지한다 — 검증 시점을 "재계산 전 기존 데이터"에서 "재계산 후 결과"로 옮긴 것이며, 검증 자체를 없앤 게 아니다.
- `npm run test:assignment-snapshot-integrity`에 이 두 번째 사전 검사가 다시 들어가지 않는지 확인하는 회귀 테스트를 추가했다.

## 2026-09-10 버전이 1개뿐인 스타일에서 공정 버전 관리 저장 버튼이 항상 비활성 상태였던 문제 수정

- **증상**: 바로 위 항목대로 백엔드에 자동 복구 로직을 만들었지만, 실제로 확인 필요 배정이 있는 스타일(예: 버전이 Ver.1 하나뿐인 스타일)에서 공정 버전 관리 창을 열어도 `저장` 버튼이 계속 회색 비활성 상태라 눌러볼 수조차 없었다. 실제 데이터로 재현: 배정 5개가 전부 `versionId: 12`(Ver.1)로 동일한데 그중 2개만 `needsSnapshotRefresh: true`였다.
- **원인**: 이 창의 `boundaries`(현재 편집 중인 적용 구간)와 `savedBoundaries`(저장 버튼 활성화 판단 기준값)는 둘 다 창을 열 때 "각 배정에 지금 저장돼 있는 versionId"에서 동일한 방식으로 계산되어, 창을 여는 순간 둘이 항상 같다. 저장 버튼은 `boundaries !== savedBoundaries`(즉 사용자가 실제로 버전 칩을 다른 배정으로 드래그해 구간을 바꿨는지)로만 활성화됐다. 그런데 버전이 하나뿐이면 그 버전의 구간은 이미 가장 오래된 배정에 고정돼 있어 드래그로 바꿀 수 있는 여지 자체가 없다 — 즉 "고칠 게 있어도 저장을 누를 방법이 아예 없는" 상태였다.
- **수정**: 저장 버튼 활성화 조건에 "구간을 바꿨는지"뿐 아니라 "확인 필요로 표시된 배정이 하나라도 있는지"(`needsSnapshotRefresh`)도 추가했다(`frontend/src/pages/App/style/styleDetail/ProcessVersionManager.jsx`). 이제 구간을 안 바꿔도, 확인 필요 배정이 있으면 저장을 눌러 같은 버전 기준으로 재계산을 실행할 수 있다. 안내 문구도 "자동으로 다시 계산하지 않는다"에서 "저장을 누르면 이 버전 기준으로 다시 계산한다"로 정정했다(완료 확정·급여 잠금 배정은 여전히 보호).
- `npm run test:assignment-snapshot-integrity`에 단일 버전 스타일에서도 저장 버튼이 활성화될 수 있는지 확인하는 회귀 테스트를 추가했다.

## 2026-09-10 공정 버전 적용 구간 저장 시 배정 CT/ST 자동 동기화 (정책 변경)

- **배경**: 스타일 공정을 나중에 재구성(예: 성별 무관 공정 하나를 남성 전용/여성 전용으로 분리)하면, 그 이전에 만든 배정은 옛 공정 구성 그대로 CT/ST가 저장돼 있다. 공정 버전 관리에서 새 버전의 적용 시작 배정을 지정해 그 배정까지 새 버전이 덮도록 저장해도, 배정 자체의 CT/ST는 예전 그대로 남아 "공정 연결 확인 필요"로 계속 표시되는 사례가 있었다.
- **원인 1 (막힘)**: `PUT /styles/:styleId/process-version-boundaries`가 저장 시작 시 `assertAssignmentProcessRefs`로 그 스타일의 **기존(저장 전) 배정 전체**가 이미 정상인지부터 검사하고 있었다. 이미 깨져 있는 배정이 있으면(정확히 이 기능으로 고치려는 대상) 저장 자체가 409로 거부되어, "깨진 배정을 고치는 유일한 화면"이 "깨진 배정이 있으면 아무것도 못 하게" 막는 모순이 있었다.
- **원인 2 (자동 반영 안 됨)**: 실제로 이 엔드포인트는 저장된 적용 구간 기준으로 각 배정이 속한 버전이 달라지면 그 배정의 CT/ST를 라이브 스타일 공정 기준으로 재생성하는 로직을 이미 갖고 있었다(`buildEditableAssignmentCtSnapshotFromLiveStyle`, 성별 전용 공정의 실제 주문 성별 수량까지 반영). 다만 원인 1의 사전 검사에 막혀 실행 자체가 안 됐을 뿐이다.
- **수정**: 저장 전 전체 배정을 검사하던 사전 검사(원인 1)를 제거했다. 대신 배정별로 실제로 재생성한 결과(라이브 공정 기준으로 새로 만든 CT/ST)만 검증하며, 이미 저장 시점에 있던 개별 검증(`hasValidAssignmentProcessRefs`, ST 누락 검사, 성별 수량 모호성 검사 등)은 그대로 유지한다. 이제 적용 구간을 저장하면 그 결과로 버전이 바뀌는 배정의 CT/ST가 자동으로 재생성된다.
- **보호 대상은 그대로 유지**: `isCompleted=true`(완료 확정)이거나 급여 잠금(`isPayrollLocked`, 완료월 기준)인 배정은 버전이 바뀌어도 자동 재생성 대상에서 제외한다(`plan.isCompleted || plan.isPayrollLocked`이면 건너뜀). 이런 배정은 계속 "공정 연결 확인 필요"로 표시된 채 남으며, 완료된 배정을 고치려면 기존 완료 조정 절차를 따로 거쳐야 한다.
- **급여(생산수당) 계산에는 이 변경이 영향을 주지 않는다**: `backend/src/payroll/payroll.service.ts`의 CT 기반 생산수당은 `AssignmentPlan.assignmentCtSnapshot`을 다시 읽지 않고, 작업기록을 저장하는 시점에 그 값을 그대로 복사해 `WorkRecord.ctSeconds`에 얼려둔 값만 사용한다. 나중에 배정의 CT가 바뀌어도 이미 저장된 작업기록의 급여는 전혀 바뀌지 않는다. 생산 ST 초과율(`PRODUCTION_ST_EXCESS_PERCENT`) 계산만 `AssignmentPlan.assignmentStSnapshot`을 매 계산 시점에 다시 읽어 반영하는데, 이 값도 해당 월 급여가 잠기면(`PayrollSnapshot` 확정) 그 이후로는 스냅샷된 결과만 쓰고 다시 계산하지 않으므로 잠긴 급여는 안전하다.
- 이 정책은 "확정 CT/ST는 원칙적으로 보존, ID 복구는 재생성이 아니라 검증된 대응으로만"이라는 기존 방침(§CT 스냅샷 감사 재검토)의 예외가 아니라, 그 "검증된 대응" 절차 자체를 공정 버전 관리 화면의 명시적 저장 행위로 규정한 것이다 — 사용자가 명시적으로 적용 구간을 저장할 때만 실행되며, 자동 배경 작업이나 조회 시 자동 저장으로 실행되지 않는다.
- `npm run test:assignment-snapshot-integrity`에 사전 전체 검사가 제거됐는지, 완료·급여잠금 배정은 계속 건너뛰는지 확인하는 회귀 테스트를 추가했다.
...
- 라인 제거 자체가 아니라 9월 9일 공정 무결성 검사와 기존 조회 필드의 불일치가 원인이다. FK 조회만 보완하며 기존 CT/ST·작업 기록은 재생성하거나 수정하지 않는다. 실제 손상된 공정 연결의 계산 제외는 유지한다.
- `test:assignment-snapshot-integrity`에서 실제 조회 select로 만든 배정의 정상 판정과 잔여 3,800초, 기존 styleId 누락 형태의 오판정을 검증한다. 운영 화면의 배포 후 확인은 별도다.

## 2026-09-10 주문 저장 버튼 변경 감지 보완

- 주문 상세·신규 저장 버튼은 `hasFormChanges`가 참일 때만 활성화한다. 저장 중이거나 수정 잠금이면 비활성화하며 저장 핸들러도 같은 상태를 확인한다.
- 주문 비교 서명에서 FK가 있는 거래처·스타일·색상의 표시 이름/코드는 제외해 조회 중 이름 갱신을 주문 수정으로 오인하지 않는다. FK, 주문번호, 납기, 상태, 사이즈 체계, 행 및 수량 변경은 계속 감지한다. FK 없는 항목의 이름/코드는 비교한다.
- 기존 저장 성공 시 서버 응답으로 폼과 비교 기준을 갱신하는 흐름을 유지한다. 원래 값으로 되돌리면 변경 상태가 해제된다.

## 2026-09-09 출퇴근 Excel 사번 매칭 및 단일 기록 보완

- Excel ID는 `Employee.employeeNo`와 비교한다. 내부 DB `id`와 비교하지 않는다. 사번이 있으면 해당 사번만 사용하고, 사번이 없는 행만 기존 이름 매칭을 사용한다. 사번 중복 또는 이름 불일치 시 전체 파일 계획 생성을 중단해 쓰기 전에 오류를 알린다. 성조 제거·Đ 변환·중간 이름 축약 호환을 유지한다.
- 직원별 하루 고유 타임스탬프가 한 개면 정오 이전은 퇴근 17:00, 정오부터는 출근 08:00을 보완한다. 실제 시각은 유지하고 직원 이름·누락 종류·생성 시간을 메모에 남긴다. 동일 타임스탬프 중복 행은 한 번으로 간주한다. 두 개 이상이면 최초/최종 시각을 사용한다.
- 목록·상세 업로드 모두 적용하며 기존 메모를 보존하고 보완 메모를 중복 없이 추가한다. 과거 오귀속 기록은 자동 삭제하지 않는다. 운영 기록 정리는 별도 확인이 필요하다.
- `npm run test:attendance-import`로 사번/내부 ID 충돌, 이름 검증, 오전·오후 보완, 직원 이름 메모 및 병합을 검증한다.

## 2026-09-09 라인 도메인 제거 — 코드 완료, 운영 적용 확인 대기

- 이 항목은 아래 9월 3~4일의 라인 과도기 호환 지침을 대체한다. Prisma의 `Line`/`LineAssignment` 및 직원·배정·작업기록·외주작업의 `lineId` 관계를 제거했다. 신규 기능에서 라인 호환 계층을 다시 만들지 않는다.
- 배정·생산능력·작업 입력·Excel 업로드·급여 집계는 `factoryId`만 사용한다. 공장 직원은 `/factory-workers`, 월 생산능력은 `/factory-month-capacity`로 조회한다. 기존 `/lines`와 라인 ID payload는 지원하지 않는다.
- 배정의 공장은 필수이며 작업기록의 과거 공장 귀속은 부모 `WorkLog.factoryId`를 사용한다. 현재 직원 소속으로 과거 작업을 재귀속하지 않는다. 확정 급여 JSON과 배정 CT/ST 스냅샷을 다시 생성하지 않는다.
- `backend/scripts/remove-line-domain.sql`을 `prisma-safe-deploy.js`의 db push 전 및 서버 시작 전에 실행한다. 기존 FK로 확인되는 공장만 백필하고, 모호한 공장·조직 충돌·과거 다른 공장 배정 이력·예상하지 못한 종속 FK가 있으면 전체 롤백한다. `migration_fix.sql`은 삭제한 라인 구조를 다시 생성하지 않는다.
- `npm run test:factory-domain`은 실제 Prisma 스키마와 PostgreSQL 엔진을 이용한 마이그레이션·반복 배포·롤백, 공장 ID 계약, 작업 조회의 무변경성, 생산수당 보존을 검증한다. 기존 배정 CT/ST 및 접근권한·내비게이션 회귀 검사도 유지한다.
- 현재 작업 환경에 운영 DB 연결 설정이 없어 Railway 실적용은 미확인이다. 배포 후 `npm --prefix backend run audit:line-removal`로 테이블·컬럼 제거와 공장 귀속을 확인해야 운영 완료다. 남은 확인 작업은 `APP_AUDIT_2026-09-08.md`에 유지한다.


## 2026-09-09 공정 연결 무결성 방어

- 공정 버전 관리 창의 조회 시 자동 적용 구간 저장을 제거했다. 불일치 배정은 경고와 개별 `공정 연결 확인` 표시로 안내하고 기존 CT/ST는 그대로 보존한다. 적용 구간 저장을 손상된 스냅샷의 재생성·복구 우회로 사용하지 않는다.
- `assignmentSnapshotIntegrity`는 원본 CT/ST의 누락·중복 ID, 서로 다른 공정 집합/버전, 실제 `StyleProcess` 소속 및 저장된 확정 버전과의 불일치를 검사한다. 신규 배정은 서버에서 CT/ST 생성 후 검사하고, 기존 배정의 저장·수동 완료·버전 적용도 관련 검사를 통과해야 한다.
- 불완전한 공정 집합의 잔여 ST는 부분 합계나 0으로 확정하지 않고 `null`과 `isProgressUnknown`으로 표시한다. 손상된 미완료 배정은 자동 완료로 추론하지 않는다. 기존 완료 확정 이력 자체는 수정하지 않는다. 검토 표는 배열 위치 대신 공정 ID로 작업수량을 매칭한다.
- 정상 3공정 잔여 3,800초 계산을 유지하고, 가운데 ID 누락 시 과거의 잘못된 1,800초 대신 확인 불가로 처리하는 회귀 테스트를 추가했다. `test:assignment-snapshot-integrity`는 원본 시간 보존과 조회의 자동 쓰기 금지도 검증한다.
- 완료한 코드 수정 항목은 `APP_AUDIT_2026-09-08.md`에서 제거했다. 운영 DB 연결 설정이 없어 현재 데이터 영향 확인·필요한 ID 복구는 그 파일의 별도 대기 항목으로 남겼다. `audit:assignment-snapshots`로 읽기 전용 점검할 수 있다. 이 단계에서 DB 기록이나 기존 시간은 수정하지 않았다.

## 2026-09-09 출퇴근 단일 월 선택

- 출퇴근 목록은 글로벌 `MonthSelector`를 사용한다. 시작·종료 월 범위를 없애고 선택한 월 하나만 조회하며, 일자 목록도 그 달의 첫날부터 말일까지 표시한다. 공용 `M+`/`M-` 동작을 그대로 사용한다.

## 2026-09-09 급여명세서 개선: 지급월 아닌 항목 표시, 성과급 열 계산 수정, 정보 레이아웃 정리

- **지급 주기가 안 맞는 항목을 목록에서 빼지 않고 "다음 지급"으로 표시**: 정산 주기가 분기·반기·연간인 항목(예: 근속수당 6개월 주기)이 해당 월에 지급 대상이 아니면 이전에는 그 달 급여명세서 항목 목록에서 통째로 사라져 사용자가 "이 항목이 왜 없지"라고 혼란스러워했다(실사용 중 실제로 재현됨: 7월 명세서에 반기 지급 근속수당이 안 보임). `backend/src/payroll/payroll.service.ts`의 `applicableItems` 필터에서 `paymentMonths` 조건을 제거해(급여 타입 조건만 유지) 지금은 그 달이 아니어도 항목 자체는 목록에 남고, 금액은 0으로 계산하지 않은 채(계산식 자체를 실행하지 않음) `isPaymentMonth: false`와 `nextPaymentMonthKey`("YYYY-MM", 새 헬퍼 `resolveNextPaymentMonthKey`로 계산)를 함께 반환한다. 프론트(`PayrollEntry.jsx`)는 이 항목의 금액 칸에 `다음 지급: 2026년 12월` 식으로 표시하고 행 전체를 옅게(opacity) 보여준다.
- `normalizePayrollSnapshotEmployee`(과거 스냅샷을 다시 읽을 때 쓰는 화이트리스트 정규화 함수)에도 `isPaymentMonth`/`nextPaymentMonthKey`를 추가했다. 이 필드 도입 이전 스냅샷은 애초에 지급 대상 항목만 저장했으므로 `isPaymentMonth`가 없으면 `true`로 취급하고 `nextPaymentMonthKey`는 복구하지 않는다(과거 데이터를 추정해서 채우지 않는다).
- **급여 계산 상세 목록의 "생산수당" 열이 성과급(INCENTIVE) 카테고리 전체가 아니라 CT 기반 생산수당 하나만 집계하던 버그를 수정**: §"성과급(INCENTIVE) 카테고리에 두 번째 항목 지급 허용" 이후 INCENTIVE 카테고리에 `생산수당` 외에 사용자 정의 항목(예: `초과 수당`)이 추가로 있을 수 있는데, `PayrollEntry.jsx`의 직원별 급여 계산 목록 테이블은 이 열의 값을 `productionAllowanceOf(employee)`(CT×수량 기반 생산수당만)로만 계산하고 있었다. 실사용 데이터로 확인: 최종 지급액(19,377,970)은 `기본급+수당+생산수당(7,135,763)+초과수당(6,442,207)`을 다 더한 값과 일치하지만, 목록의 "생산수당" 열은 초과수당을 빼먹은 7,135,763만 보여줘서 열 합계가 최종 지급액과 안 맞았다. `baseAmount`/`allowanceAmount`와 동일한 방식(`salaryItems.filter(item => item.category === 'INCENTIVE').reduce(...)`)으로 통일해 INCENTIVE 카테고리의 모든 항목 합계를 보여주도록 고쳤다.
- **급여 계산 상세 목록과 급여명세서의 열/제목을 급여 체계 화면의 카테고리 이름과 항상 같게 유지**: 기존에는 이 목록이 독자적인 문구("일반 수당", "생산수당")를 썼는데, 급여 체계 화면(`SalarySystem.jsx`)의 카테고리 라벨(`기본급`/`수당`/`성과급`)과 어긋났다. `frontend/src/constants/payType.js`에 이미 있던 `PAY_COMPONENT_DEFAULT_LABELS`(BASE_SALARY/ALLOWANCE/INCENTIVE)를 소스오브트루스로 삼아 새 헬퍼 `getPayComponentLabel(category, fallback, languageCode)`를 추가했고, `PayrollEntry.jsx`의 목록 헤더 3칸이 이 헬퍼를 직접 호출하도록 바꿨다. 이제 급여 체계 카테고리 이름이 바뀌면(그 상수를 통해서) 이 목록도 자동으로 같이 바뀐다.
- **급여명세서 상세의 "급여 계산 기본 정보" 레이아웃을 하나의 긴 4열 그리드에서 행별 독립 그리드(Stack of Box)로 분리**: 기존에는 항목 개수에 따라 필드 쌍의 좌/우 배치가 암묵적으로 밀려서(4셀 배수가 아니면 다음 필드가 엉뚱한 줄에 걸침), 실사용 중 "이번달 기준 근무시간"과 "이번달 생산 총 ST"가 서로 다른 줄에 떨어져 나와 이해하기 어렵다는 지적을 받았다. 각 행을 독립된 `Box`(같은 4열 grid sx)로 분리해 원하는 두 필드가 항상 같은 행에 오도록 고정했다: `직원|급여타입`, `실제근무일수|기준근무일수`, `휴일근무일수|만근여부`, `(OUTPUT만) 이번달 기준 근무시간|이번달 생산 총 ST`, `근속연수|급여체계버전`. 앞으로 필드를 추가/제거해도 각 행의 짝은 셀 개수 계산 없이 안전하게 유지된다.
- "이번달 생산 총 ST" 값 옆에 기준 근무시간 대비 달성률을 괄호로 함께 표시한다: `494.3시간 (228.8%)` — `PRODUCTION_ST_HOURS ÷ PRODUCTION_ST_BASELINE_HOURS × 100`으로 화면에서 계산한다(둘 다 이미 백엔드가 내려주는 값이라 새 백엔드 필드는 추가하지 않았다). 이 228.8%는 `PRODUCTION_ST_EXCESS_PERCENT`(128.8, 100% 초과분만)와 다른 값(달성률 자체, 100% 포함)이므로 혼동하지 않는다.
- **보류된 요청**: 사용자가 "연차 사용일수"도 기본 정보에 추가해달라고 요청했으나, 이 시스템에는 연차/휴가를 기록하는 개념(스키마·화면 등)이 전혀 없어 실제 값을 만들 수 없다. 레이아웃 정리만 먼저 반영했고, 연차 기능 자체(기록 방식, 만근 판정에 미치는 영향 등 정책)는 사용자 확인 후 별도로 진행하기로 보류했다.

## 2026-09-08 생산 ST 초과율 검증용 참고값을 급여명세서 기본 정보에 노출

- §"생산 ST 초과율 파라미터 단위 정정" 이후 사용자가 실제 급여명세서의 `초과 수당: 50,000 VND × 128.8%` 값을 보고 "이게 정말 기준 시간의 2배 넘게 생산했다는 뜻이냐"고 확인했다. 이 초과율이 어떻게 나왔는지(분자/분모) 화면에서 바로 확인할 수 있어야 한다는 요청에 따라, `PRODUCTION_ST_EXCESS_PERCENT` 계산에 쓰인 원본 두 값을 그대로 노출한다: **이번달 기준 근무시간**(`PRODUCTION_ST_BASELINE_HOURS`, 분모 — 기준 근무일수 × 1일 기준 근무시간)과 **이번달 생산 총 ST**(`PRODUCTION_ST_HOURS`, 분자 — 그 직원이 그 달 실제 생산한 수량의 ST초 합계). 둘 다 시간(h) 단위로 변환해 급여명세서 `급여 계산 기본 정보`(화면·인쇄 양쪽)에 `OUTPUT` 급여 타입 직원에게만 표시한다.
- 이 두 값은 `backend/src/payroll/payroll.service.ts`의 `parameters` 객체에 포함되지만 `SALARY_FORMULA_PARAMETERS`(계산식에서 실제로 쓸 수 있는 파라미터 목록)에는 등록하지 않았다 — 계산식 표현에는 이미 존재하는 `PRODUCTION_ST_EXCESS_PERCENT` 하나로 충분하고, 이 둘은 그 결과가 어떻게 나왔는지 사람이 검증하기 위한 표시 전용 참고값이다.
- `parameters` 객체 내용이 바뀌므로 이 값을 처음 배포한 시점에 기존 미잠금(provisional) 급여 스냅샷의 계산 서명(`calculationSignature`)이 달라져 `재계산 필요`로 표시될 수 있다 — 확정 잠금된 스냅샷에는 영향 없다.

## 2026-09-08 생산 ST 초과율 파라미터 단위 정정: 비율 → 퍼센트 포인트

- 사용자가 "초과 수당"이라는 신규 성과급 항목을 만들면서 확인한 문제: `PRODUCTION_ST_EXCESS_RATIO`(당시 설계)는 10% 초과를 `0.1`로 표현하는 비율값이었다. 그런데 사용자가 원한 사용법은 "1% 초과 생산한 직원에게 얼마를 줄지"를 직급별 단가에 그대로 적는 것(예: 단가 100,000 = 1% 초과당 100,000동 지급)이었다. 계산식이 `GRADE_RATE × PRODUCTION_ST_EXCESS_RATIO`이므로 이 단가 입력 방식과 맞으려면 초과 비율이 아니라 "초과 퍼센트 포인트 숫자"가 필요했다 — 10% 초과면 `10`이어야 `GRADE_RATE × 10`이 "1%당 단가 × 초과 퍼센트"가 된다. 비율(`0.1`)로는 계산 결과가 사용자 의도의 100분의 1로 나온다.
- 파라미터 자체를 **`PRODUCTION_ST_EXCESS_RATIO` → `PRODUCTION_ST_EXCESS_PERCENT`로 이름과 스케일을 함께 바꿨다** (10% 초과 → 값 `10`, 값의 의미가 바뀌므로 이름도 반드시 같이 바꿔서 옛 이름의 계산식이 남아있으면 조용히 틀린 값을 내지 않고 "알 수 없는 파라미터"로 명시적으로 실패하게 했다). 계산: `(max(0, 생산ST초 - 기준근무시간초) / 기준근무시간초) × 100`.
- **이 파라미터를 도입 당일 바로 고친 것이라 실제 운영 데이터/급여 스냅샷에 영향은 없다.** 다만 사용자가 화면에서 이미 `GRADE_RATE × PRODUCTION_ST_EXCESS_RATIO`로 계산식을 조립해본 상태였다면(스크린샷으로 확인), 저장 전이었어도 그 계산식은 이제 존재하지 않는 파라미터를 참조하므로 화면에서 파라미터 칩을 지우고 새 `PRODUCTION_ST_EXCESS_PERCENT`(파라미터 모듈의 "생산 실적" 그룹에 동일한 위치로 노출)를 다시 끼워 넣어야 한다.
- 설명 문구(툴팁 hint)도 "예: 0.1은 10% 초과"에서 "예: 10% 초과하면 10, 미달이면 0(비율 0.1이 아님). 직급별 단가는 초과 1%포인트당 지급액을 의미한다"로 명확화했다. 급여명세서 상세(`PayrollEntry.jsx`)의 값 표시도 `숫자×100 + '%'`에서 `숫자 + '%'`로 맞춰 고쳤다(값 자체가 이미 퍼센트 스케일이므로 다시 100을 곱하면 안 된다).
- 변경 파일: `backend/src/employees/salaryFormula.ts`(파라미터 셋), `backend/src/payroll/payroll.service.ts`(계산·주석), `frontend/src/pages/App/SalarySystem.jsx`(파라미터 정의·힌트·그룹), `frontend/src/pages/App/payroll/PayrollEntry.jsx`(라벨·값 포맷). 검증: `npm run test:salary-system` 통과, 백엔드/프론트 빌드 통과.

## 2026-09-08 급여 타입별 근무 기준을 급여 계산에서 급여 체계로 재배치

- 급여 타입(사무/생산(고정)/생산(변동))별 근무 요일·기준 출퇴근 시간·휴게시간·근무일 인정 기준(1일 기준 근무시간 포함)은 §"2026-09-03 급여 계산 공용 설정 및 급여 타입 변경"이 결정한 대로 `급여 계산` 화면의 `설정` 다이얼로그에 있었으나, 사용자 판단에 따라 **`급여 체계` 화면의 설정**으로 옮겼다. 이 값은 급여 계산 실행(월별 스냅샷)의 부속물이 아니라 급여 체계(계산식·직급별 단가)와 같은 층위의 조직 기준 데이터이기 때문이다.
- 이 설정 자체의 데이터 범위는 그대로다. API(`GET/PUT /employee-pay-type-policies`)와 저장 모델은 여전히 조직 전체(공장별 아님) 값이라, 급여 체계 화면의 공장 탭 선택과 무관하게 항상 같은 값을 조회·저장한다.
- 새 컴포넌트 `frontend/src/pages/App/PayTypeScheduleSettingsDialog.jsx`가 이 설정 전용 다이얼로그다. `SalarySystem.jsx`의 공장 탭 행에 `설정` 버튼(`버전 관리`/`저장` 왼쪽)을 추가해 이 다이얼로그를 연다. 탭이 하나뿐이라 내부에 별도 탭 구분 없이 바로 근무 요일/시간 폼만 보여준다.
- `frontend/src/pages/App/payroll/PayrollSettingsDialog.jsx`(급여 계산 화면의 `설정`)에서는 이 근무 기준 탭과 관련 상태·API 호출을 전부 제거했다. 이제 이 다이얼로그는 `상시 만근 직원`/`급여 예외 직원` 드래그앤드롭 관리만 남으며, 탭 UI 자체도 제거했다(섹션이 하나뿐이므로). 저장 대상 API(`/payroll/settings`)와 그 의미는 바뀌지 않았다.
- 저장 시 발행하는 워크스페이스 변경 이벤트(`SALARY_SYSTEM_SETTINGS`)는 그대로 유지해 급여 계산·급여명세서 화면이 계속 이 값 변경을 감지한다.

## 2026-09-08 성과급(INCENTIVE) 카테고리에 두 번째 항목 "생산 ST 초과율" 지급 허용

- 지금까지 `INCENTIVE` 카테고리는 물리적으로 정확히 1개(고정 `생산수당`, `code: incentiveTotal`, 계산식이 항상 `PRODUCTION_ALLOWANCE` 단일 토큰)만 존재할 수 있었다. `ensureFixedIncentiveItem`이 매 `GET /salary-system`마다 "첫 번째로 찾은 INCENTIVE 항목"을 강제로 그 고정 모양으로 되돌리고 나머지는 전부 비활성화했고, `validateSalaryFormula`도 `category==='INCENTIVE'`이면 무조건 그 단일 토큰만 허용했다. 이번에 이 제약을 풀어 **`incentiveTotal` 코드를 가진 항목만 그 고정 모양을 유지**하고, 그 외 `INCENTIVE` 카테고리 항목은 `GRADE_RATE`로 시작하는 일반 계산식(다른 수당 항목과 동일한 문법)을 쓸 수 있게 했다. 급여 항목 추가 다이얼로그의 "급여 구분" 선택지에서도 `성과급`을 더 이상 숨기지 않는다.
- ~~신규 파라미터 `PRODUCTION_ST_EXCESS_RATIO`를 계산식에 추가했다. 정의: ... 초과한 비율이다(0.1 = 10% 초과).~~ **2026-09-08 정정 (파라미터명·단위 변경)**: `PRODUCTION_ST_EXCESS_RATIO`(0.1 = 10% 초과, 비율 스케일)를 `PRODUCTION_ST_EXCESS_PERCENT`(10 = 10% 초과, 퍼센트 포인트 스케일)로 이름과 스케일을 함께 바꿨다. 아래 "2026-09-08 생산 ST 초과율 파라미터 단위 정정" 참고. 정의(수량 계산 로직)는 바뀌지 않았다: 그 직원이 그 달 실제로 생산한 수량의 ST초 합계(`AssignmentPlan.assignmentStSnapshot.processes[]`의 `styleProcessId`별 `stSeconds` × 그 공정 실적 수량을 WorkRecord 기준으로 합산, CT/생산수당과는 별개의 값)가, 생산(OUTPUT) 급여 타입의 그 달 기준 근무일수(일요일·등록 휴일 제외, 개인 출퇴근 실적과 무관한 공장 공통 고정값) × 1일 기준 근무시간(급여 체계 화면의 근무 기준 설정값)을 초과한 정도를 계산한다. 미달이면 0으로 클램프한다(초과분에 대한 보너스이며 페널티가 아니다). 이 정의는 사용자가 명시적으로 확정한 것이다: "①연속 비례 지급(임계값·구간 없음), ②기준 근무시간은 공장 공통 고정값이며 그 달의 실제 근무 일수(휴일 제외) 기준".
- 예: `GRADE_RATE × PRODUCTION_ST_EXCESS_PERCENT`로 새 성과급 항목을 만들면, 그 직급의 단가(= 초과 1%포인트당 지급액)에 초과 퍼센트를 곱한 만큼 지급된다(기존 `만근수당 = GRADE_RATE × FULL_ATTENDANCE_FACTOR`, `근속수당 = GRADE_RATE × TENURE_YEARS`와 동일한 패턴). 이 항목의 급여 타입은 다른 성과급 항목과 마찬가지로 항상 `OUTPUT` 전용, 정산 주기는 항상 매월로 서버가 강제한다. 상한값(선택)은 다른 일반 항목처럼 설정할 수 있다(고정 `생산수당` 항목만 상한을 둘 수 없다).
- 백엔드 변경: `backend/src/employees/salaryFormula.ts`(파라미터 추가, `validateSalaryFormula`를 "코드가 `incentiveTotal`인 항목만 고정 모양" 기준으로 재작성, `evaluateSalaryFormula`의 내부 재검증을 카테고리 추측 방식에서 순수 문법 검사(`isWellFormedFormulaTokenSequence`)로 분리 — 이 참에 무관한 기존 버그(상수만 있는 계산식이 나눗셈까지 못 가고 "invalid salary formula"로 먼저 막히던 것, `test:salary-system`이 이미 실패 중이었음)도 함께 고쳤다), `backend/src/employees/salarySystem.routes.ts`(`ensureFixedIncentiveItem`을 `code==='incentiveTotal'` 기준으로 재작성, "정확히 1개의 INCENTIVE 항목" 검증을 "정확히 1개의 `incentiveTotal` 항목" 검증으로 변경, PUT 저장 시 이름/계산식/상한 강제 오버라이드를 `code==='incentiveTotal'` 기준으로 좁힘, INCENTIVE 항목의 단가(rate) 저장 차단을 `incentiveTotal`에만 적용), `backend/src/payroll/payroll.service.ts`(`buildIntegratedPayrollEmployees`가 WorkRecord별로 연결된 `AssignmentPlan.assignmentStSnapshot`을 배치 조회해 직원별 월 생산 ST초 합계를 집계 → `getPayrollByMonth`가 이 값과 공장 공통 기준 근무시간으로 비율을 계산해 `parameters.PRODUCTION_ST_EXCESS_RATIO`로 노출, 급여 항목의 단가 필수 여부 판정을 "카테고리가 INCENTIVE가 아니면"에서 "계산식이 `GRADE_RATE`로 시작하면"으로 수정 — 새 성과급 항목도 정상적으로 단가 누락을 409로 막는다).
- 프론트 변경: `frontend/src/pages/App/SalarySystem.jsx`에서 `category === 'INCENTIVE'` 기준으로 잠겨 있던 8곳(항목 추가 다이얼로그 카테고리 선택지, 저장 시 단가 필터링 2곳(`salaryStateSignature`/`saveDraft`), 데이터 로드 시 강제 오버라이드, 수정 버튼/계산식 잠금 판정)을 `id === 'incentiveTotal'`(또는 계산식 내용) 기준으로 좁혔다 — 특히 `saveDraft`의 `editableItems` 필터는 이 수정 없이는 새 성과급 항목의 단가를 아예 서버로 보내지 않는 치명적인 누락이었다. 계산식 편집기에 "생산 실적" 파라미터 그룹(`incentiveOnly: true`, 성과급 항목 편집 시에만 노출)을 추가했다. `frontend/src/pages/App/payroll/PayrollEntry.jsx`의 급여명세서 계산식 표시용 파라미터 사전에도 라벨/포맷(백분율 표시)을 추가했다 — 이 파일은 이미 항목을 카테고리 개수 가정 없이 배열로 순회하고 있어 별도 구조 변경은 필요 없었다.
- 검증: `npm run test:salary-system`, `npm run test:edit-revision` 통과. `npm run test:regression`은 이 변경과 무관한 기존 실패 2건(`relational-integrity-regression.test.mjs`의 EmployeeGrade 시드 로케일 이름, WorkRecord customer-owned Style 조회 패턴 — `git stash`로 변경 전 HEAD에서도 동일하게 재현 확인)만 있고 신규 실패는 없다.

## 2026-09-08 CT 스냅샷 감사 재검토 — 당시 검토 기록, 2026-09-09 구현 항목으로 대체

- `APP_AUDIT_2026-09-08.md`의 “자가치유가 없으므로 버전 확정 시 기존 배정을 일괄 재생성” 권고는 재검토로 폐기했다. 확정 CT/ST 보존은 정상 계약이며, ID 복구를 공정·시간 기준 재생성으로 대신하지 않는다.
- 실제 검증 공백은 버전 ID가 있으면 공정 ID 누락 검사를 건너뛰는 저장 준비도와, 일부 ID가 빠지면 그 공정이 잔여 ST에서 누락되고 검토 표의 수량 행이 밀리는 계산 경로다. 3공정 재현에서 기대 3,800초가 1,800초로 계산됐다. 운영 데이터의 현재 영향 건수는 DB 연결 설정 부재로 확인하지 못했다.
- `ProcessVersionManager.load`는 불일치 발견 시 적용 구간 저장 API를 자동 호출하고, 서버는 CT/ST와 총시간을 재생성한다. 이 기존 자동 쓰기 경로를 감사에서 놓쳤다. 추후 수정 시 조회와 복구를 분리하고, 과거 수동 CT·ST·완료/급여 잠금 기준을 보존해야 한다.
- 권고는 읽기 전용 운영 감사 → 자동 재생성 중단 및 안내 → 원본 스냅샷 무결성 검증·ID 기준 표시 → 검증된 대응만 별도 복구 순서다. 이번에는 코드나 DB를 수정하지 않았으며, 상세 근거와 미완료 항목은 감사 목록에 유지한다.

## 2026-09-08 배정 보드·급여 체계 동시 저장 보호

- 배정 보드는 조직별로, 급여 체계는 조직·공장별로 조회 당시의 `editRevision`을 저장 요청의 `expectedRevision`으로 보낸다. 누락되거나 서버 현재값과 다르면 `409 STALE_EDIT`로 거부한다. 저장 트랜잭션 안에서 다시 확인하고 Serializable 격리 수준으로 검사 이후의 동시 저장도 차단한다.
- 배정 보드 revision은 보드 상태·배정·카드의 ID와 수정 시각을 반영한다. 프론트는 여러 소스 조회를 시작하기 전에 revision을 확보한다. 급여 체계 revision은 항목·단가·통화·버전·적용 월 내용을 반영하며, 조회 시 호환 정규화가 갱신하는 `updatedAt`만으로 변경으로 판정하지 않는다.
- 급여 체계의 항목·단가 저장과 새 버전 생성은 `PUT /salary-system`의 `createVersion: true`로 한 트랜잭션에 묶는다. 버전 생성 실패 시 항목 변경도 롤백한다. 별도 버전 생성과 적용 월 저장도 revision 검사를 통과해야 한다.
- 두 화면은 활성 화면에서 15초마다, 창 포커스 복귀 시, 화면 재진입 시 최신 여부를 확인한다. 다른 저장이 발견되면 안내와 `최신 현황 새로고침` 버튼을 표시하고 오래된 화면의 저장을 막는다. 미저장 변경을 자동 삭제하지 않으며 사용자가 새로고침을 선택할 때 폐기 안내를 함께 표시한다. 안내를 Esc/배경 클릭으로 닫아도 경고와 저장 차단은 유지한다.
- `test:edit-revision`은 stale 저장, 동시 저장, 적용 월 변경, 버전 생성 롤백, 보드 revision, 폴링 및 조직 전환을 검증한다. 운영 DB 마이그레이션은 없다. 실제 서로 다른 PC에서의 화면 검증은 별도로 필요하다.
...
- 라우터에서 이미 고아였던 `Line.jsx`, `line/LineBoard.jsx`, `line/LineDetail.jsx`를 삭제했다. 메뉴에서 숨겨져 있었지만 직접 URL로 접근 가능했던 구형 라인 기반 `/production-plan` 라우트와 `ProductionPlan.jsx`, `production/ProductionPlanBoard.jsx`도 삭제했다.
- 직원 재입사, 작업기록 입력 안내, Excel 업로드 완료 처리에서 사용자에게 노출되던 라인 문구와 더 이상 서버가 반환하지 않는 교차 라인 경고 분기를 제거했다. 예시 수익 분석, 예가, 재고 프로토타입 문구도 공장/생산 기준으로 바꿨다.
- 이 정리는 사용자 화면과 죽은 코드에 한정한다. 스키마와 백엔드의 과도기 호환 `Line`/`LineAssignment` 계약은 아래 전환 원칙대로 후속 제거한다.

## 2026-09-04 과거월 계획 부하와 실제 생산률 분리

- 과거월의 `계획 부하`를 실제 생산률과 동일하게 강제하지 않는다. 해당 월 실제 생산 ST와 현재 남아 있는 배정 작업 ST를 합산해 월 생산능력 대비 계획 부하를 계산하고 화면 표시는 100%를 상한으로 둔다.
- `실제 생산률`은 계속 해당 월 작업기록의 실제 생산 ST만 월 생산능력으로 나눈 값이다. 예를 들어 남은 배정 물량이 다음 달 이후까지 이어지고 8월 실제 생산률이 87.9%라면, 8월은 `계획 부하 100% / 실제 생산률 87.9%`로 구분해 표시한다.
- 남은 배정 작업이 전혀 없다면 이를 이유로 과거월 계획 부하를 무조건 100%로 만들지 않는다. 이 계약은 `test:line-removal-transition`에서 검증한다.

## 2026-09-04 배정 생산능력의 공장 재직 인원 기준 전환

- 배정 보드의 현재 인원수와 월별 생산 가능 시간은 더 이상 `LineAssignment` 존재 여부로 직원을 포함하지 않는다. `Employee.factoryId`, 봉제 작업자 역할, `joinedAt`/`leftAt` 재직 구간을 기준으로 공장 인원을 직접 계산한다.
- 신규 봉제 직원은 별도 라인 배정 이력 없이 소속 공장의 입사일부터 생산능력에 포함한다. 퇴사일은 마지막 재직일로 포함하고 다음 날부터 제외한다. 퇴사 후에도 과거 귀속 보존을 위해 `Employee.factoryId` 자체는 유지한다.
- 배정 응답의 기존 `lineId` 계약과 아직 남은 작업기록·급여 호환 경로 때문에 `Line`/`LineAssignment` 테이블을 이번 단계에서 물리 삭제하지는 않았다. 단, 배정 인원수와 생산능력 계산의 `LineAssignment` 의존은 제거했다. 나머지 읽기·쓰기 경로를 공장 기준으로 전환하고 결과 보존을 검증한 후 물리 제거한다.
- 이 계약은 `npm run test:line-removal-transition`에서 회귀 검증한다.

## 2026-09-03 라인 개념 제거 전환 상태

- 운영 기준은 `라인`이 아니라 `공장`이다. 현재 조직에는 공장 1개와 그 공장에 속한 기존 라인 1개만 있으며, 배정은 별도 라인별 순차 배정이 아니라 전부 동시 배정으로 운영한다. 새로운 기능에서 라인을 업무 단위로 다시 도입하지 않는다.
- Railway 운영 DB를 읽기 전용 감사한 결과 공장 1개, 라인 1개이며 공장별 라인 수 오류, 공장으로 변환할 수 없는 배정, 직원의 라인-공장 불일치, 작업기록 및 외주작업의 공장 충돌은 모두 0건이었다.
- `AssignmentPlan.factoryId`를 추가하고 기존 `Line.factoryId`로 백필하는 전환 마이그레이션을 작성했지만 Railway 운영 DB에는 아직 적용하지 않았다. 현재 코드는 배정 저장·응답과 배정 보드를 공장 ID 기준으로 전환하는 과도기 구현이며, 기존 데이터 호환을 위해 내부 `lineId`와 `Line` 관계도 함께 유지한다.
- 사용자 화면의 라인 관리 메뉴와 `/line` 라우트는 제거했다. 배정 보드는 공장마다 하나의 배정 영역만 표시한다.
- 아직 `Line` 테이블, `Employee.lineId`, 작업기록의 라인 참조와 일부 API·번역·호환 코드는 물리적으로 제거하지 않았다. 이들을 제거하기 전 모든 읽기·쓰기 경로를 공장 기준으로 전환하고, 과거 기록의 공장 귀속과 급여·생산 집계 결과가 동일한지 검증해야 한다. 현 상태를 "라인 완전 제거 완료"로 표현하지 않는다.
- 안전 검사는 `npm --prefix backend run audit:line-removal`로 실행한다. Supabase Postgres는 인증 전용이므로 이 검사는 Railway Postgres가 아닌 DB를 명시적으로 거부한다.
- 일회성 계획 문서 `line-domain-removal-plan.md`는 이 항목으로 핵심 상태를 이관한 뒤 삭제했다. 이후 구현 판단과 인수인계는 이 `AGENTS.md` 항목 및 실제 스키마·마이그레이션을 기준으로 한다.

### 2026-09-04 Claude 검토 후 확정 수정

- 배정 API의 식별자 계약은 과도기 동안 `lineId=기존 Line.id`, `factoryId=Factory.id`로 명확히 분리한다. 같은 `lineId` 필드에 엔드포인트별로 서로 다른 ID 의미를 넣지 않는다.
- 배정 보드는 서버 응답의 `factoryId`를 우선 사용하고, 과거 응답에는 `legacyLineId -> factoryId`를 명시적으로 변환한 뒤 내부 공장 단일 트랙에 올린다. 저장 payload에는 `factoryId`를 반드시 포함한다.
- 서버는 `factoryId`가 명시된 새 payload는 공장 ID 맵으로, `factoryId` 없이 `lineId`만 있는 구버전 payload는 기존 라인 ID 맵으로 각각 해석한다. 두 ID가 우연히 같은 숫자인지에 의존하지 않는다.
- 라인 화면 제거 후 남아 있던 대시보드의 미배정 라인 직원 알림, 직원 추가 후 라인 배정 다이얼로그, `/line` 권한 경로 및 메인 레이아웃의 라인 직원 폴링을 제거했다.
- 공장에 기존 호환 라인이 정확히 하나가 아니면 배정 화면에서 해당 공장을 조용히 숨기지 않고 오류로 중단한다. 전환 완료 전까지 잘못된 구조를 임의로 추정하지 않는다.
- 이 계약을 검증하는 `test:line-removal-transition` 회귀 테스트를 추가했다. 백엔드·프론트엔드 빌드, 접근권한 테스트, 배정 ST 스냅샷 테스트, 워크스페이스 내비게이션 테스트가 통과해야 한다.

### 2026-09-04 작업 기록 화면의 라인 노출 제거

- 작업 기록 목록과 모바일 카드에서는 라인 열·라인명·라인 검색 조건을 표시하지 않고 공장만 표시한다. 월별 직원 상세에서도 라인 요약과 라인 열을 표시하지 않는다.
- 작업 기록 추가·수정 화면은 공장을 선택하면 전환 기간의 유일한 기존 라인을 내부적으로 자동 선택하며, 사용자에게 라인 선택 필드를 노출하지 않는다. 이는 기존 `WorkRecord.lineId` 데이터와 API를 보존하면서 화면의 업무 단위를 공장으로 바꾸는 과도기 호환 처리다.
- 기존 작업 기록은 삭제하거나 재작성하지 않았다. `WorkRecord.lineId`와 작업 기록 API의 물리적 제거는 모든 계산·업로드·배정 연결 경로를 공장 기준으로 전환하는 후속 단계에서 수행한다.

### 2026-09-04 생산 분석·작업 경고·급여 라인 잔재 정리

- 생산 분석은 라인/개인 보기 토글을 제거하고 직원별 보기만 사용한다. 검색과 표·모바일 카드에서도 라인명을 노출하지 않는다.
- 공장이 유일한 배정 단위이므로 작업 저장의 교차 라인 경고를 비활성화했다. 신규 저장·수정·Excel 업로드는 더 이상 교차 라인 경고를 반환하거나 사용자 비고를 자동 수정하지 않는다.
- Excel 업로드가 과도기 내부 라인 검증 오류를 받더라도 사용자 메시지는 직원의 공장 소속과 배정 정보 기준으로 안내한다. 내부 `LineAssignment` 의존 검증을 실제 factory 기반 검증으로 교체하는 일은 `WorkRecord.lineId` 제거 단계에 포함한다.
- 프론트에서 사용하지 않던 급여 라인별 재계산 API(`/payroll/snapshots/:month/recalculate-line`)와 서비스 함수를 삭제했고, 급여 목록의 미사용 라인 문구도 제거했다.
- 통합 급여의 생산수당 집계 내부는 아직 기존 `WorkRecord.lineId`를 사용한다. 결과 보존 검증 없이 이를 제거하지 않으며, 공장별 집계로 전환한 뒤 같은 입력에서 직원별 생산수당과 총 급여가 동일함을 검증해야 한다.

## 2026-09-03 `docs/` 설계 문서 흡수 및 정리

`docs/production-stage-and-ironing-development-plan.md`(2026-08-06 작성, 815줄)와 `docs/ui-page-standardization-audit.md`를 이 파일로 흡수하고 삭제했다. 이 프로젝트는 `AGENTS.md`를 모든 AI 도구의 단일 진입점으로 쓰기로 확정했으므로(파일 하단 "문서 읽기 순서" 참고) 판단·설계 근거를 별도 `docs/` 파일에 분산시키지 않는다. 원본 전체 내용이 필요하면 git 히스토리에서 해당 경로의 삭제 이전 커밋을 조회한다.

### 생산 단계 확장(다림질 등) 설계안 — 아직 미구현, 재개 전 라인 제거 계획과 반드시 조율 필요

`production-stage-and-ironing-development-plan.md`는 다림질·검품·포장을 별도 시스템으로 복제하지 않고 기존 `StyleProcess`/`WorkRecord` 관계를 재사용하되, 생산 단계별로 계산 경계(진행률·capacity·AT·생산수당)를 분리하는 상세 설계안이었다.

**현재 구현 상태**: `StyleProcess.productionStage`(`SEWING`/`IRONING`/`INSPECTION`/`PACKING`) enum 추가와 기존 공정 전체 `SEWING` 백필까지만 완료됐다(§"2026-08-06 생산 공정 작업 종류 후속" 참고). 그 이후 설계했던 `CapacityGroup`, `WorkSession`(직접 측정), 관계형 `AssignmentProcessSnapshot`, `ProductionAllowancePolicy`, `ProductionHandoff`, AT v4 다단계 배분 등은 전혀 구현되지 않았다. 다음에 다림질 등 비봉제 단계를 실제로 활성화할 때는 이 문서가 확정했던 Phase 0(읽기 전용 데이터 감사)부터 다시 시작해야 한다.

확정됐던 핵심 업무 원칙(재개 시에도 유효):

- 다림질은 별도 시스템이 아니라 기존 `StyleProcess`/`WorkRecord` 관계를 재사용하되, PT(설계 참고시간)/CT(생산수당 지급 기준)/ST(계획·생산능력 기준)/AT(실측 관측)는 서로 자동으로 덮어쓰지 않는다.
- 생산수당은 승인된 CT 기준이며 실측 AT가 CT를 자동 변경하지 않는다. 수당 정책이 없는 신규 공정은 자동으로 생산수당에 포함하지 않는다.
- 봉제 노동시간과 다림질 노동시간은 같은 배분 풀에 절대 섞지 않는다. 배분 단위는 `Organization × Worker × coverage × ProductionStage × CapacityGroup`이며, 직접 측정한 시간은 출퇴근 기반 배분에서 먼저 차감해 이중 귀속을 막는다.
- 봉제 완료·다림질 완료·최종 생산 완료는 서로 다른 지표다. 다림질 ST를 봉제 라인의 잔여 부하·capacity에 넣지 않는다. 공장 공용 다림질 직원이 여러 봉제 라인의 제품을 처리해도 다림질 capacity는 특정 봉제 라인 소유가 아니다.
- 과거 배정 snapshot과 과거 AT 관측은 소급 변경하지 않는다. 다림질을 나중에 추가한 배정은 원본을 수정하지 않고 추가 revision(`ADDED_AFTER_ASSIGNMENT`)을 만든다.
- `INSPECTION`/`PACKING`은 DB·계산 계약에서 값만 예약해두고, 현장 규칙이 확정되기 전에는 생성 기능을 활성화하지 않는다.
- 공정명 문자열(`다림질`, `봉제` 포함 여부)로 생산 단계를 추론하지 않는다. 서버 계산은 항상 관계형/enum 값만 신뢰한다.

**2026-09-03 결정**: 라인은 제거하고 공장 하나를 단일 배정 트랙으로 사용한다. 현재 모든 배정은 동시 배정이며 공장 내부의 익명 슬롯이나 `SEWING_LINE` capacity group을 새로 만들지 않는다. 따라서 이후 다림질 확장 설계도 `Line`과의 1:1 연결을 전제로 해서는 안 되며, 공장과 생산 단계처럼 실제 업무상 필요한 축만 사용해야 한다.

현장 확인이 아직 안 된 4가지(구현 재개 전 필수):

1. **다림질 작업 단위**: 한 명이 한 벌을 처음부터 끝까지 다리는지, 초벌/완성을 별도 공정으로 나눠야 하는지, 정상 작업과 재다림질을 구분할 수 있는지.
2. **시간 측정**: 세션을 실제로 시작/종료할 수 있는지, 대기·예열·물보충·청소·휴식을 어떻게 제외할지, 직원 직접 타이머 vs 관리자 수기 입력 중 무엇을 쓸지.
3. **생산수당**: 공장 초당 단가로 일관되게 표현 가능한지, 스타일별 다림질 지급 CT가 다른지, 재다림질을 지급할지.
4. **업무 인계**: 봉제 라인에서 다림질 작업장으로 실제 부분수량 인계가 있는지, 인계·수령을 각각 누가 확정하는지, 봉제 완료 없이 넘어가는 예외가 있는지.

### 화면 공통 구조 감사 — 내용은 이미 반영됨

`ui-page-standardization-audit.md`의 레이아웃 규칙(제목 행 우측에 주요 작업 버튼, 두 번째 행 왼쪽 검색·오른쪽 필터+기간, `M+`/`M-` 최우측)은 이미 위 "공통 페이지 툴바 배치 기준" 섹션에 그대로 반영되어 있었다. 이 규칙을 구현하는 공용 컴포넌트는 `frontend/src/components/AppPageContainer.jsx`(공통 여백·제목 크기·헤더 간격), `PageToolbar.jsx`(검색 좌측/필터·기간 우측 분리), `SearchInput.jsx`(공통 높이·테두리·최대너비·지우기 버튼)다. 그 문서에 있던 라우트별 화면 목록은 당시 시점의 스냅샷이라 이후 추가된 메뉴(급여 체계, 직원 체계, 거래처 세분화, 개인 설정, 고객 생산 보고서 등)가 빠져 있었고 별도로 옮길 실익이 없어 폐기했다 — 현재 전체 라우트는 `frontend/src/router.jsx`가 항상 최신 소스오브트루스다.

## 2026-09-03 급여 예외 직원

- 급여 계산 설정의 `예외 직원` 탭은 `일반 직원 / 근태 예외 직원 / 급여 예외 직원` 세 칼럼으로 관리한다. 한 직원은 한 칼럼에만 속한다.
- `Employee.payrollExcluded`가 설정된 직원은 재직 상태를 유지하더라도 급여 준비도 검사, 생산수당 집계, 통합 급여 계산 및 새 급여 스냅샷 대상에서 완전히 제외한다. 기존 급여 스냅샷에 포함된 직원을 급여 예외로 변경하면 현재 계산 결과와 달라지므로 해당 공장·월은 재계산 대상으로 판정한다.

## 2026-09-03 급여 잠금 해제 후 재계산 요청

- 확정된 급여를 잠금 해제하면 `PayrollSnapshot.recalculationRequested`를 `true`로 저장한다. 이 값은 잠금 해제 이력을 새로고침 후에도 보존하며, 급여 계산 화면의 `재계산 필요` 상태와 일괄 계산 버튼 활성화 조건에 포함한다.
- 월 전체 급여 재계산 저장이 성공하면 `recalculationRequested`를 `false`로 되돌린다. 잠금 해제 후 재계산하지 않은 상태에서는 다시 잠글 수 없다.

## 2026-09-03 급여 계산 공용 설정 및 급여 타입 변경

- 급여 계산 화면에는 특정 기능명이 아닌 `설정` 버튼을 두고, 향후 설정 항목을 추가할 수 있는 다이얼로그로 확장한다. 첫 설정은 직원별 `상시 만근`이며 미적용/적용 목록 사이 드래그앤드롭으로 일괄 관리한다. 직원 수정 화면에서는 이 체크박스를 제거한다.
- 급여 계산 화면의 `최종 업데이트` 표시와 `설정` 버튼은 페이지 제목과 같은 첫 번째 행의 오른쪽에 배치한다. 계산 월과 일괄 계산은 그 아래 도구 행에 둔다.
- ~~직원 체계에 있던 급여 타입별 근무 요일·기준 출퇴근 시간·휴게시간·근무일 인정 기준은 급여 계산 설정으로 옮긴다. 설정 다이얼로그는 `급여 타입별 만근 기준`과 `상시 만근 직원` 탭으로 구분한다.~~ **2026-09-08 정정**: 이 근무 요일·시간 기준은 급여 계산이 아니라 급여 체계 화면의 설정으로 옮겼다. 아래 "2026-09-08 급여 타입별 근무 기준을 급여 계산에서 급여 체계로 재배치" 참고. 급여 계산 설정 다이얼로그에는 이제 `상시 만근 직원`/`급여 예외 직원` 관리(탭 없이 단일 화면)만 남는다.
- 급여 계산 설정의 저장 버튼은 두 탭의 서버 저장 의미를 정규화한 서명을 최초 조회값과 비교해 변경이 있을 때만 활성화한다. 값을 원래대로 되돌리면 다시 비활성화한다.
- 감독 직무도 급여 타입 선택을 잠그거나 `GENERAL`로 강제하지 않는다. 저장된 `GENERAL`/`OUTPUT_FIXED`/`OUTPUT` 값을 실제 급여 계산에 사용한다.
- 급여 타입 또는 상시 만근 설정이 바뀌면 계산 서명이 달라져 미잠금(`isProvisional`) 급여 결과는 재계산 대상으로 표시한다. 확정 잠금된 스냅샷은 자동으로 변경하지 않는다.
- 급여 계산의 `일괄 계산` 대상은 `미계산이면서 준비 완료`인 월·공장 또는 `미잠금 상태이면서 재계산 필요`인 월·공장만 포함한다. 단순히 미잠금이라는 이유로 이미 계산 완료된 행을 다시 포함하지 않는다.

## 2026-09-03 급여 체계 저장 안정화

- 급여 항목·단가 일괄 저장은 여러 항목의 순차 upsert가 포함되므로 Prisma 인터랙티브 트랜잭션 제한 시간을 30초로 둔다. 프론트 저장 버튼은 요청 중 로딩 상태로 잠가 중복 PUT과 중복 버전 생성을 방지한다.

## 2026-09-03 직원 급여명세서 페이지 구성

- 직원별 급여명세서는 1페이지를 급여 명세 전용으로 구성한다. 생산수당 금액이 있는 직원만 2페이지를 추가하고, 해당 직원의 생산수당 산출 내역 전체를 2페이지 표에 표시한다. 산출 항목 하나마다 페이지를 나누지 않는다.
- 인쇄 결과도 같은 페이지 구조를 사용해 생산수당 산출 내역을 새 페이지에서 시작한다. 상세 다이얼로그의 하단 `닫기`/`인쇄` 액션 영역은 제거하고, `인쇄` 버튼만 제목 행 오른쪽 끝에 배치한다.
- 급여명세서 인쇄 스타일은 A4 세로 기준의 컴팩트 레이아웃을 사용한다. 화면 미리보기 크기는 유지하고 인쇄물에서만 본문·표 글자, 셀 여백, 제목과 기본정보 간격을 줄여 생산수당 상세 행이 불필요하게 다음 장으로 넘어가지 않게 한다.
- 급여명세서 상세는 바깥 배경을 클릭하거나 Esc를 누르면 닫혀야 한다. 지급 항목은 급여 체계 화면의 분류 순서(`기본급 → 수당 → 성과급`)와 각 분류 안의 `sortOrder` 순서대로 화면과 인쇄물에 동일하게 표시한다.
- 생산수당이 있는 급여명세서의 `1 / 2` 페이지 이동 컨트롤은 상세 내용 위가 아니라 각 페이지 내용의 맨 아래에 배치한다.

## 2026-09-02 급여 계산의 공장별 완전 독립

- 급여 계산은 조직 단위가 아니라 공장 단위로 완전히 독립해서 계산·잠금·해제·삭제한다. `PayrollSnapshot`의 유니크 키를 `[orgId, month]`에서 `[orgId, month, factoryId]`로 전환했고, 급여 조회·계산·재계산·단가 override·잠금·해제·삭제를 담당하는 `backend/src/payroll/payroll.service.ts`의 모든 함수가 `factoryId`를 필수 파라미터로 받는다. 한 공장의 계산·잠금 상태 변경이 다른 공장의 그 달 스냅샷에 전혀 영향을 주지 않는다.
- 이 마이그레이션은 조직에 공장이 정확히 1개일 때만 기존 스냅샷의 `factoryId`를 그 공장으로 자동 백필한다. 공장이 여러 개인 조직에 이미 애매한(공장 미지정) 스냅샷 행이 있다면 백필 대상에서 제외되고, 뒤이은 `NOT NULL` 제약이 실패해 배포가 명시적으로 멈춘다 — 침묵 속에 엉뚱한 공장으로 귀속시키지 않는다.
- 급여 계산 목록(`PayrollBoard.jsx`)은 월×공장 교차 행으로 표시하며, 일괄 계산 버튼도 선택한 월(또는 전체 월)의 모든 공장 행을 대상으로 개별 계산 요청을 보낸다. 급여 계산 상세(`PayrollEntry.jsx`)는 `factoryId`를 URL 쿼리파라미터로 받는 것을 필수 전제로 하며(예: `/payroll/2026-07?factoryId=1`), 이 값 없이 직접 진입하면 계산을 시도하지 않고 안내 메시지만 표시한다.
- 급여 계산 상세의 직원 목록 정렬 순서는 직원 관리 메뉴(`EmployeeBoard.jsx`)의 정렬 로직(조직 역할 → 현장 직무 역할의 `sortOrder`(`WORKER_SUPERVISOR`는 항상 최우선) → 사번 숫자 비교 → 이름 가나다순)을 그대로 재사용한다. 직원 관리에서 직원 순서를 바꾸면 급여 상세의 직원 순서도 동일하게 바뀐다.
- `GET /payroll/calendar`는 이제 조직의 공장 목록(`id/name/nameKo/nameVi/managementStartDateKey`)도 함께 반환한다. 프론트는 이 값으로 월×공장 행을 만들고 급여 계산 상세 화면 제목의 공장명도 여기서 가져온다.

## 2026-09-02 급여 명세서 중심 상세 구성

- 급여 계산 상세의 직원 목록에서는 생산수당 금액만 보여주고 별도의 `생산 상세` 확장 버튼과 직원별 생산 단가 편집 및 `단가 저장` 기능을 제공하지 않는다.
- 생산수당의 공정별 산출 내역은 직원의 월 급여명세서 안에서 확인하며, 인쇄물에도 포함한다.
- 월 급여명세서는 이름·급여 타입뿐 아니라 계산에 사용한 실제 근무일수, 기준 근무일수, 휴일 근무일수, 만근 여부, 근속 연수, 급여 체계 버전을 함께 표시한다.
- 급여명세서의 항목 표는 내부 카테고리 `BASE`/`ALLOWANCE`를 보여주는 `구분` 열 대신, 해당 항목에 저장된 계산식을 현지화된 파라미터 이름으로 표시한다.

## 2026-09-02 사무 직원 출퇴근 상시 만근 처리와 임시 백필 데이터 제거 (위 항목 대체)

- 위 "사무 직원 출퇴근 기록 일회성 백필" 항목은 폐기됐다. `migration_fix.sql`에 있던 사번 `0001`/`0020`/`0024` 대상 08:00~17:00 출퇴근 UPSERT 블록은 삭제했고, 그 블록이 만들었던 사번 `0001`/`0020`의 `AttendanceEntry` 133건도 DB에서 직접 삭제했다. 그 마이그레이션 블록을 지우지 않고 남겨뒀다면 다음 `migration_fix.sql` 전체 재적용 때마다 방금 삭제한 가짜 출퇴근 기록이 그대로 되살아났을 것이다 — 멱등 UPSERT를 지울 때는 그 UPSERT가 만든 데이터도 함께 정리해야 한다는 사례로 남긴다.
- 대신 `Employee.alwaysFullAttendance`(기본값 `false`) 설정을 추가했다. 이 값이 켜진 직원은 실제 출퇴근 기록 유무와 무관하게 급여 계산의 `실제 근무일수`를 `기준 근무일수`와 동일하게 취급해 정규 근무시간을 인정한다. 연장근무는 이 설정과 무관하게 실제 출퇴근 기록이 있어야만 인정된다.
- 직원 관리 화면에서 직원별로 이 설정을 켜고 끌 수 있다. 기본값은 반드시 해제 상태이며, 급여 계산 시 실제로 이 값을 판정에 반영해야 한다(단순 표시용 플래그로 그치면 안 된다).
...
2. 앞으로 할 일, 운영 검증, 보류된 재고·수익성 계획은 `todo.md`에서 관리한다. 사용자가 별도 환경 간 로드맵을 요청한 주문 완전 잠금·청구서는 `INVOICE_LOCK_ROADMAP.md`에 상세 상태를 두고 `todo.md`에서 연결한다.
...
- 현재 구현과 판단을 강제하는 확정 규칙은 `AGENTS.md`에, 앞으로 할 일·운영 검증·보류된 결정은 `todo.md`에 기록한다. 주문 완전 잠금·청구서 상세 진행 상태는 사용자 요청에 따라 `INVOICE_LOCK_ROADMAP.md`에서 관리한다. 단순 질문·일회성 조사 과정·확정되지 않은 제안은 확정 규칙처럼 기록하지 않는다.
...
### 2026-09-03 단가 관리 화면 CMT/FP 구분 제거, 버킷·단가 통합 저장

- `CustomerPricingBoard.jsx`(단가 관리 화면)는 매출 단가만 다루는 화면이고, 사용자 판단에 따라 이 화면에서는 판매방식(CMT·임가공 / FP·완제품) 구분이 청구 방식에 영향을 주지 않으므로 화면에서 판매방식 토글을 제거했다. 프론트는 이제 조회·저장 모두 상수 `SALES_PRICING_BASIS = 'MANUFACTURING_SERVICE_PRICE'`(기존 토글의 기본값과 동일) 하나만 사용한다.
- **DB 스키마·API는 바꾸지 않았다.** `SalesPricingBasis` enum과 `CustomerSalesPriceList.pricingBasis` 컬럼은 그대로 존재하며, `/customers/:id/sales-prices` GET/PUT은 여전히 `pricingBasis`를 필수로 받는다 — 프론트가 항상 같은 값 하나만 보내도록 고정했을 뿐이다.
- **알려진 제약**: 이 변경 이전에 어떤 고객·스타일이 `FINISHED_GOODS_PRICE`(FP)로 저장한 단가가 있었다면, 그 값은 DB에 그대로 남아있지만 이 화면에서는 더 이상 조회·수정할 수 없다(항상 CMT만 조회). 데이터가 삭제되지는 않지만 화면에서 보이지 않게 된다. 실제 운영 데이터에 FP 단가가 있는지는 이번 변경에서 확인하지 않았다 — 필요하면 `CustomerSalesPriceList.pricingBasis = 'FINISHED_GOODS_PRICE'` 행 존재 여부를 조회해 확인해야 한다.
- 수량 버킷 저장(`PUT /customers/:id/quantity-buckets`)과 단가 저장(`PUT /customers/:id/sales-prices`)은 여전히 별도 API 호출이지만, 프론트 상단 저장 버튼 하나(`handleSaveAll`)가 순서를 보장하며 함께 호출한다 — 버킷 변경이 있으면 먼저 저장(확인 다이얼로그 포함)하고 성공한 뒤에만 단가 변경을 저장한다. 단가 저장이 새로 추가된 버킷 수량을 참조하려면 그 버킷이 이미 저장된 버전에 존재해야 하므로 이 순서가 필수다.
- `적용 대상`(버킷 타겟 선택)과 `버킷 방식`(프리셋 선택) 셀렉트는 테이블 헤더에서 상단 필터 행(선택 고객/스타일 수/기본 통화 칩이 있는 행)으로 옮기고 오른쪽 정렬했다. 스타일별 버킷 타겟을 선택했을 때만 보이던 `고객 기본값 사용/이 스타일만 별도 설정` 토글, 스타일 통화 선택, 안내 문구는 테이블 헤더에 그대로 남겨뒀다(적용 대상과 강하게 연동되는 컨텍스트 컨트롤이라 위치를 유지).
- 상단에 항상 떠 있던 안내 `Alert` 배너는 제거하고, 페이지 제목 옆 정보 아이콘(클릭 시 MUI `Popover`로 같은 안내문 표시)으로 대체했다.
