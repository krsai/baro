...
- 사후에 명부를 정리하는 경우를 위해 라인 화면은 기본적으로 퇴사자를 숨기되 `퇴사자 포함`을 켜면 입사일·퇴사일이 있고 라인 이력이 전혀 없는 과거 작업자를 별도 후보로 표시한다. 이 직원을 라인에 드롭하면 입사일~퇴사일을 기본 기간으로 하는 종료된 과거 이력만 생성하며 현재 라인 인원에는 포함하지 않는다. 퇴사 상태 전환이 늦었더라도 퇴사일이 지난 `WORKER`는 같은 후보 규칙을 적용한다.
