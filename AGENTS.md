...
- 신규 파라미터 `PRODUCTION_ST_EXCESS_RATIO`를 계산식에 추가했다. 정의: 그 직원이 그 달 실제로 생산한 수량의 ST초 합계(`AssignmentPlan.assignmentStSnapshot.processes[]`의 `styleProcessId`별 `stSeconds` × 그 공정 실적 수량을 WorkRecord 기준으로 합산, CT/생산수당과는 별개의 값)가, 생산(OUTPUT) 급여 타입의 그 달 기준 근무일수(일요일·등록 휴일 제외, 개인 출퇴근 실적과 무관한 공장 공통 고정값) × 1일 기준 근무시간(급여 계산 설정의 근로 기준일 값)을 초과한 비율이다. 미달이면 0으로 클램프한다(초과분에 대한 보너스이며 페널티가 아니다). 이 정의는 사용자가 명시적으로 확정한 것이다: "①연속 비례 지급(임계값·구간 없음), ②기준 근무시간은 공장 공통 고정값이며 그 달의 실제 근무 일수(휴일 제외) 기준".
- 예: `GRADE_RATE × PRODUCTION_ST_EXCESS_RATIO`로 새 성과급 항목을 만들면, 그 직급의 단가(기준액)에 초과율을 곱한 만큼 지급된다(기존 `만근수당 = GRADE_RATE × FULL_ATTENDANCE_FACTOR`, `근속수당 = GRADE_RATE × TENURE_YEARS`와 동일한 패턴). 이 항목의 급여 타입은 다른 성과급 항목과 마찬가지로 항상 `OUTPUT` 전용, 정산 주기는 항상 매월로 서버가 강제한다. 상한값(선택)은 다른 일반 항목처럼 설정할 수 있다(고정 `생산수당` 항목만 상한을 둘 수 없다).
