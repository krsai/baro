...
## 2026-09-13 주문 완전 잠금 재설계 방향 및 환경 간 로드맵

- 사용자는 기존 주문 잠금을 완전 잠금으로 전환하고, 해제 자격을 메뉴 접근 정책과 분리해 조직 관리자(`Employee.orgRole = ADMIN`)로 제한하는 방향을 요청했다. 현장 직무/표시명이나 메뉴 권한만으로 해제를 허용하지 않는다. 현재 코드는 아직 이 방향으로 변경되지 않았다.
- 현 잠금은 주문 수정·삭제 차단과 배정 카드/수량 동기화를 겸한다. 이를 전체 동결로 바꿀 때는 생산 시작이 막히지 않도록 배정 준비와 완전 잠금의 역할을 분리해야 한다. 트리거·수동 잠금 시점·공유 주문의 해제 가능 조직 범위는 아직 미확정이다.
- 이전의 별도 해제 권한 설정 계획은 이번 조직 ADMIN 전용 방향으로 대체한다. 정식 발행에 따른 잠금과 발행본 보존 목표는 유지한다. 구 잠금 데이터를 일괄 새 완전 잠금으로 해석하지 않는다.
