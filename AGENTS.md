...
- 원본 관측 장당 시간은 관계형 관측행에 그대로 보존하되 운영 AT는 생산수량 가중 최소제곱으로 `AT(q)=a+b/q`를 적합하고 `a>0`, `b>=0`을 강제해 수량 증가 시 장당 AT가 증가하지 않게 한다. 관측점이 둘 이상이면 제약 곡선의 관측 수량별 값을 기준점으로 사용하고 그 사이는 총시간이 증가하는 구간만 보간한다. 관측 수량이 하나뿐이면 곡선을 만들지 않고 기존 provisional 정책을 유지한다.
