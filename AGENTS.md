...
- 선택하지 않은 급여 타입의 직급별 단가 입력은 화면에서 비활성화하고, 저장 요청에서도 해당 항목·급여 타입 조합의 단가를 제외한다. 서버도 항목의 `payTypes`를 강제로 양쪽 타입으로 덮어쓰지 않고 검증 후 그대로 보존한다.
- 급여 화면의 상태·분류 칩은 임의의 초록/주황 조합 대신 공용 테마의 primary(`#0066cc`)와 중립 outlined 상태를 사용한다.
