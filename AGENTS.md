...
- 생산수당 월 계산은 서버가 `Σ(WorkRecord.quantity × WorkRecord.ctSeconds × 계산 시점 Factory.wagePerSecond)`로 다시 계산한다. 월중 공장 단가를 변경하면 그 달 1일부터 말일까지의 모든 작업기록에 계산 시점의 최신 공장 단가를 적용하며 과거 `WorkLog.factoryWagePerSecond`는 생산수당 계산 근거로 사용하지 않는다. 클라이언트가 보낸 생산수당 합계를 신뢰하지 않는다.
...
- 공장 생성 트랜잭션은 수정 가능한 이름 `창고 1`의 활성 기본 창고를 함께 생성한다. 기존 공장도 같은 기본 창고를 백필한다. 공장에는 활성 기본 창고가 정확히 하나 있어야 하며 기본 창고를 비활성화하기 전 다른 활성 창고를 기본으로 지정해야 한다.
- 창고 이동은 단순 입고·출고 문자열이 아니라 하나의 이동 문서가 `sourceWarehouseId`와 `destinationWarehouseId`를 각각 정확한 Warehouse FK로 참조하고 양쪽 원장을 함께 생성하는 구조로 구현한다.
