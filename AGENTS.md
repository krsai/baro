...
- 메인 목록은 계산본 유무와 무관하게 작업기록이 있는 월을 `월 1행`으로 유지한다. 계산은 해당 월의 잠금 해제 계산본을 만들고, 사용자가 별도 스위치로 잠근다. 잠금 해제는 계산본을 보존하며 삭제는 해당 월 계산본만 제거해 월 행을 미계산 상태로 되돌린다. 다른 월 계산본이나 작업기록은 변경하지 않는다. 잠금과 삭제 UI는 주문 목록의 공통 스위치·휴지통 패턴을 사용하며 잠긴 월은 해제 후에만 삭제한다. 월 행을 선택하면 해당 월 전체의 공장·라인·직원·공정별 상세를 연다.
