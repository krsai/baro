...
- 작업일 라인 이력, 현재 활성 라인 배정, `Employee.lineId`가 모두 비어 있어도 직원의 공장이 확정되어 있고 그 공장에 라인이 정확히 하나뿐이면 Excel 과거 작업기록은 그 유일한 라인을 복구 근거로 사용할 수 있다. 공장에 라인이 둘 이상이면 이 fallback을 사용하지 않는다.
- 생산 메뉴의 라인 배지는 라인 화면에서 실제로 배정할 수 있는 활성 봉제 작업자만 센다. 공장 미지정 작업자는 특정 공장의 라인 화면에 나타나지 않으므로 라인 미배정 배지에서 제외하고 직원 화면의 공장 지정 대상으로 관리한다. 공장이 지정된 다른 공장의 미배정 작업자는 조직 전체 라인 배지에 포함한다.

### 2026-08-05 AT 안정 배분 운영 모델

- 기존 `v2` 관측은 감사·비교용으로 삭제하지 않는다. 신규 운영 관측은 `v3-st-stable` 모델 버전으로 별도 생성하며 화면과 운영 AT는 이 버전만 조회한다.
...
- v1 `StyleProcess.atParams`와 v2 관측은 과거 데이터 감사·비교 목적으로 보존한다. 운영 AT 계산·표시·신뢰도 판정은 `v3-st-stable` 관측 모델을 사용한다.
