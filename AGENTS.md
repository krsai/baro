...
- 현재 운영 AT `v3-st-stable`은 `WorkLog × Worker` 노동시간을 그 안의 모든 공정에 함께 ST 비례 배분하고, 원천 조회와 배분 키에 `productionStage`가 없으며 봉제 직무 직원만 학습 대상으로 허용한다. 따라서 다림질 AT를 올바르게 추정하는 구현으로 간주하지 않고, 다림질 작업기록을 운영 학습에 포함하기 전에 단계 인식 모델 `v4-stage-aware`를 shadow 방식으로 구현·검증한다.
...
- 기존 봉제-only 데이터와 `v3-st-stable` 관측·파라미터는 보존한다. `v4-stage-aware`는 별도 모델 버전으로 생성하고 봉제-only 표본의 허용 오차, 단계 간 노동시간 교차 배분 0건, 다림질 직접 측정 표본 대조를 통과한 뒤 운영 표시를 전환한다.
