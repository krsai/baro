...
- 생산수당 확정 후 같은 월의 작업기록 또는 출퇴근 기록이 추가·수정되면 기존 확정 결과를 자동 변경하지 않고 `재계산 필요`로 표시한다. 원본 기록이 다시 완성된 상태에서 관리자가 재계산해야 최신 생산수당 결과로 교체한다.
...
- 작업기록 준비 여부는 `WorkRecord.lineId`와 레코드/작업로그의 유효 coverage 기간으로 근무일별 확인한다. 출퇴근 준비 여부는 해당 공장·성과급 직원·근무일별 `AttendanceEntry` 존재 여부로 확인한다. 마지막 날짜만 비교하지 않고 중간 누락도 계산을 차단한다.
- 공장·라인별 목록에는 작업기록 일수, 필수 출퇴근 건수, 대상 인원, 계산 준비/미완료/확정 상태를 표시한다. 월 전체가 준비된 뒤 계산하며 확정 행을 선택하면 직원별·공정별 상세를 연다.

## 2026-08-02 생산수당 계산 범위

- 급여 체계 전체는 회사 정책이 확정될 때까지 구현을 미룬다. 현재 운영 기능은 `급여 계산`이 아니라 `생산수당 계산`이며 성과급 직원의 생산수당만 계산·확정한다.
- 생산수당은 서버가 작업기록에서 `Σ(WorkRecord.quantity × WorkRecord.ctSeconds × 작업 당시 WorkLog.factoryWagePerSecond)`로 다시 계산한다. 클라이언트가 보낸 기본급·수당·보너스·공제·최종급여 또는 생산수당 금액을 저장 근거로 신뢰하지 않는다.
...
- 생산수당 계산·확정은 출고 수량 정산 완료 여부에 의존하지 않는다. 생산수당의 근거는 성과급 직원의 확정된 작업기록 수량·CT·작업 당시 공장 단가이며, 숨겨진 수량 정산 화면의 검토·차단 상태로 생산수당 저장을 409 차단하지 않는다.
- 생산수당 화면은 공장 선택 없이 선택 월의 모든 공장 작업기록을 함께 계산한다. 목록 화면의 월 필터는 관리 시작 월부터 현재 월까지 허용하며 작업기록이 있는 미계산 월에만 계산 버튼을 활성화한다. 종료된 월은 월 단위 확정 스냅샷으로 저장하고 재계산하려면 기존 결과를 먼저 삭제한다. 현재 월은 `진행 중` 스냅샷으로 저장하며 버튼을 다시 누를 때 기존 스냅샷을 계산 근거로 재사용하지 않고 최신 작업기록에서 전부 다시 계산해 덮어쓴다. 월이 종료된 뒤 다시 계산하면 같은 월 행을 최종 확정한다.
- 생산수당 계산은 목록 화면에서 실행하며 별도 `/payroll/new` 입력 화면으로 이동하지 않는다. 계산 후 생성된 월 행을 한 번 선택하면 `/payroll/:month` 읽기 전용 직원별·공정별 상세 화면을 연다.
- `PayrollSnapshot.isProvisional`은 현재 월 진행 중 계산인지 종료 월 확정 결과인지 나타내는 관계형 상태다. 현재 월 여부를 조회 시점의 달력과 비교해 과거 진행 중 행을 확정으로 추정하지 않는다. 월이 넘어가도 진행 중 행은 그대로 남고 목록 계산 버튼으로 최신 작업기록에서 다시 계산한 뒤에만 `isProvisional=false`로 확정한다.

## 2026-08-01 재고 개발 전 확정 사항

- 아직 재고 기능을 구현하지 않는다. 재고 업무 규칙이 확정될 때까지 논의와 문서 정리만 하며, 사용자가 제공할 현행 재고표와 BOM 조사 결과를 확인한 뒤 각 개발 단계 직전에 다시 논의한다.
...
- 기존 의류 종류의 `카테고리`와 혼동하지 않도록 재고 화면에서는 `자재 분류`라는 용어를 사용한다. 자재 분류는 검색·필터·집계용이고 실제 재고 계산은 정확한 자재 품목을 기준으로 한다. 분류 계층 깊이와 관리자 편집 정책은 현행 재고표 확인 후 확정한다.
...
- 입출고 원장은 매입 입고, 고객 지급 입고, 창고 이동, 생산 투입, 생산 반환, 고객 반환, 폐기·손실, 실사 조정을 지원하는 방향으로 검토한다. 오입력은 원본 삭제가 아니라 반대 거래와 정정 이력으로 처리하고, 실사 차이 조정은 관리자 승인 후 반영한다.
...
- 정확한 관측 수량은 관측 장당 시간을 그대로 사용한다. 서로 다른 실제 수량의 관측점 사이는 총시간이 수량 증가에 따라 증가하는 한 인접 관측 총시간 사이에서 보간하며, 이때 `b < 0`만을 이유로 안전한 보간값을 숨기지 않는다.
...
- 개인별 생산수당은 해당 직원의 작업기록을 근거로 `Σ(WorkRecord.quantity × WorkRecord.ctSeconds × 작업 당시 적용되는 해당 공장의 생산수당 초당 단가)`로 계산한다. CT는 전체 급여가 아니라 생산수당만 계산하는 시간 기준이다.
