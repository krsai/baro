...
- BARO 조직의 사번 `0001`, `0020`, `0024`는 각 입사일과 `2026-04-01` 중 늦은 날부터 `2026-09-02`까지 사무(고정) 정책의 근무 요일에 정시 출근·퇴근한 기록을 일회성 마이그레이션으로 생성한다.
- 조직 휴일은 제외하고, 근무시간은 출퇴근 시각 차이에서 휴게시간을 뺀 값으로 저장한다. 같은 날짜의 기존 출퇴근 기록은 덮어쓰지 않는다.
