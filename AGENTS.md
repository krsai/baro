...
- `/invoices`는 고객 선택 → 해당 고객 주문 선택 → 주문·생산 정보 기반 청구 내역 작성 순서다. `/invoices/customers`는 현재 판매자 공장 주문의 고객만 반환하고 `/invoices/orders`는 `buyerOrgId`를 필수로 검사한다. INVOICE 권한을 유지하며 주문 원본 API에도 선택 고객 범위를 전달한다. 아직 검토용 초안이고 정식 발행/잠금은 미구현이다.
