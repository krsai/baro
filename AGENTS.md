...
## 2026-09-02 버전 관리 목록 정렬

- 급여 체계, 공장 생산수당 단가, 스타일 공정 등 모든 버전 관리 화면의 왼쪽 버전 목록은 최신 버전(가장 큰 버전 번호)을 위에 표시한다.
- 적용 구간 계산과 색상 배정에 사용하는 내부 시간순 데이터는 그대로 유지하고, 화면에 렌더링하는 목록만 내림차순으로 정렬한다.

## 2026-09-02 급여 수동 재계산 1회 허용

- 급여 계산 목록에서는 서버가 변경을 감지하지 않았더라도 잠금되지 않은 기존 계산 월을 화면 진입 후 한 번 수동 재계산할 수 있다. 성공한 월은 같은 화면 세션에서 다시 일괄 계산 대상에 포함하지 않는다.
- 급여 체계나 원본 기록 변경으로 서버가 `needsRecalculation`을 반환한 월은 수동 재계산 이력과 관계없이 다시 계산 대상으로 활성화한다.

## 2026-09-02 급여 재계산 상태와 버튼

- 기존 급여 스냅샷의 `재계산 필요`는 저장된 결과와 현재 산출 결과가 달라졌을 때만 표시한다. 입력 완성도는 최초 계산 가능 여부이며, 이미 존재하는 과거 스냅샷을 재계산한 직후에도 입력 완성도만을 이유로 다시 `재계산 필요`로 만들지 않는다.
- 상단 일괄 계산 버튼은 미계산이면서 준비 완료인 월 또는 잠금 해제 상태에서 실제 변경이 감지된 월만 대상으로 한다. 재계산 직후 변경이 해소되면 버튼은 비활성화한다.
- 급여 계산 성공·조회 실패 알림은 생산수당이 아니라 통합 급여 계산으로 표현한다.

## 2026-09-02 급여 타입 3종

- 급여 타입은 `GENERAL`=사무(고정), `OUTPUT_FIXED`=생산(고정), `OUTPUT`=생산(변동)의 세 종류이며 표시 순서도 이 순서를 사용한다. 기존 `GENERAL`/`OUTPUT` 직원과 이력은 각각 사무(고정)/생산(변동)의 의미를 그대로 승계한다.
- 생산(고정)은 기본 근무 기준을 기존 생산(변동)에서 복제해 시작하지만 직원 체계에서 독립적으로 수정한다. 일반 급여 항목은 세 타입에 적용할 수 있고, 작업 실적 기반 생산수당·성과급은 생산(변동)에만 적용한다.
- 급여 타입 이름과 선택지는 직원 관리·직원 체계·급여 체계·급여 계산·급여명세서에서 공용 라벨을 사용해 한국어·영어·베트남어로 표시한다.
- 기존 생산(변동)의 근무 정책과 비성과급 직급 단가는 생산(고정)으로 백필한다. 과거 급여 체계 버전에 생산(고정)이 없을 때도 비성과급 항목과 단가는 기존 `OUTPUT` 값을 호환 기준으로 사용할 수 있다.

## 2026-09-01 제조사 직원의 실제 공장 소속 필수화

- 제조사 조직의 모든 직원은 권한이나 직무와 관계없이 실제 공장 하나에 반드시 소속한다. 공장 미지정 직원을 `운영 지원팀`이라는 가상 소속으로 분류하거나 필터링하지 않는다.
- 직원 추가·수정 및 가입 승인에서 공장 선택을 필수로 검증한다. 공장이 하나인 기존 조직의 공장 미지정 직원은 배포 마이그레이션으로 그 유일한 공장에 귀속한다. 공장이 여러 개인 조직의 기존 미지정 직원은 임의 배정하지 않으며 직원 관리에서 명시적으로 공장을 지정해야 한다.
- 브랜드 조직에는 공장 개념이 없으므로 기존처럼 `factoryId = null`을 사용한다. `Employee.factoryId` 컬럼 자체는 브랜드 직원을 위해 nullable로 유지한다.
- 직원 목록에는 실제 소속 공장 칼럼을 표시한다. 기존 공장 미지정 직원은 재직·휴직·퇴사·승인 상태와 관계없이 모두 같은 조직의 하노이 공장(우선 `factoryCode=HN`, 보조적으로 `HANOI`/`하노이` 명칭)에 일괄 귀속한다.

## 2026-09-01 급여 계산 월 노출 및 단일 공장 귀속

- 급여 계산 목록에는 공장 관리 시작일부터 지난달까지의 모든 달을 무조건 표시하지 않는다. 출퇴근·작업 기록 준비 상태가 완료된 달 또는 이미 급여 스냅샷이 존재하는 달만 표시한다. 아직 기록이 준비되지 않았고 계산 이력도 없는 달은 목록에서 숨긴다.
- 급여 계산과 준비 상태 판정에서 조직의 공장이 정확히 하나라면 공장이 지정되지 않은 일반 직원도 그 유일한 공장에 귀속된 것으로 처리해 해당 공장의 급여 체계 버전과 근무 기준을 적용한다.
- 공장이 둘 이상인 조직에서는 공장 미지정 직원을 임의 배정하지 않는다. 급여 계산을 오류로 중단하고 직원 정보에서 소속 공장을 명시하도록 안내한다. 출퇴근 기록도 계산 대상 공장과 다른 공장에 중복 또는 오기록되어 있으면 계산하지 않는다.

## 2026-09-01 월 급여 스냅샷 무결성

- 잠긴 월 급여는 전체 재계산·라인 재계산·직원별 생산 단가 변경·삭제로 덮어쓸 수 없다. 모든 스냅샷 변경은 `revision`을 사용한 조건부 갱신으로 동시 요청 중 하나만 성공시킨다.
- 임시 급여를 전체 재계산해도 `rateOverridden` 직원의 수동 생산 단가는 보존한다. 라인 재계산은 일반 급여 직원을 스냅샷에서 제거하지 않으며, 생산수당 변경 후 급여 항목·총 급여도 함께 다시 계산한다.
- 직원의 출퇴근 기록은 해당 직원의 소속 공장 한 곳에만 있어야 한다. 대상 월에 다른 공장 기록이 있으면 준비 상태에 오류로 표시하고 급여 계산은 409로 중단한다.
- 확정 급여 버전의 스냅샷에 `items` 또는 `rates`가 없으면 현재 라이브 항목으로 대체하지 않고 계산을 중단한다. 과거 급여 상세는 현재 직원 명부를 합치지 않고 저장된 스냅샷 직원만 표시하며, 월 합계는 통화별로 분리한다.
- 급여 계산 행에는 개별 계산·재계산 버튼을 두지 않는다. 상단 `일괄 계산`은 현재 월 필터 범위에서 준비가 완료된 미계산 월과 이미 스냅샷이 존재하는 잠금 해제 월을 모두 계산하며, 잠긴 월은 제외한다. 기존 스냅샷 재계산은 과거 데이터 전환을 위해 현재 완결성 조건을 다시 요구하지 않지만 공장 불일치 같은 무결성 오류는 계속 차단한다. 한 월이 실패해도 나머지 대상 월은 계속 처리한다.
- 일괄 급여 계산 오류는 짧은 토스트 원문으로만 표시하지 않는다. 별도 오류 다이얼로그에서 성공 월과 실패 월을 구분하고, 실패 월마다 직원명·원인·수정할 메뉴를 사용자가 이해할 수 있는 문장으로 안내한다.

## 2026-09-01 급여 타입별 근무 기준

- 직원 체계는 별도 메뉴를 늘리지 않고 `직급 체계`와 `급여 타입` 두 영역으로 관리한다. `근무 급여 규칙` 같은 별도 영역은 현재 만들지 않는다.
- 조직별 `GENERAL`/`OUTPUT` 급여 타입에 근무 요일, 기준 출퇴근 시간, 휴게시간, 근무일 인정 기준을 저장한다. 기본값은 일반 월~금, 생산 월~토, 08:00~17:00, 휴게 60분, 근무일 인정 4시간이다.
- 급여 계산 준비 상태의 필수 출퇴근 날짜, 실제/기준/휴일 근무일수, 정규·연장·휴일 근무시간은 모두 같은 급여 타입 설정을 사용한다. 1일 정규근무 기준은 `기준 퇴근 - 기준 출근 - 휴게시간`으로 계산하고 이를 초과한 시간부터 연장근무로 집계한다.
- 휴일 메뉴에 등록된 날짜는 급여 타입의 근무 요일보다 우선해 휴일로 처리한다. 급여 타입 설정 변경은 급여 계산 화면이 재조회하도록 `SALARY_SYSTEM_SETTINGS` 변경 이벤트를 함께 발행한다.

## 2026-09-01 통합 월 급여 계산 Phase 3 연결

- 기존 `PayrollSnapshot.data` JSON을 확장해 생산수당 공정 내역과 함께 직원별 급여 체계 버전, 계산 파라미터, 개별 급여 항목 금액, 총 급여, 공제, 실지급액, 계산 서명을 월 스냅샷으로 저장한다. 별도 테이블을 추가하지 않으며 과거 생산수당 전용 스냅샷은 정규화 계층에서 계속 읽되 재계산 대상으로 판정한다.
- 계산 대상 직원은 해당 월의 입사·퇴사 범위에 포함되는 일반/생산 타입 전체 직원이다. 직원 공장의 대상 월 적용 `SalarySystemVersion` 스냅샷을 선택하고, 직원 `payType + gradeId`에 맞는 항목별 단가를 `GRADE_RATE`로 사용한다. 공장, 적용 버전, 적용 항목의 직급 단가가 없으면 0원으로 묵인하지 않고 계산을 중단한다.
- 근태 파라미터는 `ACTUAL_WORKDAYS`, `SCHEDULED_WORKDAYS`, `HOLIDAY_WORKDAYS`, `FULL_ATTENDANCE_FACTOR`와 함께 `WORK_HOURS`, `OVERTIME_HOURS`, `HOLIDAY_HOURS`, `TENURE_YEARS`를 산출한다. 평일 하루 8시간까지는 정규시간, 초과분은 연장시간, 급여 타입별 주휴일 및 등록 휴일의 근무시간은 휴일시간으로 분리한다.
- 적용 급여 타입과 `paymentMonths`에 맞는 항목만 제한형 수식 평가기로 계산하고 상한을 적용한다. `PRODUCTION_ALLOWANCE`는 OUTPUT 직원의 생산수당 항목에만 들어간다. 현재 공제 정책은 정의되지 않았으므로 `deductions=0`, `netSalary=grossSalary`로 스냅샷하되 필드는 분리해 후속 공제 기능을 수용한다.
- 계산 서명에는 직원·공장·직급·급여 타입·급여 체계 버전·항목 결과·근태 파라미터를 포함한다. 현재 다시 계산한 서명과 저장본이 다르면 `재계산 필요`로 표시하고, 재계산은 월 전체 스냅샷을 다시 만든다. 재계산이 필요한 결과는 잠글 수 없다.
- 공장 관리 시작 월부터 현재 월까지 월 행을 연속 생성한다. 급여 계산 버튼은 완료된 과거 월 중 대상 직원의 출퇴근 기록과 생산 작업 기록이 모두 준비된 월에만 활성화한다. 준비도 또는 급여 설정 검사 하나가 실패해도 월 목록과 기존 계산본 전체를 숨기지 않고 해당 월만 비활성 상태로 유지한다. 기존 생산/근태 준비도 검증이 끝나지 않았거나 재계산이 필요하면 잠글 수 없다.
- 한 달치 기록 완결성은 달력의 1일과 말일에 기록이 존재하는지가 아니라 직원별 의무 근무일 집합으로 판단한다. 월초·월말을 포함해 급여 타입별 주휴일 또는 등록 휴일이면 출퇴근·작업 기록이 없어도 누락이 아니며, 휴일에 실제 근무한 경우 기록이 존재할 수 있다. 입사 전·퇴사 후·휴직 기간·공장 관리 시작일 전도 필수 기록일에서 제외한다. 출퇴근 완결성은 일반/생산 전체 직원, 작업 기록 완결성은 생산 직원 기준으로 검사한다.
- 월 목록의 일반 급여·생산 급여·총 급여는 스냅샷의 `grossSalary`를 급여 타입별로 합산한다. 직원 상세와 인쇄 명세서는 현재 급여 체계를 다시 조합하지 않고 계산 당시 스냅샷의 개별 `salaryItems`만 사용해 과거 명세서가 이후 정책 변경으로 달라지지 않게 한다.

## 2026-08-31 통합 급여 계산 UI 시안

- 기존 `생산수당` 메뉴의 화면 진입 명칭을 `급여 계산`으로 바꾸고, 월별 급여 계산 화면에서 조직의 전체 직원을 함께 조회하는 UI 시안을 만든다. 생산수당은 별도 최종 급여가 아니라 `OUTPUT` 급여 타입 직원에게만 포함되는 급여 구성 항목이다.
- 직원별 월 급여 표는 기본급·일반 수당·생산수당·공제·실지급액을 같은 행에서 비교한다. 아직 Phase 3 계산 로직이 없는 금액은 임의 값이나 0원으로 확정하지 않고 `미계산`으로 표시하며, 현재 계산 가능한 생산수당만 기존 스냅샷 값을 보여준다.
- 생산 타입 직원의 생산수당 상세를 펼치면 기존 초당 단가 편집과 공정별 산출 근거를 확인할 수 있다. 일반 타입 직원은 생산수당을 `해당 없음`으로 표시한다.
- 각 직원 행에서 해당 월의 개인 급여명세서 미리보기를 열 수 있고, 별도 인쇄 창으로 출력할 수 있다. 현재는 UI 시안이며 급여 체계·근태·공제까지 연결되는 실제 월 급여 계산과 명세서 확정/배포는 후속 구현 범위다.
- 월 목록은 생산수당의 공장·라인·초당 단가 관리표가 아니라 `정산 월 / 대상 인원 / 총 지급액 / 생산수당 소계 / 계산 상태 / 확정` 중심의 통합 급여 실행 목록으로 표시한다. 생산수당 공정 근거는 상세 화면의 하위 정보다.
- 계산 상태는 잠금 상태와 분리해 `미계산 / 재계산 필요 / 계산 완료` 세 가지만 사용한다. 급여 조건이나 연결 기준이 바뀐 계산본은 `재계산 필요`가 되어야 하며, 잠금은 계산 완료본의 확정 여부만 표현한다.
- 개인 급여명세서는 기본급·수당을 카테고리 합계 한 줄로 뭉치지 않고, 해당 직원의 공장·급여 타입·지급 월에 적용되는 급여 체계의 개별 항목명을 행 단위로 표시하고 인쇄한다.
- 급여 계산 결과는 삭제하지 않는다. 계산 전에는 결과가 없고, 계산 근거가 바뀐 기존 결과는 `재계산 필요` 상태로 전환해 다시 계산하는 흐름을 사용하므로 월 목록의 삭제 액션을 제공하지 않는다.
- 월 목록의 인원은 `총원 (일반/생산)` 형식(예: `15 (5/10)`)으로 한 칼럼에 표시하고, 금액은 `일반 급여 / 생산 급여 / 총 급여`를 구분한다. 기존 생산수당 스냅샷 금액을 생산 타입의 전체 급여나 월 총 급여로 오인해 표시하지 않으며, 통합 계산 엔진이 산출하지 않은 금액은 `미계산`으로 둔다. 계산 결과를 고정하는 칼럼명은 `확정`이 아니라 실제 동작에 맞는 `잠금`을 사용한다.

## 2026-08-31 실제 급여 계산 상세 정책 정리 메모

- 급여 체계의 기본 구조는 설정했지만, 실제 월 급여 계산을 연결하기 전에 출퇴근·근무시간·페널티의 세부 판정 규칙을 추가로 정의해야 한다.
- 궁극적인 메뉴는 기존 독립 `생산 수당`이 아니라 상위 `급여 계산`이다. 급여 계산은 해당 조직·공장의 모든 소속 직원을 대상으로 기본급·수당·근태 관련 금액·공제·생산수당을 합산한다.
- 기존 생산수당의 작업수량·CT·공장 초당 단가·직원별 단가 조정·재계산 근거는 삭제하지 않고 급여 계산 내부의 하위 명세로 편입한다. `PRODUCTION_ALLOWANCE`는 `OUTPUT` 직원에게만 계산·합산하며 `GENERAL` 직원에게는 항상 `0`이다.
- 기존 `/payroll`과 `PayrollSnapshot`이 `OUTPUT` 직원만 필터링하고 생산수당을 `totalEarnings`로 저장하는 구조는 통합 급여 계산 단계에서 전체 직원 스냅샷으로 전환한다. 잠금 시 적용 급여 체계 버전, 통화, 파라미터 값, 항목별 계산 결과, 생산수당 상세와 최종 지급액을 함께 고정해야 한다.
- 근무 조건을 위한 별도 관리 메뉴는 만들지 않는다. 만근 조건은 만근수당, 초과근무 인정 조건은 초과근무수당, 특근 조건은 특근수당, 지각 페널티는 해당 공제 항목처럼 급여 체계의 관련 항목별 설정으로 관리한다.
- 기본 출퇴근 시각·휴게시간·하루 소정근로시간·근무 요일처럼 여러 급여 항목이 공통으로 참조하는 값은 급여 체계 화면 안의 공장별 공통 근무 기준으로 관리하고, 항목별 조건이 이를 참조하도록 한다. 공통 기준과 항목별 조건은 모두 해당 공장의 급여 체계 버전 스냅샷에 포함한다.
- 계산식의 `만근 조건 적용` 토글은 제거하고 `만근 여부`를 다른 값과 동일한 일반 파라미터로 제공한다. 각 파라미터 옆에는 작은 정보 아이콘과 툴팁으로 값의 정의·포함·제외 조건을 설명한다.
- `실제 근무일수`는 급여 타입별 정규 근무일 중 출퇴근 기록의 `workedSeconds`가 4시간 이상인 날짜만 센다. 주말 및 휴일 메뉴에 등록된 휴일의 근무는 실제 근무일수와 만근 판정에서 제외한다.
- `기준 근무일수`는 생산(`OUTPUT`)은 일요일과 등록 휴일을 제외하고, 일반(`GENERAL`)은 토·일요일과 등록 휴일을 제외한 해당 월의 날짜 수다.
- `휴일 근무일수` 파라미터를 추가한다. 급여 타입별 주말 또는 등록 휴일에 4시간 이상 근무한 날짜 수이며 특근수당 계산에 사용한다.
- `만근 여부`는 현재 실제 근무일수가 기준 근무일수 이상이면 `1`, 아니면 `0`으로 판정한다. 승인된 유급휴가·출장·교육 등의 예외 반영은 관련 데이터와 정책을 확정한 뒤 이 파라미터 판정기에 추가한다.
- 검토할 사례: 기본 근무가 점심시간 1시간을 제외한 08:00~17:00, 실근무 8시간인 경우, 정시보다 늦게 출근했더라도 당일 실근무 8시간을 채우면 하루 만근으로 인정할지 결정해야 한다.
- 검토할 사례: 하루 기준 근무시간 8시간을 넘긴 뒤 1분이라도 초과하면 전부 초과근무로 인정할지, 최소 인정 시간·절사/반올림 단위·사전 승인 여부를 둘지 결정해야 한다.
- 검토할 사례: 지각을 몇 회 누적했을 때 어떤 페널티를 적용할지, 지각 시간과 횟수 중 무엇을 기준으로 삼을지, 월별 초기화 및 예외 승인 규칙을 정해야 한다.
- 위 사례들은 아직 확정된 급여 규칙이 아니라 정책 결정이 필요한 항목이다. 구현 시 임의의 기준으로 확정하지 않는다.
- `생산 수당`은 독립된 최종 급여가 아니라 `급여 계산`의 하위 구성 항목이다. 출근일, 실제 근무시간, 초과근무시간, 특근시간 등 급여 입력 항목의 정의와 판정이 완료된 뒤 기본급·각종 수당·공제와 생산수당을 합산해 최종 급여를 계산한다.
- 최종 화면 구조도 `급여 계산`을 상위 흐름으로 두고 생산수당 계산·근거를 그 하위 상세로 연결하는 방향으로 정리한다.

## 2026-08-28 급여 체계 공장별 분리

- 급여 항목·직급별 단가·통화·`SalarySystemVersion`은 조직 공통이 아니라 `factoryId` 기준으로 독립 관리한다. 급여 체계 화면은 공장 수와 관계없이 공장 탭을 항상 표시하며, 공장 추가·삭제에 따라 탭도 자동으로 증감한다.
- `저장`과 `버전 관리`는 현재 선택한 공장에 적용되는 동작임을 명확히 하기 위해 공장 탭과 같은 행 안에 배치한다.
- 공장별 급여 버전 타임라인은 버전 최초 등록일이 아니라 해당 공장의 `managementStartDate`가 속한 월부터 현재 월까지 이어서 표시한다. 기간이 길면 타임라인 영역 내부 스크롤로 처리한다.
- 공장 `managementStartDate`는 작업기록·생산분석·배정 보드·생산수당 계산의 최초 시작 기준일이므로 다른 공장 정보나 생산수당 설정을 저장할 때 함께 덮어쓰지 않는다. 공장 생성 또는 사용자가 관리 시작일 자체를 명시적으로 바꾼 경우에만 수정 요청에 포함한다.
- 급여 항목 추가, 급여 체계 저장, 버전 적용 구간 저장처럼 작업 직후 잠깐 알리면 되는 성공·안내·작업 오류 메시지는 본문 `Alert` 배너로 쌓지 않고 공용 토스트로 표시한다. 화면 데이터 자체를 불러오지 못한 오류처럼 지속적으로 보여야 하는 상태만 본문 배너를 사용한다.
- 전환 마이그레이션은 기존 조직 단위 급여 항목·단가·버전·통화를 조직의 모든 기존 공장에 복제한 뒤 공장별 유니크 키와 FK를 적용한다. 이후 한 공장의 변경이 다른 공장 데이터에 섞이면 안 된다.
...
- 기본급·수당 항목은 `GENERAL`(일반), `OUTPUT`(생산) 적용 여부를 급여 체계 화면의 명시적인 체크박스로 선택·해제한다. 기존 지급 설정 배지를 눌러 선택하는 동작은 사용하지 않는다. 최소 한 타입은 반드시 선택해야 하며, 성과급은 기존 원칙대로 `OUTPUT` 전용 고정 항목이다.
- 체크박스는 오른쪽 상세 영역이 아니라 왼쪽 `급여 항목` 목록에서 각 아이템명 오른쪽에 배치한다. 목록 상단에 `항목명 | 일반 | 생산` 열 제목을 한 번만 표시하고 각 행은 두 급여 타입 열에 체크박스만 정렬한다. 정산 주기·지급 월·상한은 오른쪽 상세의 `정산 설정`에만 표시한다.
- 기본급·수당의 마지막으로 선택된 급여 타입 체크박스를 비활성화하지 않는다. 사용자가 마지막 타입을 해제하려 하면 값을 유지하고 `선택된 급여 타입이 없습니다.` 경고 토스트를 표시한다. 성과급의 `OUTPUT` 전용 체크박스만 고정 비활성 상태로 둔다.
...
- 급여 항목 목록의 선택 행은 진한 `primary.main` 단색 채움을 사용하지 않고, `primary.main` 12% 배경과 `primary.dark` 텍스트로 옅게 표시한다.
...
- 제조사의 `ADMIN`/`OPERATOR`/`ACCOUNTANT`/`WORKER`는 모두 실제 공장 하나에 소속해야 한다. `factoryId = null`인 운영 지원팀 개념은 더 이상 사용하지 않는다.
