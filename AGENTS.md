...
- **적용한 수정 (신규 배정 생성 시점에만, 기존 배정은 건드리지 않음)**: `PUT /assignment-board-state`에서 **새로 생성되는** `AssignmentPlan`(`createPlanRows`)에 한해, 저장 직전에 `validateNewAssignmentPlanCtSnapshotProcesses`(`backend/src/index.ts`, `toAssignmentPlanWriteData` 바로 아래)가 해당 스타일의 살아있는 `StyleProcess` 목록과 클라이언트가 보낸 스냅샷의 `processes[]`를 비교한다. 스타일에는 있는데 스냅샷엔 없는 공정 코드가 하나라도 있으면 저장 전체를 `409`로 거부하고 어떤 공정이 빠졌는지 알려준다. 조용히 불완전한 스냅샷을 저장하는 대신 "정확 계산 원칙"(계산 실패는 조용히 다른 값으로 대체하지 않고 드러낸다)에 맞춰 즉시 실패시키는 방식을 택했다.
  - 왜 신규 생성 시점만인가: §9/§11 잠금 원칙("assignment는 저장 시점 구성을 고정하며, 이후 스타일이 바뀌어도 자동 갱신하지 않는다")을 그대로 지키기 위해서다. 이미 존재하는(과거에 얼어붙은) 배정까지 이 검증으로 막으면 그 원칙과 충돌한다.
  - 프론트(`AssignBoard.jsx`)의 `resolveBoardSaveErrorMessage`에 이 에러 메시지 매칭 분기를 추가해 "이 배정을 만든 뒤 스타일 공정이 바뀌었습니다. 화면을 새로고침하고 다시 시도해 주세요." 안내를 띄운다(ko/en/vi).
  - 검증: `npm --prefix backend run build`, `npm run test:regression`, `npm --prefix frontend run build` 통과.
- **이번에 고치지 않은 것 (알려진 한계)**:
  - 이미 잘못 저장된 기존 배정(L15-2/AJ1528 등)은 이 수정으로 자동 복구되지 않는다. 운영자가 그 배정을 다시 만들어야(취소 후 재배정 등) 새 검증을 통과한 완전한 스냅샷으로 교체된다.
  - `refreshUnlinkedAssignmentPlanSnapshotsForOrg`가 정확히 언제/무엇을 트리거로 도는지, 이번 케이스가 그 경로로 이미 커버됐어야 했는지는 미확인 상태로 남아있다.
  - 크로스탭 이벤트 전파(다른 탭의 스타일 편집을 열려있는 배정 보드 탭에 알리는 것) 자체는 고치지 않았다 — 이번 수정은 "불완전한 상태로 저장되는 것 자체를 막는" 서버 쪽 안전장치이지, 프론트가 항상 최신 데이터를 들고 있게 만드는 근본 수정은 아니다.
