...
- 헤더 인식(`detectHeaderColumns`)도 시각 열과 사번(ID) 열이 함께 있어야 유효한 파일로 인정한다. 이름 열만 있고 사번 열이 없는 파일은 더 이상 가져오기 대상으로 인정하지 않는다(이름 열은 선택 사항, 있으면 검증에만 쓰인다).
- 매칭 실패 항목은 더 이상 성공/실패 건수만 토스트로 보여주지 않는다. `buildAttendanceImportPlan`이 `unmatchedDetails`(사번·이름·시각·사유)를 반환하고, 미매칭이 하나라도 있으면 실제 저장 전에 상세 내역 표(사번/이름/시각/사유)를 보여주는 확인 다이얼로그를 먼저 띄운다. 사용자가 확인 후 "일치한 항목만 가져오기"를 눌러야 그 시점에 저장한다. 미매칭이 없으면 기존처럼 `window.confirm` 요약만 보여준다.
- 사번 접두어(조직 코드 `BRVN` 등, 위 "조직 코드 기반 사번 채번" 항목 참고)는 형태와 무관하게 사번 끝의 숫자만 뽑아 선행 0을 지우고 비교하므로, 출퇴근 기기가 내보내는 접두어 없는 순수 숫자 ID와도 정상 매칭된다.
