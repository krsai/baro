...
- 현재 구현된 공통 변경 주제는 `STYLES`, `ORDERS`, `ASSIGNMENT_BOARD`다. `useWorkspaceRefreshOnEvent`와 `AppContext.hasExternalChanges`로 활성 탭 자동 갱신 및 비활성 탭 변경점 표시의 공통 기반이 구현되어 있다. 스타일 목록·주문 목록/상세·배정·생산계획·QC·대시보드 일부가 이를 사용한다.
- 직원 저장과 라인 저장은 현재 각각 `baro:org-memberships-updated`, `baro:line-assignments-updated`라는 같은 브라우저 창 전용 이벤트로 좌측 메뉴의 승인 대기 및 봉제 라인 미배정 건수만 갱신한다. 직원/라인 화면 자체와 배정·작업기록·생산수당 등 연계 탭의 자동 갱신 또는 탭 변경 표시는 아직 공통 체계에 연결되지 않았다.
