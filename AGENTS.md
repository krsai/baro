...
- **이번에 손 안 댄 것**: "그룹 대표 행(anchor row) 뒤에 실제 문제 행이 숨는다"는 별도 이슈(§48 계열 CT 스냅샷 조사에서도 동일 패턴 확인됨, `groupAnchorRow`)는 이번 수정 범위 밖이다. 이번 케이스는 실제 중복 행 자체가 정확히 8행(4쌍)뿐이라 영향이 크지 않았지만, 그룹이 더 크거나 실제 중복이 여러 건이면 여전히 첫 그룹의 대표 행 하나만 보여주고 나머지를 숨길 수 있다.
