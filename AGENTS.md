...
- **운영 배포 시 필수**: pre-deploy가 꺼져 있으므로 Step 0n도 자동 적용되지 않는다. 배포 후 반드시 운영 DB에 직접 접속해 Step 0n SQL을 수동 실행하고, `information_schema.columns`로 4개 컬럼이 실제로 사라졌는지 직접 확인해야 한다.
