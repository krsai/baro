...
## 2026-09-02 사무 직원 출퇴근 기록 일회성 백필

- 정동원(`0001`)이 속한 BARO 조직의 사번 `0001`, `0020`, `0024`는 각 입사일과 `2026-04-01` 중 늦은 날부터 `2026-08-31`까지 월~금에 08:00 출근, 17:00 퇴근, 휴게시간 제외 8시간 근무 기록을 생성한다.
- 이 데이터 보정은 실제 배포 시 실행되는 `backend/migration_fix.sql`에 멱등 UPSERT로 둔다. 대상 기간의 기존 기록도 요청한 만근 기준으로 맞추며 토·일 기록은 만들지 않는다.
