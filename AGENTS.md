...
- 공장 생산수당 설정에는 단가 전용 최근 변경 시각 `productionAllowanceUpdatedAt`을 저장·표시한다. 주소·관리자 등 다른 공장 정보만 수정할 때는 이 시각을 바꾸지 않는다.
...
- 공장 생성 트랜잭션은 수정 가능한 이름 `기본 창고`의 활성 기본 창고를 함께 생성한다. 기존 공장도 창고가 전혀 없는 경우 같은 기본 창고를 백필한다. 공장에는 활성 기본 창고가 정확히 하나 있어야 하며 기본 창고를 비활성화하기 전 다른 활성 창고를 기본으로 지정해야 한다.
