...
- **계산 방식 UI 시안**: 급여 항목은 하나의 고정 드롭다운 조합 대신 `계산식 조합`/`직급별 정액`/`조건부 정액`/`외부 연동` 유형을 먼저 선택한다. `계산식 조합`은 자유 텍스트 수식이 아니라 사전 정의된 파라미터(직급별 단가, 실제/기준 근무일수, 근무·연장·특근시간, 근속연수 등), 제한된 연산자, 숫자 상수를 모달에서 순서대로 조립하는 방식이다. 이 계산식·유형은 2026-08-23 현재 UI 시안이며 아직 서버 저장이나 급여 계산에는 연결되지 않았다.
