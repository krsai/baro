...
- 메인 목록은 계산본 유무와 무관하게 작업기록이 있는 범위를 `정산 월 × Factory × Line` 행으로 구분한다. 월 필터의 기본값은 선택 없음(전체 월)이며 계산 대상을 제한하지 않고 목록만 필터한다. 계산 버튼은 필터와 무관하게 공장 관리 시작일 이후부터 직전 월까지 준비가 완료된 모든 미계산 월과 재계산 필요 월을 오래된 월부터 계산하며, 이 대상이 하나 이상일 때만 활성화한다. 잠금 해제 상태 자체는 재계산 사유가 아니며 계산 후 원천자료 또는 계산 결과가 달라졌을 때만 재계산 대상으로 본다. 계산본의 생성·잠금·해제·삭제 단위는 계속 월 전체이고, 잠긴 월은 해제 후에만 삭제한다. 라인 행을 선택하면 해당 월 전체의 공장·라인·직원·공정별 상세를 연다.
...
- 생산수당 화면은 공장 선택 없이 선택 월의 모든 공장 작업기록을 함께 계산한다. 월 필터는 관리 시작 월부터 직전월까지 허용하며 현재월 계산은 Ver1 범위에서 제외한다. 계산 근거가 바뀐 확정본은 `재계산 필요`로 표시하고, 확정 해제 후 최신 작업기록에서 다시 계산해 덮어쓴다. 계산본은 별도 확인 후 삭제할 수 있다.
