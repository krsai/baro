...
- 원본 관측 장당 시간은 관계형 관측행에 그대로 보존하되 운영 AT는 생산수량을 가중치로 한 단조 감소 회귀를 적용해 수량 증가 시 장당 AT가 증가하지 않게 한다. 역전된 인접 관측은 가중 평균으로 풀링하며, 서로 다른 실제 수량의 보정 관측점 사이는 총시간이 수량 증가에 따라 증가하는 구간만 보간한다.
