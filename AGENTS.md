...
- 자재 마스터는 별도 `자재 대분류` 없이 `자재 종류 → 자재 품목 → 색상·규격·세부 구분별 SKU` 관계로 관리한다. 자재 종류는 마스터에서 관리하고 품목 등록 시 필수 선택한다. 겉감과 `HS-SCW-90003`처럼 자재 종류와 그 종류에 속한 품목은 독립 문자열이 아니라 관계형 FK로 연결한다.
...
- 재고 메뉴와 화면 골격은 사용자 검토용 UI 시안으로 제공한다. `재고 현황·입출고·거래 내역·자재 관리·자재 설정` 메뉴는 열 수 있지만 샘플 데이터와 로컬 입력만 사용하며, 관계형 자재 마스터·재고 원장 API가 구현되기 전에는 운영 저장 기능으로 간주하지 않는다.
...
- 기존 의류 종류의 `카테고리`와 혼동하지 않도록 재고 화면에서는 필수 선택 마스터를 `자재 종류`라고 부른다. 별도 대분류나 계층형 분류는 두지 않으며 겉감·안감·일반 스냅·가시 스냅처럼 실제 업무에서 바로 선택할 값을 평면 목록으로 관리한다. 실제 재고 계산은 자재 종류가 아니라 정확한 품목/SKU를 기준으로 한다.
