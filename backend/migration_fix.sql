...
CREATE TABLE IF NOT EXISTS "OrgRelationshipStyleSalesCurrency" (
...
CREATE UNIQUE INDEX IF NOT EXISTS "OrgRelationshipStyleSalesCurrency_relationship_style_key"
  ON "OrgRelationshipStyleSalesCurrency"("orgRelationshipId", "styleId");
CREATE INDEX IF NOT EXISTS "OrgRelationshipStyleSalesCurrency_org_scope_idx"
  ON "OrgRelationshipStyleSalesCurrency"("manufacturerOrgId", "brandOrgId");
CREATE INDEX IF NOT EXISTS "OrgRelationshipStyleSalesCurrency_styleId_idx"
  ON "OrgRelationshipStyleSalesCurrency"("styleId");
CREATE INDEX IF NOT EXISTS "OrgRelationshipStyleSalesCurrency_currencyId_idx"
  ON "OrgRelationshipStyleSalesCurrency"("currencyId");
CREATE INDEX IF NOT EXISTS "OrgRelationship_salesCurrencyId_idx"
  ON "OrgRelationship"("salesCurrencyId");

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'OrgRelationship_salesCurrency_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'OrgRelationshipStyleSalesCurrency_relationship_scope_fkey') THEN
...
COMMIT;
-- AT v2 shadow observations: one row per assignment plan and style process.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'StyleProcessAtObservation_orgId_fkey'
  ) THEN
    ALTER TABLE "StyleProcessAtObservation"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'StyleProcessAtObservation_process_org_fkey'
  ) THEN
    ALTER TABLE "StyleProcessAtObservation"
...
-- AT observations are valid only inside the same organization as their
...
ALTER TABLE "AtTrainingBucket"
  ADD COLUMN IF NOT EXISTS "productionStage" "ProductionStage";
UPDATE "AtTrainingBucket" SET "productionStage" = 'SEWING'::"ProductionStage" WHERE "productionStage" IS NULL;
ALTER TABLE "AtTrainingBucket"
  ALTER COLUMN "productionStage" SET DEFAULT 'SEWING'::"ProductionStage",
  ALTER COLUMN "productionStage" SET NOT NULL;
...
FROM "StyleProcess" process
WHERE observation."styleProcessId" = process.id AND observation."productionStage" IS NULL;
UPDATE "StyleProcessAtObservation" SET "productionStage" = 'SEWING'::"ProductionStage" WHERE "productionStage" IS NULL;
ALTER TABLE "StyleProcessAtObservation"
  ALTER COLUMN "productionStage" SET DEFAULT 'SEWING'::"ProductionStage",
  ALTER COLUMN "productionStage" SET NOT NULL;
...
CREATE TABLE IF NOT EXISTS "EmployeeGradeSet" (
  ...
  "name" TEXT NOT NULL, "isActive" BOOLEAN NOT NULL DEFAULT true,
  "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS "EmployeeGrade" (
  "id" SERIAL PRIMARY KEY, "orgId" INTEGER NOT NULL, "setId" INTEGER NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeGrade_setId_sortOrder_key" ON "EmployeeGrade"("setId", "sortOrder");
CREATE INDEX IF NOT EXISTS "EmployeeGrade_orgId_isActive_sortOrder_idx" ON "EmployeeGrade"("orgId", "isActive", "sortOrder");
CREATE UNIQUE INDEX IF NOT EXISTS "EmployeeGrade_one_default_per_org_key" ON "EmployeeGrade"("orgId") WHERE "isDefault" = true;
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS "Employee_gradeId_idx" ON "Employee"("gradeId");
DO $$ BEGIN
...
SELECT
  ...
  row."factoryId",
  row."workerId",
  TO_CHAR(row."workDate", 'YYYY-MM-DD'),
  '08:00',
  '17:00',
  28800,
  'Office weekday full-attendance backfill through 2026-08-31',
  'system@baro.local',
  CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
...
ON CONFLICT ("orgId", "factoryId", "workerId", "workDate") DO UPDATE SET
  ...
  "workedSeconds" = EXCLUDED."workedSeconds",
  "note" = EXCLUDED."note",
  "updatedAt" = CURRENT_TIMESTAMP;
...
-- 2026-08-16: localized employee grade names
ALTER TABLE "EmployeeGrade"
  ADD COLUMN IF NOT EXISTS "nameKo" TEXT,
  ADD COLUMN IF NOT EXISTS "nameEn" TEXT,
  ADD COLUMN IF NOT EXISTS "nameVi" TEXT;
