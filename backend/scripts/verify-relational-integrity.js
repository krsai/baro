...
const checks = [
  ...
  ["OutsourcedWorkRecord canonical relation mismatch", `SELECT COUNT(*)::int AS count FROM "OutsourcedWorkRecord" owr JOIN "AssignmentPlan" ap ON ap.id=owr."assignmentPlanId" JOIN "Style" s ON s.id=owr."styleId" JOIN "StyleProcess" sp ON sp.id=owr."styleProcessId" JOIN "Organization" bp ON bp.id=owr."outsourcingPartnerId" WHERE owr."orgId"<>ap."orgId" OR owr."orgId"<>sp."orgId" OR ap."styleId" IS DISTINCT FROM owr."styleId" OR ap."styleId" IS DISTINCT FROM sp."styleId" OR sp."styleId"<>owr."styleId" OR s.id<>sp."styleId" OR owr."orgId"<>bp."ownerOrgId"`],
  ...
  ["Employee factory org mismatch", `SELECT COUNT(*)::int AS count FROM "Employee" e LEFT JOIN "Factory" f ON f.id=e."factoryId" WHERE (f.id IS NOT NULL AND f."orgId"<>e."orgId")`],
