...
const { assertApplicationDatabaseUrl } = require("../dist/config/databaseTarget");

assertApplicationDatabaseUrl(process.env.DATABASE_URL, "DATABASE_URL");
...
const checks = [
  ...
  ["WorkRecord org mismatch", `SELECT COUNT(*)::int AS count FROM "WorkRecord" wr JOIN "AssignmentPlan" ap ON ap.id=wr."assignmentPlanId" JOIN "Style" s ON s.id=wr."styleId" JOIN "StyleProcess" sp ON sp.id=wr."styleProcessId" WHERE wr."orgId"<>ap."orgId" OR wr."orgId"<>sp."orgId" OR sp."styleId"<>wr."styleId" OR s.id<>sp."styleId"`],
