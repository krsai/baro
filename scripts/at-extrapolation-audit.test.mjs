...
test('withholds the entire largest quantity and never leaks stored fitted parameters', () => {
  const process = { id: 1, atParams: { a: 999, b: 999 }, atV2Observations: [100, 200, 1000, 1000].map((quantity, index) => ({ assignmentPlanId: index + 1, quantity, allocatedLaborInputSeconds: 40 * quantity + 2000 })) };
...
test('one training quantity permits a provisional prediction, but one total quantity cannot validate extrapolation', () => {
  let assignmentPlanId = 0;
  const observation = quantity => ({ assignmentPlanId: ++assignmentPlanId, quantity, allocatedLaborInputSeconds: 60 * quantity });
