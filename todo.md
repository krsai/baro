...
- Done: added `migration_fix.sql` cleanup for legacy `AssignmentCard.payload` display/FK copy keys (`styleCode/styleName/previewUrl/orderNo/dueDate/customer/customerNameKo/customerNameVi/colorName/gender/styleId/workOrderId/buyerOrgId`). After deploy, the DB itself no longer retains those hidden fallback values; only the real FK columns + joins remain.
