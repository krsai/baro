...
시킬지 판단 포인트:
- 삭제 편의보다 데이터 무결성을 우선할지
- "기록이 있으면 삭제 금지" 정책을 운영에서 받아들일 수 있는지
...
시킬지 판단 포인트:
- 실제로 동시 수정이 자주 일어나는지
- 저장 충돌 시 에러를 보여주고 다시 불러오게 하는 UX가 괜찮은지

### [중간] 보드 날짜 범위 밖에서 오늘 인덱스를 0으로 잡는 문제

문제:
`AssignBoard.jsx`의 `getTodayDayIndex`가 현재 보드 범위 밖인 날짜에서도 `0`으로 fallback하면, 재배치 기준이 의도치 않게 맨 앞 날짜로 잡힐 수 있다.

해야 할 일:
- [ ] 보드 범위 밖일 때 `0`으로 fallback하지 않도록 수정

시킬지 판단 포인트:
- 자동 편의보다 잘못된 자동 배치를 막는 쪽이 더 중요한지
...
시킬지 판단 포인트:
- 화면이 덜 매끈해져도 데이터 이상을 숨기지 않는 정책이 맞는지
...
시킬지 판단 포인트:
- 운영자가 "입력 오류"와 "생산 편차"를 다른 문제로 받아들이도록 UI를 나눌 필요가 있는지
