...
const TEXT = {
  ko: {
    ...
    trade: '2. 거래 조건', productionType: '생산 방식', accessories: '부자재 구매', logistics: '베트남 현지 물류비', buyerPays: '고객 부담', factoryPays: '우리 회사 부담', accessoryCost: '한 벌 부자재비', logisticsCost: '총 현지 물류비',
    time: '3. 예상 작업시간', direct: '직접 입력', similar: '유사 스타일 참조', seconds: '한 벌 예상시간', similarStyle: '비슷한 스타일', bucketNote: '향후 실제 개발에서는 입력 수량에 맞는 관계 범위 ST(q) 버킷과 버전을 조회합니다.',
    production: '4. 생산 조건', workers: '투입 인원', advanced: '세부 계산 조건', hoursDay: '1일 근무시간', workdays: '월 근무일', wage: '1인 월 인건비',
    cost: '5. 비용과 목표', fixed: '월 고정비', directCost: '한 벌 직접비', margin: '목표 이익률', fixedHint: '임대료·전기료·관리비 등 월 고정비 합계', directHint: '재료비·외주비 등 한 벌당 직접 발생 비용',
  ...
  en: {
    ...
    trade: '2. Commercial Terms', productionType: 'Production Type', accessories: 'Trims Purchasing', logistics: 'Vietnam Local Logistics', buyerPays: 'Customer Pays', factoryPays: 'Our Company Pays', accessoryCost: 'Trims Cost per Piece', logisticsCost: 'Total Local Logistics Cost',
    time: '3. Estimated Work Time', direct: 'Direct Input', similar: 'Reference Similar Style', seconds: 'Seconds per Piece', similarStyle: 'Similar Style', bucketNote: 'The final version will resolve the relationship-scoped ST(q) bucket and version for the entered quantity.',
    production: '4. Production Conditions', workers: 'Workers', advanced: 'Detailed Calculation Settings', hoursDay: 'Hours per Day', workdays: 'Workdays per Month', wage: 'Monthly Labor Cost per Worker',
    cost: '5. Costs and Target', fixed: 'Monthly Fixed Cost', directCost: 'Direct Cost per Piece', margin: 'Target Profit Margin', fixedHint: 'Total monthly rent, electricity, management, and other fixed costs', directHint: 'Materials, outsourcing, and other direct cost per piece',
  ...
  vi: {
    ...
    trade: '2. Điều kiện thương mại', productionType: 'Hình thức sản xuất', accessories: 'Mua phụ liệu', logistics: 'Phí logistics nội địa Việt Nam', buyerPays: 'Khách hàng chịu', factoryPays: 'Công ty chúng ta chịu', accessoryCost: 'Chi phí phụ liệu mỗi sản phẩm', logisticsCost: 'Tổng phí logistics nội địa',
    time: '3. Thời gian dự kiến', direct: 'Nhập trực tiếp', similar: 'Tham khảo kiểu tương tự', seconds: 'Giây mỗi sản phẩm', similarStyle: 'Kiểu dáng tương tự', bucketNote: 'Bản chính thức sẽ tìm bucket và phiên bản ST(q) theo quan hệ phù hợp với số lượng đã nhập.',
    production: '4. Điều kiện sản xuất', workers: 'Số người', advanced: 'Điều kiện tính chi tiết', hoursDay: 'Giờ làm mỗi ngày', workdays: 'Ngày làm mỗi tháng', wage: 'Chi phí lao động tháng mỗi người',
    cost: '5. Chi phí và mục tiêu', fixed: 'Chi phí cố định tháng', directCost: 'Chi phí trực tiếp mỗi sản phẩm', margin: 'Tỷ suất lợi nhuận mục tiêu', fixedHint: 'Tổng tiền thuê, điện, quản lý và chi phí cố định hàng tháng', directHint: 'Nguyên liệu, gia công ngoài và chi phí trực tiếp mỗi sản phẩm',
...
const RevenueForecast = () => {
  ...
  const [draft, setDraft] = useState({ category: 'Jacket', styleName: 'AJ2000', quantity: 300, productionType: 'CMT', accessoriesPayer: 'buyer', logisticsPayer: 'buyer', accessoryCost: 35000, logisticsCost: 3000000, timeMode: 'similar', similarStyleId: 'AJ1972', seconds: 2163, workers: 8, hoursDay: 8, workdays: 26, wage: 9000000, fixed: 80000000, directCost: 120000, margin: 20 });
  const set = (key) => (event, value) => setDraft((current) => ({ ...current, [key]: value ?? event.target.value }));
  const setProductionType = (_event, value) => {
    if (!value) return;
    setDraft((current) => ({ ...current, productionType: value, accessoriesPayer: value === 'FP' ? 'factory' : 'buyer', logisticsPayer: value === 'FP' ? 'factory' : 'buyer' }));
  };
  ...
  const calculate = (quantity) => {
    ...
    const commercial = (draft.accessoriesPayer === 'factory' ? qty * number(draft.accessoryCost) : 0) + (draft.logisticsPayer === 'factory' ? number(draft.logisticsCost) : 0);
    const direct = qty * number(draft.directCost) + commercial; const labor = number(draft.wage) * workers * durationDays / workdays; const fixed = number(draft.fixed) * durationDays / workdays;
  ...
  return <AppPageContainer title={text.title}><Stack spacing={2.5}>
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(360px, 0.9fr) minmax(420px, 1.1fr)' }, gap: 2.5, alignItems: 'start' }}>
      <Stack spacing={2}>
        ...
        <FieldCard title={text.trade}>
          <ToggleButtonGroup exclusive fullWidth value={draft.productionType} onChange={setProductionType} size="small"><ToggleButton value="CMT">CMT</ToggleButton><ToggleButton value="FP">FP</ToggleButton></ToggleButtonGroup>
          <FormControl><InputLabel>{text.accessories}</InputLabel><Select label={text.accessories} value={draft.accessoriesPayer} onChange={set('accessoriesPayer')}><MenuItem value="buyer">{text.buyerPays}</MenuItem><MenuItem value="factory">{text.factoryPays}</MenuItem></Select></FormControl>
          {draft.accessoriesPayer === 'factory' && <TextField label={text.accessoryCost} type="number" value={draft.accessoryCost} onChange={set('accessoryCost')} InputProps={{ endAdornment: <InputAdornment position="end">₫</InputAdornment> }} />}
          <FormControl><InputLabel>{text.logistics}</InputLabel><Select label={text.logistics} value={draft.logisticsPayer} onChange={set('logisticsPayer')}><MenuItem value="buyer">{text.buyerPays}</MenuItem><MenuItem value="factory">{text.factoryPays}</MenuItem></Select></FormControl>
          {draft.logisticsPayer === 'factory' && <TextField label={text.logisticsCost} type="number" value={draft.logisticsCost} onChange={set('logisticsCost')} InputProps={{ endAdornment: <InputAdornment position="end">₫</InputAdornment> }} />}
        </FieldCard>
        ...
        <FieldCard title={text.production}><TextField label={text.workers} type="number" value={draft.workers} onChange={set('workers')} /><Box component="details" sx={{ '& summary': { cursor: 'pointer', color: 'primary.main', fontSize: '0.875rem', fontWeight: 700 }, '&[open] summary': { mb: 2 } }}><Box component="summary">{text.advanced}</Box><Stack spacing={2}><TextField label={text.hoursDay} type="number" value={draft.hoursDay} onChange={set('hoursDay')} /><TextField label={text.workdays} type="number" value={draft.workdays} onChange={set('workdays')} /><TextField label={text.wage} type="number" value={draft.wage} onChange={set('wage')} InputProps={{ endAdornment: <InputAdornment position="end">₫</InputAdornment> }} /></Stack></Box></FieldCard>
