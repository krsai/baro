    ...
    <Stack spacing={2} sx={{ maxWidth: 1180 }}>
      <Section icon={<BusinessRoundedIcon />} title={text.identity} help={text.identityHelp}>
        <Grid container spacing={2}>
          <Grid item xs={12} md={3}><TextField {...common} required label={t('customerCode')} name="code" value={customerFormData.code} onChange={onBasicFieldChange} /></Grid>
          <Grid item xs={12} md={6}><TextField {...common} required label={text.nameEn} name="name" value={customerFormData.name} onChange={onBasicFieldChange} /></Grid>
          <Grid item xs={12} md={3}><TextField {...common} label={t('registeredAt')} value={formatCustomerDate(customerFormData.registeredAt, languageCode)} disabled /></Grid>
          <Grid item xs={12} md={6}><TextField {...common} label={text.nameKo} name="nameKo" value={customerFormData.nameKo} onChange={onBasicFieldChange} /></Grid>
          <Grid item xs={12} md={6}><TextField {...common} label={text.nameVi} name="nameVi" value={customerFormData.nameVi} onChange={onBasicFieldChange} /></Grid>
        </Grid>
      </Section>

      <Section icon={<StraightenRoundedIcon />} title={text.order} help={text.orderHelp} accent="#7c3aed">
      ...
      <Section icon={<LocationOnRoundedIcon />} title={text.address} help={text.addressHelp} accent="#059669">
        <Grid container spacing={2}>
          <Grid item xs={12} md={4}><TextField {...common} select label={t('country')} name="country" value={normalizeCountry(customerFormData.country) || 'VN'} onChange={onCountryChange}>{countryOptions.map((option) => <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>)}</TextField></Grid>
          <Grid item xs={12} md={8}><TextField {...common} label={t('address')} name="address" value={customerFormData.address} onChange={onBasicFieldChange} placeholder={text.addressPlaceholder} multiline minRows={2} /></Grid>
        </Grid>
      </Section>
