...
const AttrGender = () => {
  const [items, setItems] = useState(initialGenders);
  ...
  return (
    <Box>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="xs" fullWidth>
        <DialogTitle>{editingItem ? '성별 수정' : '신규 성별 등록'}</DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <TextField name="name" label="성별" value={formData.name} onChange={handleInputChange} fullWidth />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
