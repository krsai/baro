    ...
    {tab === 0 ? <Stack direction={{ xs: 'column', lg: 'row' }} spacing={2} alignItems="stretch">
      ...
      <Paper variant="outlined" sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Box sx={{ p: 2, bgcolor: 'action.hover', borderBottom: 1, borderColor: 'divider' }}><Typography fontWeight={700} sx={{ mb: 1.5 }}>계산 방식</Typography>
          ...
          {designVersion === 2 && <Paper variant="outlined" sx={{ mt: 1.5, p: 2, bgcolor: 'background.paper', borderColor: 'primary.light' }}><Stack spacing={1.5}><Stack direction="row" alignItems="center"><Box><Typography fontWeight={700}>빠른 계산식 조립</Typography><Typography variant="caption" color="text.secondary">이 화면에서 모듈을 바로 추가하고 필요 없는 토큰은 삭제합니다.</Typography></Box><Chip label="인라인형" size="small" color="primary" sx={{ ml: 'auto' }} /></Stack>{selected.calcType === 'FORMULA' ? <><Stack direction="row" spacing={1} useFlexGap flexWrap="wrap" sx={{ p: 1.5, minHeight: 56, bgcolor: 'action.hover', borderRadius: 1 }}>{(selected.formula || []).map((token, index) => <Chip key={`${token}-${index}`} label={formulaTokenLabel(token)} onDelete={() => removeSelectedFormulaToken(index)} color={FORMULA_PARAMETERS[token] ? 'primary' : 'default'} variant={FORMULA_PARAMETERS[token] ? 'filled' : 'outlined'} />)}</Stack><Box><Typography variant="caption" color="text.secondary">값 모듈</Typography><Stack direction="row" spacing={0.75} useFlexGap flexWrap="wrap" sx={{ mt: 0.5 }}>{Object.entries(FORMULA_PARAMETERS).map(([key, parameter]) => <Button key={key} size="small" variant="outlined" onClick={() => appendSelectedFormulaToken(key)}>{parameter.label}</Button>)}</Stack></Box><Box><Typography variant="caption" color="text.secondary">연산 모듈</Typography><Stack direction="row" spacing={0.75} useFlexGap flexWrap="wrap" sx={{ mt: 0.5 }}>{FORMULA_OPERATORS.map((operator) => <Button key={operator} size="small" color="inherit" variant="outlined" onClick={() => appendSelectedFormulaToken(operator)}>{operator}</Button>)}</Stack></Box></> : <Typography fontWeight={700}>{selected.calcType === 'EXTERNAL' ? formulaLabel(selected.formula) : selected.calcType === 'CONDITIONAL' ? `${CONDITIONS[selected.condition]} 직급별 정액 지급` : '직급별 정액 지급'}</Typography>}</Stack></Paper>}
