    ...
    <Stack direction={{ xs: 'column', lg: 'row' }} spacing={2} alignItems="stretch">
      ...
      <Paper variant="outlined" sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ md: 'center' }}><Typography variant="body2" fontWeight={700}>{t('지급 설정')}</Typography><Stack direction="row" spacing={1} useFlexGap flexWrap="wrap" alignItems="center">{PAY_TYPE_ORDER.map((payType) => { const active = (selected.payTypes || []).includes(payType); const isLastActiveType = active && (selected.payTypes || []).length === 1; return <FormControlLabel key={payType} control={<Checkbox size="small" checked={active} disabled={isFixedIncentive || isLastActiveType} onChange={() => toggleSelectedPayType(payType)} />} label={t(PAY_TYPES[payType]?.label || payType)} sx={{ m: 0 }} />; })}<Chip size="small" variant="outlined" label={t(PAY_CYCLES[selected.payCycle])} />{selected.payCycle !== 'MONTHLY' && <Chip size="small" variant="outlined" label={(selected.paymentMonths || []).map((month) => `${month}${t('월')}`).join(' · ')} />}{selected.capValue && <Chip size="small" variant="outlined" label={`${t('상한')} ${money(selected.capValue)} ${currencyCode}`} />}</Stack></Stack>
