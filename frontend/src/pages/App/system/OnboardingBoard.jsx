...
const OnboardingBoard = () => {
  ...
  const adminEmail = useMemo(
    () => (typeof activeProfile?.email === 'string' ? activeProfile.email.trim().toLowerCase() : ''),
    [activeProfile?.email]
  );
  ...
  const handleApproveMembership = async (requestRow) => {
    if (!requestRow?.id) return;
    const key = `membership-approve-${requestRow.id}`;
    setProcessingKey(key);
    try {
      await requestJSON(`/org-memberships/${requestRow.id}/approve`, {
        method: 'PATCH',
      ...
      showNotification('소속 회사 신청을 승인했습니다.', 'success');
  ...
  const handleRejectMembership = async (requestRow) => {
    if (!requestRow?.id) return;
    const key = `membership-reject-${requestRow.id}`;
    setProcessingKey(key);
    try {
      await requestJSON(`/org-memberships/${requestRow.id}/reject`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          approvedBy: adminEmail,
      ...
      showNotification('소속 회사 신청을 거절했습니다.', 'success');
      await fetchRequests({ isRefresh: true });
    } catch (error) {
      ...
      header={
        <>
          ...
          <Typography sx={{ mt: 1, color: 'text.secondary' }}>
            신규 소셜 로그인 사용자의 회사 소속 신청/신규 회사 등록 요청을 승인합니다.
          </Typography>
        </>
      }
    >
      <Grid container spacing={3}>
        <Grid item xs={12}>
          <Paper variant="outlined" sx={{ p: 3 }}>
            ...
            >
              <Typography variant="h6">소속 회사 선택 요청</Typography>
              <Button
                variant="outlined"
                size="small"
            ...
            {loading ? (
              ...
              <Table size="small">
                ...
                <TableBody>
                  {pendingMembershipRequests.map((row) => (
                    <TableRow key={row.id}>
                      <TableCell>{toDateTimeText(row.requestedAt || row.createdAt)}</TableCell>
                      ...
                      <TableCell align="right">
                        <Box sx={{ display: 'inline-flex', gap: 1 }}>
                          <Button
                            size="small"
                            variant="contained"
                            onClick={() => handleApproveMembership(row)}
                            disabled={processingKey !== ''}
                          >
                            {processingKey === `membership-approve-${row.id}` ? '처리 중...' : '승인'}
                          </Button>
                          <Button
                            size="small"
                            variant="outlined"
                            color="error"
                            onClick={() => handleRejectMembership(row)}
                            disabled={processingKey !== ''}
                          >
                            {processingKey === `membership-reject-${row.id}` ? '처리 중...' : '거절'}
                          </Button>
                        </Box>
                      </TableCell>
                    </TableRow>
