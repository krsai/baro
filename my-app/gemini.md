...
- 각 공장은 **PT** 또는 **ST** 중 하나의 기준 시간을 선택하여 공임을 산출합니다.
  - **PT 공장**: `작업 1건 공임 = PT * 초당 임금`
  - **ST 공장**: `작업 1건 공임 = ST * 초당 임금`
...
## 9. 주요 변경 로그 (Major Change Log)

### 2026년 1월 29일
- **스타일 기본 정보 (`StyleBasicInfo`) 페이지 기능 개선:**
  - **이미지 삭제 기능 추가:** 사용자가 스타일 관련 이미지를 관리할 수 있도록 삭제 기능을 구현했습니다.
    - 각 이미지 썸네일에 마우스를 올리면 삭제 아이콘 버튼이 표시됩니다.
    - 삭제 버튼을 클릭하면, 사용자 실수를 방지하기 위해 "이 이미지를 삭제하시겠습니까?"라는 확인 창을 통해 재확인하는 로직을 추가했습니다.
