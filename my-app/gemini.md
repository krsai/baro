...
- **예시**:
  - 하노이 공장이 **PT 기준**으로 설정되어 있다면, 작업 기록 입력 시 해당 스타일의 공정 관리에 정의된 **PT 시간**을 호출합니다.
...
- **공정 정보 페이지 (Process Information Page):**
  - **평균 공정 시간 계산:** `작업 기록(Work)` 데이터를 기반으로, 공정별 평균 소요 시간을 계산하여 참고용으로 표시합니다.
  - **평균 시간의 분류:** 평균 시간은 `제품 카테고리`별로 분류되어야 합니다. (예: '셰프복 주머니 달기' vs '바지 주머니 달기') 이는 유사한 작업의 소요 시간을 예측하는 데 사용됩니다.
